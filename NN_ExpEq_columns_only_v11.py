...
KFOLD_GROUP_ALIAS: Dict[str, str] = {
    "015_jetti_project_file_amcf_6in": "015_jetti_project_file_amcf",
    "015_jetti_project_file_amcf_8in": "015_jetti_project_file_amcf",
}
...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "PythonProjects/Rosetta/NN_Pytorch_ExpEq_columns_only_v11"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v11")
...
DIRECT_INTERACTION_TERM_NAMES = (
    ...
    "column_height_m_raw",
    "column_inner_diameter_m_raw",
    "material_size_p80_in_raw",
    "column_height_m",
...
CONFIG = {
    ...
    "control_weight_tail_boost": 0.35,
    ...
    "loss_weights": {
        ...
        "cap":                      4.5,    # was 3.0 — back to v1 original
        ...
        "ferric_orp_aux":           0.10,
        # (D) New early-window shape penalties to fix slow takeoffs (SCL
        # Escondida, 026 secondary sulfide) and over-steep early predictions
        # on refractory ores (Elephant, Los Bronces).  ``slope_early`` is
        # |chord-slope_pred - chord-slope_true| over [0, slope_early_end_day].
        # ``t50_match`` is |t50_pred - t50_true| / 1000d, applied to both the
        # control and catalyzed curves.
        "slope_early":              0.75,
        "t50_match":                0.50,
    },
    "plot_dpi": 300,
    ...
    "leach_pct_primary_sulfides_catalyzed": 0.75,
...
MODEL_LOGIC_VERSION = "v11_cumulative_diff_pls_orp_col_level_20260417"
...
LATENT_INTERACTION_SPECS: Dict[str, Dict[str, float]] = {
    ...
    "primary_passivation_drive": {
        "residual_cpy_%": 0.72,
        "copper_primary_sulfides_equivalent": 0.78,
        "copper_secondary_sulfides_equivalent": -0.18,
        ...
        "material_size_to_column_diameter_ratio": 0.18,
        "bornite": -0.06,
    ...
    "primary_catalyst_synergy": {
        "residual_cpy_%": 0.46,
        "copper_primary_sulfides_equivalent": 0.54,
    ...
    "diffusion_drag_strength": {
        ...
        "apparent_bulk_density_t_m3": 0.32,
        "material_size_to_column_diameter_ratio": 0.48,
        "column_height_m": 0.24,
        "residual_cpy_%": 0.10,
        "copper_primary_sulfides_equivalent": 0.12,
        "copper_secondary_sulfides_equivalent": -0.12,
        "copper_oxides_equivalent": -0.12,
...
def generate_virtual_points(
    ...
    The biexponential fit parameters must be cap-free (fitted to observed data without any
    mineralogy cap constraint). The ``cap`` argument is the ONLY place where the
    mineralogy-derived leach cap is applied: virtual/augmented recovery values are clipped
    to ``cap`` so that the training targets never exceed the physics ceiling. Actual observed
    data points are never modified by the cap.
    ...
    # Apply mineralogy cap ONLY to virtual (augmented) points — never to real observations.
    # This is the single authoritative place where the cap constrains training targets.
    if cap is not None and np.isfinite(float(cap)) and float(cap) > 0.0:
        virtual_recovery = np.minimum(virtual_recovery, float(cap))
...
def augment_curve_with_virtual_data(
    ...
    ``cap`` is the mineralogy-derived leach cap for this specific curve
    (ctrl_cap for control curves, cat_cap for catalyzed curves). It is forwarded
    to generate_virtual_points so that augmented recovery values are clipped to
    the physics ceiling. Real observations are never modified.
    """
    catalyst_history_window_days = float(
        CONFIG.get("catalyst_extension_window_days", 21.0)
        if catalyst_history_window_days is None
    ...
    pred = double_exp_curve_np(curve.time, fit_params[0], fit_params[1], fit_params[2], fit_params[3])
    r2, rmse, _ = calculate_fit_metrics(curve.recovery, pred)
...
def get_pair_training_tensors(
    ...
    bundle["cat_ctrl_start_day"] = bundle["cat_start_day"]
...
def get_pair_plot_profile(pair: PairSample) -> Dict[str, Any]:
    ...
    _start = float(pair.catalyzed.catalyst_start_day)
    _stop = float(pair.catalyzed.catalyst_stop_day)
    if np.isfinite(_start) or np.isfinite(_stop):
        ...
        if np.isfinite(_stop):
            plot_profile["catalyst_addition_stop_day"] = _stop
...
def apply_training_pair_exclusions(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    ...
    if TARGET_COLUMNS in df.columns:
        ...
        if n_auto > 0:
            ...
            print(
                f"[Data v11] Auto-excluded {n_auto} rows with <{MIN_VALID_RECOVERY_POINTS} valid "
...
def prefit_biexponential_for_rows(df: pd.DataFrame) -> pd.DataFrame:
    prefit_parallel = resolve_prefit_parallelism(len(df))
    row_payloads = build_prefit_row_payloads(df)

    if int(prefit_parallel["workers"]) <= 1 or len(row_payloads) <= 1:
        return pd.DataFrame([prefit_biexponential_for_row_payload(row) for row in row_payloads])
...
def build_prefit_row_payloads(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    return df.loc[:, input_cols].to_dict(orient="records")
...
def prefit_biexponential_for_row_payload(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fit without cap: prefit parameters must reflect observed kinetics only.
    # The cap is applied downstream in generate_virtual_points when augmenting data.
    params = fit_biexponential_params(t, y, cap=None) if t.size >= 6 else np.array([np.nan] * 4, dtype=float)
...
def build_pair_samples(df: pd.DataFrame) -> List[PairSample]:
    ...
    for sample_id, group in grouped:
        ...
        for idx, row in group.iterrows():
            ...
            # Fallback refit when prefit CSV values are missing — also cap-free so that
            # fit_params always represent observed kinetics, never the mineralogy ceiling.
            if not np.all(np.isfinite(fit_params)):
                fit_params = fit_biexponential_params(t, y, cap=None)
        ...
        for ctrl_curve, ctrl_static, ctrl_input_only, ctrl_project_name, ctrl_feed_orp, ctrl_pls_orp in by_status["Control"]:
            for cat_curve, cat_static, cat_input_only, cat_project_name, cat_feed_orp, cat_pls_orp in by_status["Catalyzed"]:
                ...
                # v11: caps derived from per-sample mineralogy (project_sample_id level).
...
class PairCurveNet(nn.Module):
    def __init__(
        ...
        # Catalyzed fraction maps to [0.15, 0.80], control fraction maps to
        # [0.05, 0.45]. These replace the scalar CONFIG constants
        # `leach_pct_primary_sulfides_catalyzed` / `_control` when the caps
        # are computed inside `_compute_sample_leach_caps_torch`. The bias
        # init centres them at mid-range so early training is close to the
        # old constants.
        ...
        nn.init.constant_(self.cpy_leach_frac_cat_head.bias, 0.0)   # sigmoid(0)=0.5 → 0.475
        nn.init.constant_(self.cpy_leach_frac_ctrl_head.bias, 0.0)  # sigmoid(0)=0.5 → 0.25
    ...
    def _compute_residual_primary_uplift_capacity(
        ...
        pct_ox = float(CONFIG.get("leach_pct_oxides", 1.00))
        pct_sec = float(CONFIG.get("leach_pct_secondary_sulfides", 0.67))
        pct_pri_ctrl_default = float(CONFIG.get("leach_pct_primary_sulfides_control", 0.30))
        pct_pri_cat_default = float(CONFIG.get("leach_pct_primary_sulfides_catalyzed", 0.70))
    ...
    def predict_params(
        ...
        catalyst_use_frac = torch.sigmoid(self.use_frac_head(h))
        ...
        # (B) Per-sample leachable primary-sulfide fractions. These override
        # the scalar CONFIG constants in the chemistry cap computation. The
        # catalyzed fraction is bounded in [0.15, 0.80]; the control fraction
        # in [0.05, 0.45]. We also enforce cat >= ctrl by construction.
        cpy_cat_frac = 0.15 + 0.65 * torch.sigmoid(self.cpy_leach_frac_cat_head(h))
        cpy_ctrl_frac = 0.05 + 0.40 * torch.sigmoid(self.cpy_leach_frac_ctrl_head(h))
        cpy_ctrl_frac = torch.minimum(cpy_ctrl_frac, cpy_cat_frac)
        ...
        acid_amp_pp = 4.0 * torch.sigmoid(self.acid_consumption_amp_head(h))
        acid_tau_days = 15.0 + 150.0 * torch.sigmoid(self.acid_consumption_tau_head(h))
        acid_gate_raw = torch.sigmoid(self.acid_consumption_gate_head(h))
    ...
    def curves_given_params(
        ...
        # (B) If the network produced per-sample primary-sulfide
        # leachable fractions, pass them to the chemistry cap. This lets
        # refractory samples (Elephant, Los Bronces) learn a lower cap and
        # fast-kinetics samples learn a higher one.
...
def _pair_training_loss_from_tensors(
    ...
    _, pred_cat, cat_states = model.curves_given_params(
        ...
        kappa,
        aging_strength,
        catalyst_start_day_override=tensors["cat_start_day"],
        latent_params=latent,
        return_states=True,
    )
    ...
    loss_cat_uplift = _weighted_smooth_l1_loss(pred_gap, true_gap, cat_fit_weight)
    ...
    total = (
        ...
        + float(loss_weights.get("slope_early", 0.0)) * slope_early_pen
...
def predict_pair_record(
    ...
    with torch.inference_mode():
        ...
        _, pred_cat_plot = model.curves_given_params(
            ...
            kappa,
            aging_strength,
            catalyst_start_day_override=plot_tensors["cat_plot_start_day"],
            latent_params=latent,
        )
...
def aggregate_ensemble_predictions(
    ...
    for pair in pairs:
        ...
        rec = {
            ...
            "control_pred_p10": _nanpercentile_array(ctrl_stack, pi_low, axis=0),
            "control_pred_p90": _nanpercentile_array(ctrl_stack, pi_high, axis=0),
            ...
            "catalyzed_pred_p10": _nanpercentile_array(cat_stack, pi_low, axis=0),
            "catalyzed_pred_p90": _nanpercentile_array(cat_stack, pi_high, axis=0),
            ...
            "control_pred_plot_p10": _nanpercentile_array(plot_ctrl_stack, pi_low, axis=0),
            "control_pred_plot_p90": _nanpercentile_array(plot_ctrl_stack, pi_high, axis=0),
            "catalyzed_pred_plot_mean": _nanmean_array(plot_cat_stack, axis=0),
            "catalyzed_pred_plot_p10": _nanpercentile_array(plot_cat_stack, pi_low, axis=0),
            "catalyzed_pred_plot_p90": _nanpercentile_array(plot_cat_stack, pi_high, axis=0),
...
def plot_ensemble_record(record: Dict[str, Any], plot_path: str, title: str) -> None:
    ...
    plt.plot(kt, kp_lo, "-", lw=0.9, color="#ff7f0e", alpha=0.35, zorder=1)
    plt.plot(kt, kp_hi, "-", lw=0.9, color="#ff7f0e", alpha=0.35, zorder=1)
    ...
    plt.plot(ct, cp_lo, "-", lw=0.9, color="#1f77b4", alpha=0.35, zorder=4)
    plt.plot(ct, cp_hi, "-", lw=0.9, color="#1f77b4", alpha=0.35, zorder=4)
...
def plot_ensemble_records_per_sample(
    ...
    for sample_id, grouped_curves in sorted(sample_groups.items(), key=lambda item: item[0]):
        ...
        for idx, curve in enumerate(control_curves):
            ...
            ax.plot(curve["plot_t"], curve["pred_p10"], color=color, lw=0.8, alpha=0.30, zorder=1)
            ax.plot(curve["plot_t"], curve["pred_p90"], color=color, lw=0.8, alpha=0.30, zorder=1)
            ax.plot(
                curve["plot_t"],
                curve["pred_mean"],
                color=color,
        ...
        for idx, curve in enumerate(catalyzed_curves):
            ...
            ax.plot(curve["plot_t"], curve["pred_p10"], color=color, lw=0.8, alpha=0.32, zorder=4)
            ax.plot(curve["plot_t"], curve["pred_p90"], color=color, lw=0.8, alpha=0.32, zorder=4)
            ax.plot(
                curve["plot_t"],
                curve["pred_mean"],
                color=color,
...
def main() -> None:
    ...
    print(f"[Data v11] Loading from: {DATA_PATH}")
    usecols = resolve_dataset_usecols(DATA_PATH)
    df = pd.read_csv(DATA_PATH, sep=",", usecols=usecols)
    print(f"[Data v11] Loaded {len(df)} rows × {len(df.columns)} columns | "
    ...
    # col_id row and used only to initialize / augment training data.
    ...
    # for each of the 500 members (the expensive pre-executor rebuild path).
    pairs_observed_by_sid: Dict[str, "PairSample"] = {p.sample_id: p for p in pairs_observed}
    pairs_training_by_sid: Dict[str, "PairSample"] = {p.sample_id: p for p in pairs_training}
    ...
    for split in cv_splits:
        ...
        val_pairs_metrics_seed = val_pairs_observed_seed
    ...
    val_pairs_for_agg = [p for p in pairs_observed if p.pair_id in val_union_set]
