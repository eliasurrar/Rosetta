...
BASE_SCRIPT_PATH = os.path.join(THIS_DIR, "NN_ExpEq_columns_only_v8.py")
MINERALOGY_CLUSTERING_PATH = os.path.join(THIS_DIR, "rosetta_mineralogy_clustering.py")


def load_base_module() -> Any:
    spec = importlib.util.spec_from_file_location("nn_expeq_columns_only_v8_base", BASE_SCRIPT_PATH)
...
def candidate_project_roots() -> List[str]:
    ...
    for attr in ["PROJECT_ROOT", "DEFAULT_PROJECT_ROOT", "LOCAL_PROJECT_ROOT"]:
        ...
        if isinstance(value, str) and value not in roots:
            roots.append(value)
    sibling_root = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v8")
...
def resolve_project_root(requested_root: str | None = None) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v8.py first so it writes the checkpoint paths listed in outputs/run_manifest.json.\n"
        f"Searched:\n{searched}"
    )
...
def resolve_member_model_dir(project_root: str) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v8.py first and then use the checkpoint directory recorded in outputs/run_manifest.json.\n"
        f"Searched:\n{searched_text}"
    )
        ...
        [
            html.Div(
                [
                    html.Div(
                        label,
                        style={"fontWeight": 600, "fontSize": "13px", "lineHeight": "1.2", "minWidth": "0"},
                    ),
                    html.Div(
                        [
                            dcc.Input(
...
def load_saved_member_models(project_root: str) -> Tuple[List[Dict[str, Any]], str, List[str]]:
    ...
    if len(model_paths) == 0:
        raise FileNotFoundError(
            ...
            "Run NN_ExpEq_columns_only_v8.py first to generate them."
        )
    ...
    for path in model_paths:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Re-run NN_ExpEq_columns_only_v8.py to regenerate the saved member models with the current model structure."
            ) from exc
        model.eval()
        members.append(
            {
    ...
    if len(members) == 0:
        ...
        raise RuntimeError(
            ...
            "Re-run NN_ExpEq_columns_only_v8.py to regenerate them.\n"
            f"Skipped:\n{skipped_text}"
        )
...
def make_prediction_figure(pred: Dict[str, Any], max_day: float) -> go.Figure:
    ...
    fig.update_layout(
        ...
        title="Interactive Ensemble Prediction (v8)",
        height=760,
        ...
        meta={
            "png_export_filename": "interactive_ensemble_prediction_v8_300dpi",
            "png_export_annotation_full": full_annotation_text,
            "png_export_annotation_compact": compact_annotation_text,
        },
    )
    fig.update_xaxes(title_text="Leach duration (days)", range=[0, float(max_day)])
    fig.update_yaxes(title_text="Cu Recovery %", range=[0, 80])
...
def build_live_constraints_summary(
    geometry_values: Dict[str, float],
    geometry_guardrail_state: Dict[str, Any],
    grouped_balance_state: Dict[str, Any],
    resolved_chemistry: Dict[str, float],
    ...
    note_text = str(geometry_guardrail_state.get("note", "")).strip()
    grouped_note = str(grouped_balance_state.get("note", "")).strip()
    status_color = "#b42318" if (not ratio_ok or not density_ok or grouped_note) else "#55606d"
    ...
    if note_text:
        children.append(
            html.Div(
                note_text,
                style={"fontSize": "12px", "color": status_color, "lineHeight": "1.5"},
            )
        )
    if grouped_note:
        children.append(
            html.Div(
                grouped_note,
                style={"fontSize": "12px", "color": "#b42318", "marginTop": "6px", "lineHeight": "1.5"},
            )
        )
    if len(children) == 0:
        return html.Div()
...
def create_app(project_root: str) -> Dash:
    ...
    app = Dash(__name__, title="Rosetta | Jetti Resources")
    app.index_string = """
    <!DOCTYPE html>
    <html>
        <head>
    ...
    app.layout = html.Div(
        [
            ...
            dcc.Store(
                ...
                data={
                    "filename": "interactive_ensemble_prediction_v8_300dpi",
                ...
                [
                    html.Div(
                        [
                                ...
                                [
                                    html.Div(
                                        [
                                            ...
                                            html.Div(
                                                "Downloads two PNGs: the full annotation version and a confidence-band-only version.",
                            ...
                            html.Div(
                                id="live-constraints-summary",
                                children=build_live_constraints_summary(
    ...
    app.clientside_callback(
        """
        function(nClicks, figure, exportOptions) {
            if (!nClicks || !figure || !window.Plotly) {
            ...
            const fullFigure = cloneObj(figure);
            const compactFigure = cloneObj(figure);
            const baseFilename = (exportOptions && exportOptions.filename) || meta.png_export_filename || "interactive_ensemble_prediction_v8_300dpi";
            const compactSuffix = (exportOptions && exportOptions.compact_suffix) || "confidence_band_only";
            const fullAnnotation = meta.png_export_annotation_full || null;
            const compactAnnotation = meta.png_export_annotation_compact || null;

            if (fullAnnotation && fullFigure.layout && Array.isArray(fullFigure.layout.annotations) && fullFigure.layout.annotations.length > 0) {
                fullFigure.layout.annotations[0].text = fullAnnotation;
            }
            if (compactAnnotation && compactFigure.layout && Array.isArray(compactFigure.layout.annotations) && compactFigure.layout.annotations.length > 0) {
                compactFigure.layout.annotations[0].text = compactAnnotation;
            }

            const options = exportOptions || {width: 1200, height: 800, scale: 1};
            (async () => {
                await renderAndDownload(fullFigure, baseFilename, options);
                await new Promise((resolve) => setTimeout(resolve, 150));
                await renderAndDownload(compactFigure, `${baseFilename}_${compactSuffix}`, options);
            })();
    ...
    @app.callback(
        *[Output(input_id, "value") for input_id in control_input_ids],
        ...
        Output("live-constraints-summary", "children"),
    ...
    def sync_input_values(*callback_values: float) -> Tuple[Any, ...]:
        ...
        ui_values, _feed_mass_state = resolve_internal_feed_mass(ui_values, input_specs)
        static_values, _derived_values, _inactive_grouped_values, grouped_balance_state, resolved_chemistry = resolve_model_static_values(ui_values, input_specs)
        ...
        outputs.extend(
            [
                ...
                build_live_constraints_summary(
                    geometry_values=geometry_values,
    ...
    def download_predictions_excel(
        ...
        return dcc.send_bytes(excel_bytes, "interactive_ensemble_prediction_v8_7day.xlsx")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Interactive Plotly/Dash app for NN_ExpEq_columns_only_v8 using the saved "
            "member checkpoints created after running NN_ExpEq_columns_only_v8.py."
        )
    )
    ...
    parser.add_argument(
        "--project-root",
        default=None,
        help="Optional override for the NN_Pytorch_ExpEq_columns_only_v8 root.",
    )
