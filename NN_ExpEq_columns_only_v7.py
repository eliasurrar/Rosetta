...
DEFAULT_PROJECT_ROOT = os.environ.get(
    ...
    (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_ExpEq_columns_only_v7"
    ),
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v7")
...
MODEL_LOGIC_VERSION = "v7_raw_chem_uplift_20260401"
...
def fit_biexponential_params(t_days: np.ndarray, recovery: np.ndarray, cap: Optional[float] = None) -> np.ndarray:
    ...
    max_asym = float(CONFIG.get("fit_max_asymptote", 80.0))
    plateau_day = float(CONFIG.get("ensemble_plot_target_day", 2500.0) * 4)
    ...
    n_pts = len(t)
    weights = np.linspace(1.0, 5.0, n_pts)
...
class PairCurveNet(nn.Module):
    ...
    def curves_given_params(
        ...
        base_uplift = F.softplus(y_cat_base - y_ctrl)
        ...
        cat_gap_raw = base_uplift * separation_factor
