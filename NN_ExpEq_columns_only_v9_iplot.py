...
BASE_SCRIPT_PATH = os.path.join(THIS_DIR, "NN_ExpEq_columns_only_v9.py")
...
def load_base_module() -> Any:
    spec = importlib.util.spec_from_file_location("nn_expeq_columns_only_v9_base", BASE_SCRIPT_PATH)
...
def candidate_project_roots() -> List[str]:
    ...
    for attr in ["PROJECT_ROOT", "DEFAULT_PROJECT_ROOT", "LOCAL_PROJECT_ROOT"]:
        ...
        if isinstance(value, str) and value not in roots:
            roots.append(value)
    sibling_root = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v9")
...
def resolve_project_root(requested_root: str | None = None) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v9.py first so it writes the checkpoint paths listed in outputs/run_manifest.json.\n"
        f"Searched:\n{searched}"
    )
...
def resolve_member_model_dir(project_root: str) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v9.py first and then use the checkpoint directory recorded in outputs/run_manifest.json.\n"
        f"Searched:\n{searched_text}"
    )
...
def load_saved_member_models(project_root: str) -> Tuple[List[Dict[str, Any]], str, List[str]]:
    ...
    if len(model_paths) == 0:
        raise FileNotFoundError(
            ...
            "Run NN_ExpEq_columns_only_v9.py first to generate them."
        )
    ...
    for path in model_paths:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Re-run NN_ExpEq_columns_only_v9.py to regenerate the saved member models with the current model structure."
            ) from exc
        model.eval()
    ...
    if len(members) == 0:
        ...
        raise RuntimeError(
            ...
            "Re-run NN_ExpEq_columns_only_v9.py to regenerate them.\n"
            f"Skipped:\n{skipped_text}"
        )
...
def make_prediction_figure(pred: Dict[str, Any], max_day: float, theme: str = "dark") -> go.Figure:
    ...
    fig.update_layout(
        ...
        title={
            "text": "Interactive Ensemble Prediction (v9)",
        ...
        meta={
            "png_export_filename": "interactive_ensemble_prediction_v9_300dpi",
...
def make_display_figure(
    ...
    if is_mobile:
        ...
        figure.update_layout(
            title_text="Interactive Ensemble Prediction (v9)",
            title_font_size=24,
            margin={"l": 62, "r": 24, "t": 66, "b": 56},
            legend={
                "orientation": "h",
...
def create_app(project_root: str) -> Dash:
    ...
    app.layout = html.Div(
        [
            ...
            dcc.Store(
                ...
                data={
                    "filename": "interactive_ensemble_prediction_v9_300dpi",
    ...
    app.clientside_callback(
        ...
        function(nClicks, exportFigures, exportOptions) {
            ...
            const baseFilename = (exportOptions && exportOptions.filename) || lightMeta.png_export_filename || darkMeta.png_export_filename || "interactive_ensemble_prediction_v9_300dpi";
    ...
    def download_predictions_excel(
        ...
        return dcc.send_bytes(excel_bytes, "interactive_ensemble_prediction_v9_7day.xlsx")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Interactive Plotly/Dash app for NN_ExpEq_columns_only_v9 using the saved "
            "member checkpoints created after running NN_ExpEq_columns_only_v9.py."
        )
    )
    ...
    parser.add_argument(
        "--project-root",
        default=None,
        help="Optional override for the NN_Pytorch_ExpEq_columns_only_v9 root.",
    )
