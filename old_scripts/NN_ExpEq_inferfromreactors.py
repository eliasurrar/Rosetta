...
DEFAULT_PROJECT_ROOT = "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/Rosetta/NN_Pytorch_ExpEq_inferfromReactors"
LOCAL_PROJECT_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "NN_Pytorch_ExpEq_inferfromReactors")
...
def _constrain_catalyzed_params_against_control(
    ...
    if catalyst_signal > min_signal:
        a_uplift = float(
            np.clip(
                coupling_cfg["a_uplift_per_kg_t"] * catalyst_signal * prior_boost,
                0.0,
                coupling_cfg["a_uplift_max"],
            )
        )
        a_fast_frac = float(np.clip(coupling_cfg["a_uplift_fast_fraction"], 0.0, 1.0))
        b_mult = 1.0 + float(
            np.clip(
                coupling_cfg["b_uplift_per_kg_t"] * catalyst_signal * prior_boost,
                0.0,
...
def _prepare_member_params_for_sample(
    ...
    for j, row_idx in enumerate(sample_df.index):
        ...
        for pred in member_param_preds[:, j, :]:
            ...
            p = _apply_column_kinetics_caps(
                params=p,
                status=normalize_status(row_meta.get(CATALYZED_COLUMNS_ID, "Control")),
                row=row_meta,
                shaping_cfg=shaping_cfg,
            )
...
def train_columns_member(
    ...
    transfer_weights(reactor_model, columns_model, reactor_features, columns_features)
...
def ensemble_predictions(
    ...
    return mean_pred, lower, upper, members
...
def transfer_weights(
    ...
    with torch.no_grad():
        ...
        # Copy hidden layers if same shape
        ...
        # Copy output if same shape
        if reactor_model.out.weight.shape == columns_model.out.weight.shape:
...
def _compute_loocv_group_statistics(
    ...
    for group_name, mask in group_masks:
        ...
        y_true = pd.to_numeric(sub.get(y_true_col, np.nan), errors="coerce").to_numpy(dtype=float)
        y_pred = pd.to_numeric(sub.get(pred_col, np.nan), errors="coerce").to_numpy(dtype=float)
        y_low = pd.to_numeric(sub.get(low_col, np.nan), errors="coerce").to_numpy(dtype=float)
        y_high = pd.to_numeric(sub.get(high_col, np.nan), errors="coerce").to_numpy(dtype=float)
...
def main():
    ...
    print("Columns are used for validation only; inference does not fit on column targets.")
    ...
    for sample_id in sample_ids:
        sample_df = df_columns_params[df_columns_params["project_sample_id"] == sample_id].copy()
        if sample_df.empty:
            continue
        sample_df = _attach_uncertainty_scores(
            sample_df=sample_df,
            shaping_cfg=shaping_cfg,
            unc_cfg=uncertainty_cfg,
        )

        pred_params_by_idx = _infer_sample_params_from_reactor_knowledge(
            sample_df=sample_df,
            reactor_status_medians=reactor_status_medians,
            curve_cfg=curve_cfg,
            coupling_cfg=coupling_cfg,
        ...
        member_param_preds = _generate_member_param_predictions_from_base(
            sample_df=sample_df,
            base_pred_by_idx=pred_params_by_idx,
            ensemble_seeds=ensemble_seeds,
            curve_cfg=curve_cfg,
            unc_cfg=uncertainty_cfg,
            shaping_cfg=shaping_cfg,
        ...
        for row_idx in sample_df.index:
            row_meta = sample_df.loc[row_idx]
            param_members = member_params_by_idx[row_idx]
            ...
            if not curve_df.empty:
                curve_df["fold_sample_id"] = sample_id
    ...
    for true_col, pred_col, label in [
        ...
        if valid.sum() == 0:
            continue
        rmse = float(np.sqrt(mean_squared_error(validation_param_df.loc[valid, true_col], validation_param_df.loc[valid, pred_col])))
    ...
    # -------- Inference-only outputs (no column-supervised fit) --------
    full_curve_df = validation_curve_df.drop(columns=["fold_sample_id"], errors="ignore").copy()
    full_param_df = validation_param_df.drop(columns=["fold_sample_id"], errors="ignore").copy()
    ...
    final_ensemble_plot_dir = os.path.join(PLOTS_ROOT, "columns_curve_final_ensemble")
    os.makedirs(final_ensemble_plot_dir, exist_ok=True)
    ...
    print("Columns full-data inference metrics:", full_metrics)

    final_overall_plot_path = os.path.join(final_ensemble_plot_dir, "final_ensemble_overall_statistics.png")
    plot_loocv_overall_statistics(
        full_curve_df,
        plot_path=final_overall_plot_path,
        plot_title="Final Ensemble Overall Statistics",
        status_col=CATALYZED_COLUMNS_ID,
        y_true_col="y_true",
        pred_col="y_pred_mean",
        low_col="y_pred_p05",
    ...
    print(f"Final ensemble overall statistics plot saved: {final_overall_plot_path}")

    for sample_id, sample_df in full_curve_df.groupby("project_sample_id"):
        final_plot_path = os.path.join(final_ensemble_plot_dir, f"{sample_id}.png")
        plot_sample_predictions(
            sample_df,
            str(sample_id),
            final_plot_path,
            time_col=TIME_COL_COLUMNS,
            target_col="y_true",
            title_prefix="Final Ensemble Prediction",
            status_col=CATALYZED_COLUMNS_ID,
        )
    print(f"Final ensemble sample prediction plots saved in: {final_ensemble_plot_dir}")

    inference_knowledge = {
        "ore_similarity_features": ore_similarity_features,
        "feature_cols_for_transfer_context": feature_cols,
        "target_cols_for_validation_only": target_cols,
        ...
        "uses_column_targets_for_training": False,
    }
