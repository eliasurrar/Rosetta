...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_free_model_v2"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_free_model_v2")
...
INTERACTION_GROUP_CANDIDATES = {
    "all_static": list(STATIC_PREDICTOR_COLUMNS),
    "chemistry": list(CHEMISTRY_INTERACTION_COLUMNS),
    "geometry": list(GEO_PRIORITY_COLUMNS),
    "chalcopyrite": [
        "residual_cpy_%",
...
def resolve_cv_parallelism(n_members: int) -> Dict[str, Any]:
    total_cores = max(1, int(os.cpu_count() or 1))
    accelerator_type = str(getattr(device, "type", "cpu")).lower()
    parallel_disabled_on_accelerator = accelerator_type in {"cuda"} #  "mps"

    requested_workers = int(CONFIG.get("cv_parallel_workers", total_cores))
    if requested_workers <= 0:
        requested_workers = total_cores
    if parallel_disabled_on_accelerator:
        workers = 1
    else:
    ...
    return {
        ...
        "parallel_disabled_on_accelerator": bool(parallel_disabled_on_accelerator),
...
def interpolate_recovery_profile(
    query_time_days: np.ndarray,
    anchor_time_days: np.ndarray,
    anchor_recovery: np.ndarray,
) -> np.ndarray:
    qt = np.asarray(query_time_days, dtype=float)
    at = np.asarray(anchor_time_days, dtype=float)
    ay = np.asarray(anchor_recovery, dtype=float)
    valid = np.isfinite(qt)
    if valid.sum() == 0:
        return np.asarray([], dtype=float)
    if at.size == 0 or ay.size == 0:
        return np.zeros(valid.sum(), dtype=float)
    if at.size == 1:
        return np.full(valid.sum(), float(np.clip(ay[0], 0.0, 100.0)), dtype=float)
    if at.size >= 3:
        interp = PchipInterpolator(at, ay, extrapolate=False)
        out = np.asarray(interp(qt[valid]), dtype=float)
        if np.any(~np.isfinite(out)):
            fallback = np.interp(qt[valid], at, ay)
            out = np.where(np.isfinite(out), out, fallback)
    else:
        out = np.interp(qt[valid], at, ay)
...
# Model
# ---------------------------
class AvailabilityAwareGroupEncoder(nn.Module):
    def __init__(
        self,
        input_dim: int,
        output_dim: int,
        ...
        mix_hidden = int(max(self.output_dim, hidden_dim))
        mix_rank = int(max(1, min(rank, self.input_dim)))
        self.linear = nn.Linear(self.input_dim, self.output_dim)
        self.mlp = nn.Sequential(
            nn.Linear(self.input_dim, mix_hidden),
            nn.ReLU(),
            nn.Dropout(float(max(0.0, dropout))),
            nn.Linear(mix_hidden, self.output_dim),
        )
        self.cross_u = nn.Linear(self.input_dim, mix_rank, bias=False)
        self.cross_v = nn.Linear(self.input_dim, mix_rank, bias=False)
        self.cross_out = nn.Linear(mix_rank, self.output_dim, bias=False)
        self.norm = nn.LayerNorm(self.output_dim)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.input_dim == 0 or x.shape[1] == 0:
            return torch.zeros((x.shape[0], self.output_dim), dtype=x.dtype, device=x.device)
        out = self.linear(x) + self.mlp(x) + self.cross_out(self.cross_u(x) * self.cross_v(x))
...
class PairCurveNet(nn.Module):
    def __init__(
        ...
        self.group_names = list(INTERACTION_GROUP_CANDIDATES.keys())
        self.group_indices = {
            name: self._resolve_feature_indices(candidate_names)
            for name, candidate_names in INTERACTION_GROUP_CANDIDATES.items()
        }
        self.group_embed_dim = int(max(8, min(24, hidden_dim // 4)))
        self.group_encoders = nn.ModuleDict(
            {
                name: AvailabilityAwareGroupEncoder(
                    input_dim=len(self.group_indices[name]),
                    output_dim=self.group_embed_dim,
                    hidden_dim=max(16, self.group_embed_dim * 2),
                    rank=min(8, max(1, len(self.group_indices[name]))),
                    dropout=0.5 * float(max(0.0, dropout)),
                )
                for name in self.group_names
            }
        )
        self.latent_context_dim = int(hidden_dim + len(self.group_names) * self.group_embed_dim)
        self.latent_context_norm = nn.LayerNorm(self.latent_context_dim)

        # Free monotonic curve cores with learnable interaction encoders over the
        # currently available predictor set and predictor groups.
        self.ctrl_core_head = nn.Linear(self.latent_context_dim, 10)
        self.cat_core_head = nn.Linear(self.latent_context_dim, 8)

        self.delay_head = nn.Linear(self.latent_context_dim, 1)
        self.temp_head = nn.Linear(self.latent_context_dim, 1)
        self.kappa_head = nn.Linear(self.latent_context_dim, 1)
        self.aging_head = nn.Linear(self.latent_context_dim, 1)

        self.primary_focus_head = nn.Linear(self.latent_context_dim, 1)
        self.chem_mix_head = nn.Linear(self.latent_context_dim, 1)
        self.fast_inventory_head = nn.Linear(self.latent_context_dim, 1)
        self.secondary_inventory_head = nn.Linear(self.latent_context_dim, 1)
        self.oxide_inventory_head = nn.Linear(self.latent_context_dim, 1)
        self.carb_buffer_head = nn.Linear(self.latent_context_dim, 1)
        self.buffer_decay_head = nn.Linear(self.latent_context_dim, 1)
        self.gangue_drag_head = nn.Linear(self.latent_context_dim, 1)
        self.ferric_synergy_head = nn.Linear(self.latent_context_dim, 1)
        self.surface_refresh_head = nn.Linear(self.latent_context_dim, 1)
        self.passivation_head = nn.Linear(self.latent_context_dim, 1)
        self.passivation_tau_head = nn.Linear(self.latent_context_dim, 1)
        self.passivation_temp_head = nn.Linear(self.latent_context_dim, 1)
        self.depassivation_head = nn.Linear(self.latent_context_dim, 1)
        self.catalyst_reactivity_head = nn.Linear(self.latent_context_dim, 1)
        self.sep_abs_head = nn.Linear(self.latent_context_dim, 4)
        self.sep_norm_head = nn.Linear(self.latent_context_dim, 4)
        self.ctrl_uplift_head = nn.Linear(self.latent_context_dim, 3)
    ...
    @staticmethod
    def _resolve_feature_indices(candidate_names: List[str]) -> List[int]:
        idx: List[int] = []
        seen: set = set()
        for name in candidate_names:
            if name in STATIC_PREDICTOR_COLUMNS and name not in seen:
                idx.append(STATIC_PREDICTOR_COLUMNS.index(name))
                seen.add(name)
        return idx

    @staticmethod
    def _feature_block(x_static: torch.Tensor, idxs: List[int]) -> torch.Tensor:
        if len(idxs) == 0:
            return x_static[:, :0]
        return x_static[:, idxs]
    ...
    def predict_params(
        ...
        group_embeddings = [
            self.group_encoders[name](self._feature_block(x_static, self.group_indices[name]))
            for name in self.group_names
        ]
        latent_context = self.latent_context_norm(torch.cat([h] + group_embeddings, dim=1))

        tau_days = self.tmax_days * torch.sigmoid(self.delay_head(latent_context))
        temp_days = self.min_transition_days + F.softplus(self.temp_head(latent_context))
        kappa = 1e-3 + F.softplus(self.kappa_head(latent_context))
        aging_strength = self.max_catalyst_aging_strength * torch.sigmoid(self.aging_head(latent_context))

        primary_focus = torch.sigmoid(self.primary_focus_head(latent_context))
        chem_interaction = 0.50 + torch.sigmoid(self.chem_mix_head(latent_context))
        fast_leach_inventory = torch.sigmoid(self.fast_inventory_head(latent_context))
        secondary_inventory = torch.sigmoid(self.secondary_inventory_head(latent_context))
        oxide_inventory = torch.sigmoid(self.oxide_inventory_head(latent_context))
        acid_buffer_strength = 0.85 * torch.sigmoid(self.carb_buffer_head(latent_context))
        acid_buffer_decay = 0.20 + 1.40 * torch.sigmoid(self.buffer_decay_head(latent_context))
        diffusion_drag_strength = 0.75 * torch.sigmoid(self.gangue_drag_head(latent_context))
        ferric_synergy = 0.50 + torch.sigmoid(self.ferric_synergy_head(latent_context))
        surface_refresh = 0.50 + torch.sigmoid(self.surface_refresh_head(latent_context))

        passivation_strength = 0.85 * torch.sigmoid(self.passivation_head(latent_context))
        passivation_tau = self.tmax_days * torch.sigmoid(self.passivation_tau_head(latent_context))
        passivation_temp = self.min_transition_days + F.softplus(self.passivation_temp_head(latent_context))
        depassivation_strength = 1.35 * torch.sigmoid(self.depassivation_head(latent_context))
        catalyst_reactivity = 0.50 + torch.sigmoid(self.catalyst_reactivity_head(latent_context))

        sep_abs_raw = self.sep_abs_head(latent_context)
        sep_abs_capacity = 2.0 + 35.0 * torch.sigmoid(sep_abs_raw[:, 0:1])
        sep_abs_scale = 0.10 + 3.00 * torch.sigmoid(sep_abs_raw[:, 1:2])
        sep_abs_mid = 0.05 + 0.85 * torch.sigmoid(sep_abs_raw[:, 2:3])
        sep_abs_temp = 0.05 + 0.45 * torch.sigmoid(sep_abs_raw[:, 3:4])

        sep_norm_raw = self.sep_norm_head(latent_context)
        sep_norm_scale = 0.05 + 1.20 * torch.sigmoid(sep_norm_raw[:, 0:1])
        sep_norm_progress_gain = 0.10 + 0.90 * torch.sigmoid(sep_norm_raw[:, 1:2])
        sep_norm_depassivation_gain = 0.10 + 0.90 * torch.sigmoid(sep_norm_raw[:, 2:3])
        sep_norm_cap = 0.20 + 3.50 * torch.sigmoid(sep_norm_raw[:, 3:4])

        ctrl_uplift_raw = self.ctrl_uplift_head(latent_context)
        ctrl_uplift_progress_mix = 0.05 + 0.90 * torch.sigmoid(ctrl_uplift_raw[:, 0:1])
        ctrl_uplift_increment_mix = 0.05 + 0.90 * torch.sigmoid(ctrl_uplift_raw[:, 1:2])
        ctrl_uplift_sharpness = 0.50 + 3.00 * torch.sigmoid(ctrl_uplift_raw[:, 2:3])

        ctrl_raw = self.ctrl_core_head(latent_context)
        ctrl_core = torch.cat(
            [
                0.05 + F.softplus(ctrl_raw[:, 0:1]),
                0.05 + F.softplus(ctrl_raw[:, 1:2]),
                0.05 + F.softplus(ctrl_raw[:, 2:3]),
                0.02 + F.softplus(ctrl_raw[:, 3:4]),
                0.20 + F.softplus(ctrl_raw[:, 4:5]),
                0.06 + F.softplus(ctrl_raw[:, 5:6]),
                0.02 + F.softplus(ctrl_raw[:, 6:7]),
                0.004 + F.softplus(ctrl_raw[:, 7:8]),
                0.01 + F.softplus(ctrl_raw[:, 8:9]),
                # Keep capacity floors low enough that low-recovery samples do not collapse
                # onto the same artificial ~25% control / ~28% catalyzed bands.
                2.0 + 93.0 * torch.sigmoid(ctrl_raw[:, 9:10]),
            ],
            dim=1,
        )

        cat_raw = self.cat_core_head(latent_context)
        cat_core = torch.cat(
            [
                0.05 + F.softplus(cat_raw[:, 0:1]),
                0.05 + F.softplus(cat_raw[:, 1:2]),
                0.02 + F.softplus(cat_raw[:, 2:3]),
                0.04 + F.softplus(cat_raw[:, 3:4]),
                0.006 + F.softplus(cat_raw[:, 4:5]),
                0.10 + 2.50 * torch.sigmoid(cat_raw[:, 5:6]),
                torch.sigmoid(cat_raw[:, 6:7]),
                0.25 + 55.0 * torch.sigmoid(cat_raw[:, 7:8]),
            ],
            dim=1,
        )
        ...
        latent_params = {
            ...
            "catalyst_reactivity": catalyst_reactivity,
            "sep_abs_capacity": sep_abs_capacity,
            "sep_abs_scale": sep_abs_scale,
            "sep_abs_mid": sep_abs_mid,
            "sep_abs_temp": sep_abs_temp,
            "sep_norm_scale": sep_norm_scale,
            "sep_norm_progress_gain": sep_norm_progress_gain,
            "sep_norm_depassivation_gain": sep_norm_depassivation_gain,
            "sep_norm_cap": sep_norm_cap,
            "ctrl_uplift_progress_mix": ctrl_uplift_progress_mix,
            "ctrl_uplift_increment_mix": ctrl_uplift_increment_mix,
            "ctrl_uplift_sharpness": ctrl_uplift_sharpness,
        }
        return ctrl_core, cat_core, tau_days, temp_days, kappa, aging_strength, latent_params
    ...
    def curves_given_params(
        ...
        if latent_params is None:
            ...
            latent_params = {
                ...
                "primary_focus": 0.5 * ones,
                "fast_leach_inventory": 0.5 * ones,
                "secondary_inventory": 0.5 * ones,
                ...
                "catalyst_reactivity": ones,
                "sep_abs_capacity": 8.0 * ones,
                "sep_abs_scale": ones,
                "sep_abs_mid": 0.20 * ones,
                "sep_abs_temp": 0.12 * ones,
                "sep_norm_scale": 0.50 * ones,
                "sep_norm_progress_gain": 0.50 * ones,
                "sep_norm_depassivation_gain": 0.50 * ones,
                "sep_norm_cap": 1.0 * ones,
                "ctrl_uplift_progress_mix": 0.50 * ones,
                "ctrl_uplift_increment_mix": 0.50 * ones,
                "ctrl_uplift_sharpness": 1.0 * ones,
            }
        ...
        effective_catalyst = torch.clamp(effective_catalyst - effective_catalyst[:, :1], min=0.0)
        catalyst_factor = 1.0 - torch.exp(-torch.clamp(kappa, min=1e-6) * effective_catalyst)
        catalyst_gate = delay_factor * catalyst_factor

        chem_interaction = torch.clamp(latent_params["chem_interaction"], min=0.25, max=1.75)
        primary_focus = torch.clamp(latent_params["primary_focus"], min=0.05, max=1.0)
        fast_leach_inventory = torch.clamp(latent_params["fast_leach_inventory"], min=0.0, max=1.0)
        secondary_inventory = torch.clamp(latent_params["secondary_inventory"], min=0.0, max=1.0)
        ...
        catalyst_reactivity = torch.clamp(latent_params["catalyst_reactivity"], min=0.35, max=1.75)
        sep_abs_capacity = torch.clamp(latent_params["sep_abs_capacity"], min=0.0, max=95.0)
        sep_abs_scale = torch.clamp(latent_params["sep_abs_scale"], min=0.0, max=5.0)
        sep_abs_mid = torch.clamp(latent_params["sep_abs_mid"], min=0.0, max=0.98)
        sep_abs_temp = torch.clamp(latent_params["sep_abs_temp"], min=0.02, max=0.75)
        sep_norm_scale = torch.clamp(latent_params["sep_norm_scale"], min=0.0, max=3.0)
        sep_norm_progress_gain = torch.clamp(latent_params["sep_norm_progress_gain"], min=0.0, max=2.0)
        sep_norm_depassivation_gain = torch.clamp(latent_params["sep_norm_depassivation_gain"], min=0.0, max=2.0)
        sep_norm_cap = torch.clamp(latent_params["sep_norm_cap"], min=0.0, max=5.0)
        ctrl_uplift_progress_mix = torch.clamp(latent_params["ctrl_uplift_progress_mix"], min=0.0, max=1.5)
        ctrl_uplift_increment_mix = torch.clamp(latent_params["ctrl_uplift_increment_mix"], min=0.0, max=1.5)
        ctrl_uplift_sharpness = torch.clamp(latent_params["ctrl_uplift_sharpness"], min=0.25, max=5.0)

        t_norm = torch.clamp(t / max(self.tmax_days, 1e-6), min=0.0)
        fast_release = torch.clamp(
            (0.55 * fast_leach_inventory + 0.25 * secondary_inventory + 0.20 * oxide_inventory)
            * torch.exp(-1.50 * t_norm),
            min=0.0,
            max=1.25,
        )
        secondary_release = torch.clamp(
            secondary_inventory * torch.exp(-0.85 * t_norm),
            min=0.0,
            max=1.25,
        )

        acid_buffer_remaining = torch.exp(-acid_buffer_decay * t_norm)
        ...
        diffusion_progress = torch.sqrt(torch.clamp(t_norm, min=0.0, max=1.0))
        diffusion_drag = torch.clamp(
            diffusion_drag_strength * diffusion_progress * (0.65 + 0.35 * primary_focus),
            min=0.0,
            max=0.95,
        )
        ...
        passivation = torch.clamp(
            ...
            * (0.75 + 0.25 * chem_interaction)
            * (1.0 + 0.15 * diffusion_drag)
            * (1.0 - 0.30 * fast_release),
            min=0.0,
            max=0.95,
        )

        transformation = torch.clamp(
            (0.45 * secondary_release + 0.35 * oxide_inventory + 0.20 * catalyst_gate)
            * chem_interaction
            * (0.50 + 0.50 * surface_refresh),
            min=0.0,
            max=1.25,
        )
        ...
        ctrl_easy_mass = p_ctrl[:, 0:1]
        ctrl_secondary_mass = p_ctrl[:, 1:2]
        ctrl_primary_mass = p_ctrl[:, 2:3]
        ctrl_tail_mass = p_ctrl[:, 3:4]
        ctrl_easy_rate = p_ctrl[:, 4:5]
        ctrl_secondary_rate = p_ctrl[:, 5:6]
        ctrl_primary_rate = p_ctrl[:, 6:7]
        ctrl_tail_rate = p_ctrl[:, 7:8]
        ctrl_bias = p_ctrl[:, 8:9]
        ctrl_capacity = torch.clamp(p_ctrl[:, 9:10], min=1.0, max=100.0)

        uplift_primary_mass = p_cat[:, 0:1]
        uplift_secondary_mass = p_cat[:, 1:2]
        uplift_tail_mass = p_cat[:, 2:3]
        uplift_primary_rate = p_cat[:, 3:4]
        uplift_tail_rate = p_cat[:, 4:5]
        uplift_scale = p_cat[:, 5:6]
        uplift_blend = torch.clamp(p_cat[:, 6:7], min=0.0, max=1.0)
        uplift_capacity = torch.clamp(p_cat[:, 7:8], min=0.0, max=95.0)

        chalcopyrite_window_ctrl = torch.clamp(primary_focus * (1.0 - passivation), min=0.0, max=1.5)
        chalcopyrite_window_cat = torch.clamp(primary_focus * (1.0 - 0.30 * passivation + depassivation), min=0.0, max=2.0)
        ore_accessibility = torch.clamp(
            (0.30 + 0.35 * fast_release + 0.20 * secondary_release + 0.15 * chalcopyrite_window_ctrl)
            * (1.0 - 0.55 * acid_buffer_penalty)
            * (1.0 - 0.65 * diffusion_drag),
            min=self.min_remaining_ore_factor,
            max=1.75,
        )

        ctrl_rate_track: List[torch.Tensor] = []
        cat_rate_track: List[torch.Tensor] = []
        uplift_track: List[torch.Tensor] = []
        ctrl_inc_track: List[torch.Tensor] = []
        extra_inc_track: List[torch.Tensor] = []
        cat_inc_track: List[torch.Tensor] = []
        control_steps: List[torch.Tensor] = []
        cat_steps: List[torch.Tensor] = []
        ore_access_steps: List[torch.Tensor] = []
        ctrl_progress_track: List[torch.Tensor] = []
        ctrl_sep_gate_track: List[torch.Tensor] = []
        sep_proxy_abs_track: List[torch.Tensor] = []
        sep_proxy_norm_track: List[torch.Tensor] = []
        realized_abs_sep_track: List[torch.Tensor] = []
        realized_norm_sep_track: List[torch.Tensor] = []

        prev_ctrl = torch.zeros((p_ctrl.shape[0], 1), dtype=p_ctrl.dtype, device=p_ctrl.device)
        prev_cat = torch.zeros((p_ctrl.shape[0], 1), dtype=p_ctrl.dtype, device=p_ctrl.device)

        for i in range(t.shape[1]):
            ti = t[:, i : i + 1]
            tni = t_norm[:, i : i + 1]
            dt = ti if i == 0 else torch.clamp(ti - t[:, i - 1 : i], min=0.0)
            dt_norm = torch.clamp(dt / max(self.tmax_days, 1e-6), min=0.0)

            easy_term = ctrl_easy_mass * torch.exp(-ctrl_easy_rate * tni)
            secondary_term = ctrl_secondary_mass * torch.exp(-ctrl_secondary_rate * tni)
            primary_term = ctrl_primary_mass * torch.exp(-ctrl_primary_rate * tni) * (0.25 + 0.75 * chalcopyrite_window_ctrl[:, i : i + 1])
            tail_term = ctrl_tail_mass * torch.exp(-ctrl_tail_rate * tni)

            ctrl_drive = (
                ctrl_bias
                + easy_term * (0.85 + 0.15 * oxide_inventory)
                + secondary_term * (0.75 + 0.25 * secondary_inventory)
                + primary_term
                + 0.30 * tail_term
            )
            ctrl_drive = ctrl_drive * ore_accessibility[:, i : i + 1]
            remaining_ctrl_mass = torch.clamp(ctrl_capacity - prev_ctrl, min=0.0)
            # Keep the free model monotone, but let the remaining inventory decay all the
            # way to zero so learned capacities behave like true asymptotes.
            ctrl_inc = torch.clamp(dt_norm * F.softplus(ctrl_drive) * remaining_ctrl_mass, min=0.0)
            next_ctrl = torch.clamp(prev_ctrl + ctrl_inc, min=0.0, max=100.0)
            next_ctrl = torch.minimum(next_ctrl, ctrl_capacity)
            ctrl_progress = torch.clamp(
                next_ctrl / torch.clamp(ctrl_capacity, min=1.0),
                min=0.0,
                max=1.25,
            )
            ctrl_inc_ratio = torch.clamp(
                ctrl_inc / torch.clamp(next_ctrl + 1.0, min=1.0),
                min=0.0,
                max=1.0,
            )
            control_separation_gate = torch.sigmoid(
                ctrl_uplift_sharpness
                * (
                    ctrl_uplift_progress_mix
                    * (ctrl_progress - sep_abs_mid)
                    / torch.clamp(sep_abs_temp, min=0.02)
                    + ctrl_uplift_increment_mix * ctrl_inc_ratio
                )
            )

            uplift_primary_term = uplift_primary_mass * torch.exp(-uplift_primary_rate * tni) * (0.25 + 0.75 * chalcopyrite_window_cat[:, i : i + 1])
            uplift_secondary_term = uplift_secondary_mass * torch.exp(-uplift_primary_rate * tni) * (0.30 + 0.70 * secondary_release[:, i : i + 1])
            uplift_tail_term = uplift_tail_mass * torch.exp(-uplift_tail_rate * tni) * (0.25 + 0.75 * chalcopyrite_window_cat[:, i : i + 1])

            uplift_gate = torch.clamp(
                uplift_scale
                * catalyst_gate[:, i : i + 1]
                * catalyst_reactivity
                * chem_interaction
                * ferric_synergy
                * (0.70 + 0.30 * surface_refresh),
                min=0.0,
                max=12.0,
            )
            uplift_mix = (
                uplift_blend * uplift_primary_term
                + (1.0 - uplift_blend) * uplift_secondary_term
                + 0.35 * uplift_tail_term
            )
            existing_uplift = torch.clamp(prev_cat - prev_ctrl, min=0.0)
            existing_norm_sep = existing_uplift / torch.clamp(prev_ctrl, min=1.0)
            sep_proxy_abs = torch.clamp(
                sep_abs_capacity
                * catalyst_gate[:, i : i + 1]
                * control_separation_gate
                * (0.35 + 0.65 * primary_focus)
                * (0.45 + 0.55 * chem_interaction)
                * (0.50 + 0.50 * ferric_synergy)
                * (1.0 - 0.30 * acid_buffer_penalty[:, i : i + 1])
                * (1.0 - 0.35 * diffusion_drag[:, i : i + 1]),
                min=0.0,
                max=95.0,
            )
            sep_proxy_norm = torch.clamp(
                sep_norm_cap
                * catalyst_gate[:, i : i + 1]
                * control_separation_gate
                * sep_norm_scale
                * (0.25 + sep_norm_progress_gain * ctrl_progress)
                * (0.25 + sep_norm_depassivation_gain * torch.clamp(depassivation[:, i : i + 1], min=0.0, max=1.5))
                * (0.40 + 0.60 * primary_focus),
                min=0.0,
                max=5.0,
            )
            sep_abs_gap = torch.relu(sep_proxy_abs - existing_uplift)
            sep_norm_gap = torch.relu(sep_proxy_norm - existing_norm_sep)
            separation_helper = torch.clamp(
                1.0
                + sep_abs_scale * sep_abs_gap / torch.clamp(sep_abs_capacity + 1.0, min=1.0)
                + sep_norm_scale * sep_norm_gap,
                min=1.0,
                max=8.0,
            )
            uplift_multiplier = (
                uplift_gate
                * (1.0 - 0.35 * acid_buffer_penalty[:, i : i + 1])
                * (1.0 - 0.45 * diffusion_drag[:, i : i + 1])
                * (1.0 + 0.20 * depassivation[:, i : i + 1])
                * (1.0 + 0.15 * transformation[:, i : i + 1])
            )
            uplift_multiplier = uplift_multiplier * separation_helper
            uplift_drive = uplift_multiplier * uplift_mix
            remaining_uplift_mass = torch.clamp(uplift_capacity - existing_uplift, min=0.0)
            extra_inc = torch.clamp(dt_norm * F.softplus(uplift_drive) * remaining_uplift_mass, min=0.0)

            next_uplift = torch.clamp(existing_uplift + extra_inc, min=0.0)
            next_uplift = torch.minimum(next_uplift, uplift_capacity)
            next_cat = torch.clamp(next_ctrl + next_uplift, min=0.0, max=100.0)
            next_cat = torch.maximum(next_cat, next_ctrl)
            realized_abs_sep = torch.clamp(next_cat - next_ctrl, min=0.0)
            realized_norm_sep = realized_abs_sep / torch.clamp(next_ctrl, min=1.0)

            control_steps.append(next_ctrl)
            cat_steps.append(next_cat)
            ctrl_rate_track.append(torch.clamp(ctrl_drive, min=0.0))
            cat_rate_track.append(torch.clamp(ctrl_drive + uplift_drive, min=0.0))
            uplift_track.append(torch.clamp(uplift_drive, min=0.0))
            ctrl_inc_track.append(ctrl_inc)
            extra_inc_track.append(extra_inc)
            cat_inc_track.append(torch.clamp(ctrl_inc + extra_inc, min=0.0))
            ore_access_steps.append(ore_accessibility[:, i : i + 1])
            ctrl_progress_track.append(ctrl_progress)
            ctrl_sep_gate_track.append(control_separation_gate)
            sep_proxy_abs_track.append(sep_proxy_abs)
            sep_proxy_norm_track.append(sep_proxy_norm)
            realized_abs_sep_track.append(realized_abs_sep)
            realized_norm_sep_track.append(realized_norm_sep)

            prev_ctrl = next_ctrl
            prev_cat = next_cat

        y_ctrl = torch.cat(control_steps, dim=1)
        y_cat = torch.cat(cat_steps, dim=1)
        ctrl_rate_multiplier = torch.cat(ctrl_rate_track, dim=1)
        cat_rate_multiplier = torch.cat(cat_rate_track, dim=1)
        uplift_factor = torch.cat(uplift_track, dim=1)
        control_increment = torch.cat(ctrl_inc_track, dim=1)
        uplift_increment = torch.cat(extra_inc_track, dim=1)
        catalyzed_increment = torch.cat(cat_inc_track, dim=1)
        ore_accessibility_curve = torch.cat(ore_access_steps, dim=1)
        control_progress_curve = torch.cat(ctrl_progress_track, dim=1)
        control_separation_gate_curve = torch.cat(ctrl_sep_gate_track, dim=1)
        separation_proxy_abs = torch.cat(sep_proxy_abs_track, dim=1)
        separation_proxy_norm = torch.cat(sep_proxy_norm_track, dim=1)
        realized_abs_separation = torch.cat(realized_abs_sep_track, dim=1)
        realized_norm_separation = torch.cat(realized_norm_sep_track, dim=1)

        if not return_states:
            return y_ctrl.squeeze(0), y_cat.squeeze(0)

        states = {
            "ore_accessibility": ore_accessibility_curve.squeeze(0),
            "fast_release": fast_release.squeeze(0),
            "secondary_release": secondary_release.squeeze(0),
            ...
            "uplift_factor": uplift_factor.squeeze(0),
            "control_increment": control_increment.squeeze(0),
            "uplift_increment": uplift_increment.squeeze(0),
            "catalyzed_increment": catalyzed_increment.squeeze(0),
            "control_progress": control_progress_curve.squeeze(0),
            "control_separation_gate": control_separation_gate_curve.squeeze(0),
            "separation_proxy_abs": separation_proxy_abs.squeeze(0),
            "separation_proxy_norm": separation_proxy_norm.squeeze(0),
            "realized_abs_separation": realized_abs_separation.squeeze(0),
            "realized_norm_separation": realized_norm_separation.squeeze(0),
            "chalcopyrite_window_ctrl": chalcopyrite_window_ctrl.squeeze(0),
            "chalcopyrite_window_cat": chalcopyrite_window_cat.squeeze(0),
...
def predict_new_sample_member(
    ...
    out = {
        ...
        "primary_focus": float(latent["primary_focus"].squeeze().detach().cpu().item()),
        "fast_leach_inventory": float(latent["fast_leach_inventory"].squeeze().detach().cpu().item()),
        "secondary_inventory": float(latent["secondary_inventory"].squeeze().detach().cpu().item()),
        ...
        "catalyst_reactivity": float(latent["catalyst_reactivity"].squeeze().detach().cpu().item()),
        "sep_abs_capacity": float(latent["sep_abs_capacity"].squeeze().detach().cpu().item()),
        "sep_abs_scale": float(latent["sep_abs_scale"].squeeze().detach().cpu().item()),
        "sep_norm_scale": float(latent["sep_norm_scale"].squeeze().detach().cpu().item()),
        "sep_norm_cap": float(latent["sep_norm_cap"].squeeze().detach().cpu().item()),
        "ctrl_uplift_progress_mix": float(latent["ctrl_uplift_progress_mix"].squeeze().detach().cpu().item()),
        "ctrl_uplift_increment_mix": float(latent["ctrl_uplift_increment_mix"].squeeze().detach().cpu().item()),
        "ctrl_uplift_sharpness": float(latent["ctrl_uplift_sharpness"].squeeze().detach().cpu().item()),
    }
...
def predict_new_sample_ensemble(
    ...
    ctrl_p10_raw = np.percentile(ctrl_stack, pi_low, axis=0)
    ctrl_p90_raw = np.percentile(ctrl_stack, pi_high, axis=0)
    cat_p10_raw = np.percentile(cat_stack, pi_low, axis=0)
    cat_p90_raw = np.percentile(cat_stack, pi_high, axis=0)
    ...
    record = {
        ...
        "control_pred_p10": ctrl_p10_raw,
        "control_pred_p90": ctrl_p90_raw,
        "catalyzed_pred_mean": np.mean(cat_stack, axis=0),
        "catalyzed_pred_p10": cat_p10_raw,
        "catalyzed_pred_p90": cat_p90_raw,
    ...
    record["control_pred_p10"], record["control_pred_p90"] = smooth_predictive_interval_bounds(
        time_days=record["time_days"],
        mean_curve=record["control_pred_mean"],
        low_curve=record["control_pred_p10"],
        high_curve=record["control_pred_p90"],
        smoothing_days=interval_smoothing_days,
    )
    record["catalyzed_pred_p10"], record["catalyzed_pred_p90"] = smooth_predictive_interval_bounds(
        time_days=record["time_days"],
        mean_curve=record["catalyzed_pred_mean"],
        low_curve=record["catalyzed_pred_p10"],
        high_curve=record["catalyzed_pred_p90"],
        smoothing_days=interval_smoothing_days,
    )
    if control_time_days is not None:
        ...
        record["control_pred_plot_p10"], record["control_pred_plot_p90"] = smooth_predictive_interval_bounds(
            time_days=record["control_plot_time_days"],
            mean_curve=record["control_pred_plot_mean"],
            low_curve=record["control_pred_plot_p10"],
            high_curve=record["control_pred_plot_p90"],
            smoothing_days=interval_smoothing_days,
        )
        record["catalyzed_plot_time_days"] = np.asarray(time_days, dtype=float)
    ctrl_width = np.asarray(ctrl_p90_raw) - np.asarray(ctrl_p10_raw)
    cat_width = np.asarray(cat_p90_raw) - np.asarray(cat_p10_raw)
    uncertainty = {
...
def plot_inference_ensemble_curve(record: Dict[str, Any], risk: Dict[str, Any], plot_path: str, title: str) -> None:
    ...
    plt.plot(ct, cmean, color="#1f77b4", lw=2, label="Control Pred Mean")
    plt.fill_between(ct, clow, chigh, color="#1f77b4", alpha=0.20, label="Control P10-P90")
    plt.plot(kt, kmean, color="#ff7f0e", lw=2, label="Catalyzed Pred Mean")
    plt.fill_between(kt, klow, khigh, color="#ff7f0e", alpha=0.20, label="Catalyzed P10-P90")
...
def pair_training_loss(
    ...
    ctrl_y_on_cat_true = torch.tensor(
        interpolate_recovery_profile(pair.catalyzed.time, pair.control.time, pair.control.recovery),
        dtype=torch.float32,
        device=device,
    )
    ...
    pred_ctrl_cat_time, pred_cat, cat_states = model.curves_given_params(
        p_ctrl,
        p_cat,
        cat_t,
        cat_c,
    ...
    anchor_pen = torch.abs(pred_ctrl_cat_time[:1] - pred_cat[:1]).mean()
    sep_abs_pred = torch.clamp(pred_cat - pred_ctrl_cat_time, min=0.0)
    sep_abs_true = torch.clamp(cat_y - ctrl_y_on_cat_true, min=0.0)
    sep_norm_pred = sep_abs_pred / torch.clamp(pred_ctrl_cat_time, min=1.0)
    sep_norm_true = sep_abs_true / torch.clamp(ctrl_y_on_cat_true, min=1.0)
    sep_abs_loss = F.smooth_l1_loss(sep_abs_pred, sep_abs_true)
    sep_norm_loss = F.smooth_l1_loss(torch.log1p(sep_norm_pred), torch.log1p(sep_norm_true))
    sep_proxy_loss = (
        F.smooth_l1_loss(cat_states["separation_proxy_abs"], sep_abs_true)
        + F.smooth_l1_loss(
            torch.log1p(cat_states["separation_proxy_norm"]),
            torch.log1p(sep_norm_true),
        )
    )
    ...
    latent_smooth_pen = (
        ...
        + _curvature_penalty(cat_states["uplift_factor"], cat_t)
        + _curvature_penalty(cat_states["control_separation_gate"], cat_t)
        + _curvature_penalty(cat_states["separation_proxy_abs"], cat_t)
        + _curvature_penalty(cat_states["separation_proxy_norm"], cat_t)
    )
    ...
    total = (
        ...
        + float(loss_weights.get("anchor", 0.18)) * anchor_pen
        + float(loss_weights.get("sep_abs", 0.20)) * sep_abs_loss
        + float(loss_weights.get("sep_norm", 0.10)) * sep_norm_loss
        + float(loss_weights.get("sep_proxy", 0.08)) * sep_proxy_loss
...
def predict_pair_record(model: PairCurveNet, pair: PairSample, cum_scale: float) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        ...
        plot_profile = extend_catalyst_profile_for_ensemble_plot(
            time_days=pair.catalyzed.time,
            catalyst_cum=pair.catalyzed.catalyst_cum,
        ...
        control_plot_time_days = build_plot_time_grid(
            observed_time_days=pair.control.time,
            start_day=0.0,
            target_day=float(plot_profile["target_day"]),
            step_days=float(CONFIG.get("ensemble_plot_step_days", 1.0)),
        )
        ...
        cat_plot_t = torch.tensor(plot_profile["time_days"], dtype=torch.float32, device=device)
        cat_plot_c = torch.tensor(
            np.asarray(plot_profile["catalyst_cum"], dtype=float) / cum_scale,
            dtype=torch.float32,
            device=device,
        )
    ...
    rec = {
        ...
        "catalyzed_secondary_release_last": float(cat_states["secondary_release"][-1].detach().cpu().item()),
        ...
        "catalyzed_uplift_factor_last": float(cat_states["uplift_factor"][-1].detach().cpu().item()),
        "catalyzed_sep_proxy_abs_last": float(cat_states["separation_proxy_abs"][-1].detach().cpu().item()),
        "catalyzed_sep_proxy_norm_last": float(cat_states["separation_proxy_norm"][-1].detach().cpu().item()),
        "catalyzed_abs_separation_last": float(cat_states["realized_abs_separation"][-1].detach().cpu().item()),
        "catalyzed_norm_separation_last": float(cat_states["realized_norm_separation"][-1].detach().cpu().item()),
        "control_progress_last": float(cat_states["control_progress"][-1].detach().cpu().item()),
        "control_separation_gate_last": float(cat_states["control_separation_gate"][-1].detach().cpu().item()),
        ...
        "control_chalcopyrite_window_last": float(ctrl_states["chalcopyrite_window_ctrl"][-1].detach().cpu().item()),
        "catalyzed_chalcopyrite_window_last": float(cat_states["chalcopyrite_window_cat"][-1].detach().cpu().item()),
        "catalyzed_ore_accessibility_last": float(cat_states["ore_accessibility"][-1].detach().cpu().item()),
        "control_easy_mass": float(p_ctrl.squeeze(0)[0].detach().cpu().item()),
        "control_secondary_mass": float(p_ctrl.squeeze(0)[1].detach().cpu().item()),
        "control_primary_mass": float(p_ctrl.squeeze(0)[2].detach().cpu().item()),
        "control_tail_mass": float(p_ctrl.squeeze(0)[3].detach().cpu().item()),
        "control_capacity": float(p_ctrl.squeeze(0)[9].detach().cpu().item()),
        "uplift_primary_mass": float(p_cat.squeeze(0)[0].detach().cpu().item()),
        "uplift_secondary_mass": float(p_cat.squeeze(0)[1].detach().cpu().item()),
        "uplift_tail_mass": float(p_cat.squeeze(0)[2].detach().cpu().item()),
        "uplift_scale": float(p_cat.squeeze(0)[5].detach().cpu().item()),
        "uplift_capacity": float(p_cat.squeeze(0)[7].detach().cpu().item()),
        "control_plot_time_days": np.asarray(control_plot_time_days, dtype=float),
        "control_pred_plot": pred_ctrl_plot.detach().cpu().numpy(),
        "catalyzed_plot_time_days": np.asarray(plot_profile["time_days"], dtype=float),
        "plot_catalyst_cum": np.asarray(plot_profile["catalyst_cum"], dtype=float),
...
def records_to_df(records: List[Dict[str, Any]], ensemble: bool = False) -> pd.DataFrame:
    ...
    for r in records:
        ...
        scalar_keys = [
            ...
            "aging_strength",
            "control_easy_mass",
            "control_secondary_mass",
            "control_primary_mass",
            "control_tail_mass",
            "control_capacity",
            "uplift_primary_mass",
            "uplift_secondary_mass",
            "uplift_tail_mass",
            "uplift_scale",
            "uplift_capacity",
            "sep_abs_capacity",
            ...
            "sep_norm_cap",
            "ctrl_uplift_progress_mix",
            "ctrl_uplift_increment_mix",
            "ctrl_uplift_sharpness",
            "catalyzed_sep_proxy_abs_last",
            "catalyzed_sep_proxy_norm_last",
            "catalyzed_abs_separation_last",
            "catalyzed_norm_separation_last",
            "control_progress_last",
            "control_separation_gate_last",
...
def plot_single_record(record: Dict[str, Any], plot_path: str, title: str) -> None:
    plt.figure(figsize=(9, 5))
    plt.plot(record["control_t"], record["control_true"], "o", ms=3, alpha=0.6, color="#1f77b4", label="Control True")
    plt.plot(record["control_t"], record["control_pred"], "-", lw=2, color="#1f77b4", label="Control Pred")
    ...
    plt.plot(record["catalyzed_t"], record["catalyzed_pred"], "-", lw=2, color="#ff7f0e", label="Catalyzed Pred")
...
def plot_ensemble_record(record: Dict[str, Any], plot_path: str, title: str) -> None:
    ...
    plt.plot(np.asarray(record["control_t"], dtype=float), cy, "o", ms=3, alpha=0.6, color="#1f77b4", label="Control True")
    plt.plot(ct, cp, "-", lw=2, color="#1f77b4", label="Control Ensemble Mean")
    plt.fill_between(ct, cp_lo, cp_hi, color="#1f77b4", alpha=0.18, label="Control P10-P90")

    plt.plot(np.asarray(record["catalyzed_t"], dtype=float), ky, "o", ms=3, alpha=0.6, color="#ff7f0e", label="Catalyzed True")
    plt.plot(kt, kp, "-", lw=2, color="#ff7f0e", label="Catalyzed Ensemble Mean")
    plt.fill_between(kt, kp_lo, kp_hi, color="#ff7f0e", alpha=0.18, label="Catalyzed P10-P90")
...
def main() -> None:
    ...
    if bool(cv_parallel.get("parallel_disabled_on_accelerator", False)):
        print(
            "[CV Ensemble] Process parallelism disabled because device is "
            f"{cv_parallel.get('accelerator_type', 'accelerator')}."
        )
    ...
    for input_path in input_excel_files:
        try:
            ...
            control_plot_time_days = build_plot_time_grid(
                observed_time_days=input_payload["time_days"],
                start_day=0.0,
                target_day=float(plot_profile["target_day"]),
                step_days=float(CONFIG.get("ensemble_plot_step_days", 1.0)),
            )
