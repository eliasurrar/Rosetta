...
CONFIG = {
    ...
    'column_tests_feature_weighting': {
        ...
        'weights': { # title and effect per parameter predicted in this order: a1, b1, a2, b2, a3, b3, a4, b4
            ...
            'grouped_acid_generating_sulfides': ['Acid Generating Sulphides (%)', 0, 0.64, 0, 0.48, 0, 0.36, 0, 0.48], # 0, 0.64, 0, 0.48, 0, 0.36, 0, 0.48
...
class AdaptiveTwoPhaseRecoveryModel(nn.Module):
    ...
    """
    def __init__(self, total_features, hidden_dim=128, dropout_rate=0.30, init_mode='kaiming', feature_weight_signs=None, feature_weight_magnitudes=None):
        super().__init__()
    ...
    def forward(self, x, catalyst, sample_ids=None):
        ...
        a1_min, a1_max = lim.get('a1', (10.0, 40.0)) # 1.5 to 68.0
        b1_min, b1_max = lim.get('b1', (1e-3, 2.1)) # 3e-4 to 2.1
        a2_min, a2_max = lim.get('a2', (5.0, 25.0)) # 2.2 to 79.0
        b2_min, b2_max = lim.get('b2', (1e-4, 2.1)) # 1e-4 to 2.1

        # Catalyst parameters
        a3_min, a3_max = lim.get('a3', (1.0, 15.0)) # 0.5 to 45.0
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for j in range(val_samples):
                ...
                val_losses.append(((weighted_loss_val.sum() / total_weighted_points) + ratio_penalty_val) * loss_scale)
