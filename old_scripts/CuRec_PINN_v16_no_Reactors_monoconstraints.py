...
CONFIG = {
    ...
    'column_tests_feature_weighting': {
        ...
        'weights': { # title and weight
            ...
            'material_size_p80_in': ['Material Size P80 (in)', -1.0],
            # 'reactors_PCA1': ['Reactors PCA1', 0.0],
            # 'reactors_PCA2': ['Reactors PCA2', 0.0],
            'grouped_copper_sulfides': ['Copper Sulphides (%)', 1.0],
            'grouped_secondary_copper': ['Secondary Copper (%)', 1.0],
            'grouped_acid_generating_sulfides': ['Acid Generating Sulphides (%)', -0.25],
            ...
            'grouped_fe_oxides': ['Fe Oxides (%)', -0.3],
            # 'grouped_accessory_misc': ['Accessory Misc (%)', 0.0],
            'grouped_carbonates': ['Carbonates (%)', -0.3],
...
def get_feature_weight_signs(config, feature_names):
    ...
    if not col_config.get('enabled', False) or not col_config.get('use_monotonic_constraints', False):
        return torch.zeros(len(feature_names))
    ...
    for feat_name in feature_names:
        if feat_name in special_dynamic:
            weight_signs.append(0.0)
        elif feat_name in raw_weights:
            weight_value = raw_weights[feat_name][1]
            if weight_value > 0:
                weight_signs.append(1.0)
            elif weight_value < 0:
                weight_signs.append(-1.0)
            else:
                weight_signs.append(0.0)
    ...
    return torch.tensor(weight_signs, dtype=torch.float32)
...
class AdaptiveTwoPhaseRecoveryModel(nn.Module):
    ...
    """
    def __init__(self, total_features, hidden_dim=128, dropout_rate=0.3, init_mode='kaiming', feature_weight_signs=None):
        super().__init__()
        ...
        if feature_weight_signs is not None:
            ...
            self.register_buffer('feature_weight_signs', torch.zeros(total_features))
    ...
    def apply_monotonic_constraints(self):
        ...
        with torch.no_grad():
            # Base network
            first_weight_base = self.base_network[0].weight
            for feat_idx, sign in enumerate(self.feature_weight_signs):
                if sign > 0:
                    first_weight_base[:, feat_idx].data = torch.abs(first_weight_base[:, feat_idx].data)
                elif sign < 0:
                    first_weight_base[:, feat_idx].data = -torch.abs(first_weight_base[:, feat_idx].data)
            
            # Catalyst network
            first_weight_cat = self.catalyst_network[0].weight
            for feat_idx, sign in enumerate(self.feature_weight_signs):
                if sign > 0:
                    first_weight_cat[:, feat_idx].data = torch.abs(first_weight_cat[:, feat_idx].data)
                elif sign < 0:
                    first_weight_cat[:, feat_idx].data = -torch.abs(first_weight_cat[:, feat_idx].data)
...
def compute_sample_weights(time_steps, device):
    ...
    full_weight_curve = np.linspace(1.0, 2.0, num_max_days_standard)
...
if __name__ == "__main__":
    ...
    # Model wrapper to handle forward call for SHAP (ignores catalyst/sample_ids, as they're not used for params)
    class ModelWrapper(nn.Module):
        ...
        def forward(self, x):
            catalyst_dummy = torch.zeros(x.shape[0], 1, device=x.device)  # Dummy, since unused
            return self.model(x, catalyst_dummy)  # sample_ids=None by default
        
    # Select background and explanation data
    X_cpu = X_tensor.cpu().numpy()  # Move to CPU for sampling
    background_indices = random.sample(range(X_cpu.shape[0]), min(CONFIG['shap_background_size'], X_cpu.shape[0]))
    X_background = X_tensor[background_indices]

    explain_indices = random.sample(range(X_cpu.shape[0]), min(CONFIG['shap_sample_size'], X_cpu.shape[0]))
    X_explain = X_tensor[explain_indices]

    # Compute SHAP for each model in ensemble
    all_shap_values = []  # List of [num_models, num_outputs=8, num_samples, num_features]
    for idx, model in enumerate(scaling_models.models):
        wrapped_model = ModelWrapper(model).to(device)
        explainer = shap.DeepExplainer(wrapped_model, X_background)
        shap_values_model = explainer.shap_values(X_explain, check_additivity=False)  # List of 8 arrays: [num_samples, num_features] per output
        # Convert to list if it's a 3D array (samples, features, outputs)
        ...
        all_shap_values.append(shap_values_model)

    # Weighted average SHAP across models
    num_outputs = 8  # a1 to b4
    weighted_shap = []
    for output_dim in range(num_outputs):
        output_shap = sum(scaling_models.weights[m] * all_shap_values[m][output_dim] for m in range(len(scaling_models.models)))
        weighted_shap.append(output_shap)  # List of 8 arrays: [num_samples, num_features]
    ...
    # Optional: Beeswarm plots for distribution (one per parameter)
    for output_dim, param in enumerate(param_names):
        shap.summary_plot(weighted_shap[output_dim], features=X_explain.cpu().numpy(), feature_names=num_cols, show=False)
        plt.title(f'SHAP Beeswarm for {param}')
        plt.tight_layout()
        plt.savefig(os.path.join(plots_dir, f'shap_beeswarm_{param}.png'), dpi=300)
        plt.close()

    print("SHAP analysis complete. Plots and CSV saved.")
    ...
    # Generate a params summary importance plots per predictor variable
    a_cols = [col for col in importance_df.columns if col.startswith('a')]
    importance_df_a = importance_df[a_cols]
    ...
    plt.title(f'SHAP overall % importance for a-params')
    plt.tight_layout()
    plt.ylabel('Feature')
    plt.xlabel('% importance')
    plt.savefig(os.path.join(plots_dir, f'percentual_feature_importance_a-params.png'), dpi=300)
    plt.show()
    plt.close()


    # Generate b params summary importance plots per predictor variable
    b_cols = [col for col in importance_df.columns if col.startswith('b')]
    importance_df_b = importance_df[b_cols]
    ...
    plt.title(f'SHAP overall % importance for b-params')
    plt.tight_layout()
    plt.ylabel('Feature')
    plt.xlabel('% importance')
    plt.savefig(os.path.join(plots_dir, f'percentual_feature_importance_b-params.png'), dpi=300)
    plt.show()
    plt.close()
