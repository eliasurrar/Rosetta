...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_ExpEq_columns_only_v6"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v6")
...
CONFIG = {
    ...
    "epochs": 400,
    "patience": 100,
    "learning_rate": 2.5e-3, #2.5e-3,
    "weight_decay": 2e-5,
    "grad_clip_norm": 5.0,
    "hidden_dim": 96,
    "dropout": 0.08,
    "min_transition_days": 5.0,
    "max_cat_slope_per_day": 0.50,
    "max_catalyst_aging_strength": 5.0,
    "late_tau_impact_decay_strength": 1.15,
    "min_remaining_ore_factor": 0.05,
    "flat_input_transition_sensitivity": 0.0, # initially 10.0
    "flat_input_uplift_response_days": 30.0,
    "flat_input_response_ramp_days": 30.0,
    "flat_input_late_uplift_response_boost": 2.5,
    "practical_stop_min_weekly_catalyst_kg_t": 1.0e-4,
    "practical_stop_min_weekly_lixiviant_m3_t": 5.0e-3,
    "loss_weights": {
        "gap": 1.0,
        "monotonic": 0.02,
        "param": 0.08,
        ...
        "flat_input_smooth": 0.0, # 0.18 (initially)
        "flat_input_accel": 0.0, # 0.16 (initially)
        "cap_violation": 0.25,
    },
    "plot_dpi": 300,
    "ensemble_pi_low": 10,
    ...
    "fit_max_asymptote": 80.0,
    # --- Mineralogy-based leach cap percentages ---
    # Fraction of each copper species considered leachable (0-1)
    "leach_pct_oxides": 1.00,
...
def sanitize_curve_params(params: np.ndarray) -> np.ndarray:
    ...
    total = p[0] + p[2]
    if total > max_asym and total > 0:
        scale = max_asym / total
        p[0] *= scale
        p[2] *= scale
    return p
...
def fit_biexponential_params(t_days: np.ndarray, recovery: np.ndarray) -> np.ndarray:
    ...
    max_asym = float(CONFIG.get("fit_max_asymptote", 80.0))
    plateau_day = float(CONFIG.get("fit_plateau_target_day",
                                   CONFIG.get("ensemble_plot_target_day", 2500.0) + 1000.0))
    ...
    upper = [max_asym, 1.0, max_asym, 1.0]

    # ---- soft-plateau penalty ------------------------------------------------
    plateau_penalty_weight = 0.15
    ...
    def _penalised_wmse(params):
        ...
        return wmse + plateau_penalty_weight * plateau_penalty
    ...
    try:
        ...
        popt = sanitize_curve_params(np.asarray(result.x, dtype=float))
        pred = double_exp_curve_np(t, popt[0], popt[1], popt[2], popt[3])
        r2 = float(r2_score(y, pred))
        if r2 > best_r2:
            best_r2 = r2
...
def plot_fitted_curve_per_sample(
    ...
    Title is red if criteria not met (R² <= 0.8 or RMSE >= 5.0).
    """
    ...
    for pair in pairs:
        ...
        for curve_data, status_label, color in curves_info:
            ...
            if not (np.isfinite(r2) and r2 > 0.8 and np.isfinite(rmse) and rmse < 5.0):
                criteria_met = False
...
def augment_pair_with_virtual_data(
    ...
    Each curve is extended only when its own fit meets R² > 0.8 and RMSE < 5.0.
    """
    ...
    ctrl_meets_criteria = np.isfinite(ctrl_r2) and ctrl_r2 > 0.8 and np.isfinite(ctrl_rmse) and ctrl_rmse < 5.0
    cat_meets_criteria = np.isfinite(cat_r2) and cat_r2 > 0.8 and np.isfinite(cat_rmse) and cat_rmse < 5.0
...
class PairSample:
    ...
    ctrl_cap: float = 80.0  # per-sample mineralogy-based cap for control curve
    cat_cap: float = 80.0   # per-sample mineralogy-based cap for catalyzed curve
...
class PairCurveNet(nn.Module):
    ...
    def predict_params(
        ...
        # Default to 1.0 if columns are missing to avoid division by zero later
        batch_size = x_static.shape[0]
        ones = torch.ones((batch_size, 1), dtype=x_static.dtype, device=x_static.device)
        
        interaction_terms["cu_percent"] = x_static[:, cu_idx:cu_idx+1] if cu_idx is not None else ones
        interaction_terms["cu_primary"] = x_static[:, cu_prim_idx:cu_prim_idx+1] if cu_prim_idx is not None else ones * 0.5
        interaction_terms["cu_oxides"] = x_static[:, cu_oxide_idx:cu_oxide_idx+1] if cu_oxide_idx is not None else ones * 0.1
        interaction_terms["cu_secondary"] = x_static[:, cu_sec_idx:cu_sec_idx+1] if cu_sec_idx is not None else ones * 0.0
        ...
        # Pass raw copper species through for remaining-ore-factor calculation
        latent_params["cu_percent"] = interaction_terms.get("cu_percent")
        latent_params["cu_primary"] = interaction_terms.get("cu_primary")
        latent_params["cu_oxides"] = interaction_terms.get("cu_oxides")
        latent_params["cu_secondary"] = interaction_terms.get("cu_secondary")
    ...
    def curves_given_params(
        ...
        delay_factor = torch.sigmoid((t - effective_tau_days) / torch.clamp(temp_days, min=1e-3))
        ...
        # New chemistry-based calculation using leachable_real:
        ...
        if cu_percent is not None and cu_primary is not None and cu_oxides is not None:
            remaining_ore_factor = self._compute_remaining_ore_factor_chemistry_based(
                y_ctrl, cu_percent, cu_primary, cu_oxides,
                min_floor=self.min_remaining_ore_factor,
                copper_secondary_sulfides_equiv=cu_secondary,
                is_catalyzed=False,  # remaining ore factor is based on control extraction
            )
        else:
            # Fallback to old method if ore chemistry columns are missing
        ...
        """
        # Alternative (weighted sum, more permissive):
        separation_factor = torch.clamp(
            0.25 * delay_factor
            + 0.25 * catalyst_factor
            + 0.15 * remaining_ore_factor
            + 0.15 * late_tau_factor
            + 0.20 * latent_gap_factor,
            min=0.0,
            max=1.0,
        )
        """

        cat_gap_raw = separation_factor * delta_limit
        ...
        cat_gap_smoothed = self._causal_response_smooth_torch(cat_gap_raw, t_days, cat_gap_response_days)
        cat_gap_smoothed = torch.clamp(cat_gap_smoothed, min=0.0)

        y_cat = y_ctrl + cat_gap_smoothed

        # --- Per-sample mineralogy caps ---
        # Compute caps from copper species in latent_params
        _cu_pct = latent_params.get("cu_percent")
        _cu_pri = latent_params.get("cu_primary")
        _cu_ox  = latent_params.get("cu_oxides")
        _cu_sec = latent_params.get("cu_secondary")
        if _cu_pct is not None and _cu_pri is not None and _cu_ox is not None:
            pct_ox  = float(CONFIG.get("leach_pct_oxides", 1.00))
            pct_sec = float(CONFIG.get("leach_pct_secondary_sulfides", 0.70))
            pct_pri_ctrl = float(CONFIG.get("leach_pct_primary_sulfides_control", 0.30))
            pct_pri_cat  = float(CONFIG.get("leach_pct_primary_sulfides_catalyzed", 0.70))
            _cu_safe = torch.clamp(_cu_pct, min=1e-9)
            _ctrl_real = _cu_ox * pct_ox + _cu_pri * pct_pri_ctrl
            _cat_real  = _cu_ox * pct_ox + _cu_pri * pct_pri_cat
            if _cu_sec is not None:
                _ctrl_real = _ctrl_real + _cu_sec * pct_sec
                _cat_real  = _cat_real  + _cu_sec * pct_sec
            ctrl_cap_t = torch.clamp(_ctrl_real / _cu_safe * 100.0, min=1.0, max=100.0)  # (B,1)
            cat_cap_t  = torch.clamp(_cat_real  / _cu_safe * 100.0, min=1.0, max=100.0)  # (B,1)
        else:
            fallback = float(CONFIG.get("fit_max_asymptote", 80.0))
            ctrl_cap_t = torch.full_like(y_ctrl[:, :1], fallback)
            cat_cap_t  = torch.full_like(y_cat[:, :1], fallback)

        y_ctrl = torch.cummax(torch.minimum(torch.clamp(y_ctrl, min=0.0), ctrl_cap_t), dim=1).values
        y_cat  = torch.cummax(torch.minimum(torch.clamp(y_cat,  min=0.0), cat_cap_t),  dim=1).values
        ...
        states = {
            ...
            "catalyzed_gap_raw": cat_gap_raw.squeeze(0),
            "catalyzed_gap_smoothed": cat_gap_smoothed.squeeze(0),
...
def pair_training_loss(
    ...
    # Penalise predictions that exceed the per-sample mineralogy caps
    ctrl_cap_val = torch.tensor(pair.ctrl_cap, dtype=torch.float32, device=device)
    cat_cap_val  = torch.tensor(pair.cat_cap,  dtype=torch.float32, device=device)
    cap_violation_pen = (
        torch.relu(pred_ctrl - ctrl_cap_val).mean()
        + torch.relu(pred_cat  - cat_cap_val).mean()
    )
    ...
    total = (
        ...
        + float(loss_weights.get("cap_violation", 0.25)) * cap_violation_pen
    )
    return total
