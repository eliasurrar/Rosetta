...
def generate_two_phase_recovery_exp(time, catalyst, transition_time, params):
    """
    Exponential two-phase recovery (matches training):
      Control: a1*(1-exp(-b1*t)) + a2*(1-exp(-b2*t))
      Catalyzed add-on (after transition): catalyst_effect * [a3*(1-exp(-b3*(t-t_trans)))
    ...
    exp1 = torch.exp(-torch.abs(b1) * time).clamp(min=1e-8, max=1.0)
    exp2 = torch.exp(-torch.abs(b2) * time).clamp(min=1e-8, max=1.0)
    recovery_control = torch.abs(a1) * (1 - exp1) + torch.abs(a2) * (1 - exp2)
    recovery = recovery_control.clone()

    has_catalyst = torch.any(catalyst > 0, dim=1, keepdim=True)
    ...
    if params.shape[1] > 4 and torch.any(~torch.isnan(params[:, 4:])):
        ...
        exp3 = torch.exp(-torch.abs(b3) * t_shift * rate_mult3).clamp(min=1e-8, max=1.0)
        exp4 = torch.exp(-torch.abs(b4) * t_shift * rate_mult4).clamp(min=1e-8, max=1.0)
        additional = catalyst_effect * (torch.abs(a3) * (1 - exp3) + torch.abs(a4) * (1 - exp4))
...
def compute_default_pi_pct(run_name):
    """Return (default_pct, min_pct, max_pct) for the prediction interval slider."""
    artifacts = combined_checkpoint.get(run_name, {}) if isinstance(combined_checkpoint, dict) else {}
    meta = artifacts.get('predictive_interval_metadata', {})
    bounds = meta.get('prediction_interval_bounds', {})
    min_pct = float(bounds.get('min_pct', 50.0))
    max_pct = float(bounds.get('max_pct', 99.0))

    if 'nominal_coverage_pct' in meta and meta.get('nominal_coverage_pct') is not None:
        default_pct = float(meta.get('nominal_coverage_pct'))
    else:
        cfg = artifacts.get('config', {})
        z_nom = float(cfg.get('z_score', 1.645))
        scale = float(artifacts.get('uncertainty_scale', 1.0))
        default_pct = (2.0 * NormalDist().cdf(z_nom * scale) - 1.0) * 100.0

    default_pct = max(min_pct, min(max_pct, default_pct))
    return default_pct, min_pct, max_pct
...
default_pi_pct, default_pi_min, default_pi_max = compute_default_pi_pct(DEFAULT_RUN)
...
slider_configs = {
    ...
    'pi_level': {'label': 'Prediction Interval (%)', 'min': default_pi_min, 'max': default_pi_max, 'step': 1.0, 'value': default_pi_pct},
}
...
@app.callback(
    [
        ...
        Output('slider-pi_level', 'min'),
        Output('slider-pi_level', 'max'),
        Output('slider-pi_level', 'value', allow_duplicate=True),
        Output('input-pi_level', 'value', allow_duplicate=True),
    ],
...
def update_prediction(run_name, acid_soluble, residual_cpy, material_size_p80, copper_sulfides,
    ...
    pi_meta = artifacts.get('predictive_interval_metadata', {})
    pi_bounds = pi_meta.get('prediction_interval_bounds', {})
    pi_min = float(pi_bounds.get('min_pct', slider_configs['pi_level']['min']))
    pi_max = float(pi_bounds.get('max_pct', slider_configs['pi_level']['max']))
    default_pi = pi_meta.get('nominal_coverage_pct')
    if default_pi is None:
        z_default = float(run_config.get('z_score', 1.645)) * float(uncert_scale)
        default_pi = (2.0 * NormalDist().cdf(z_default) - 1.0) * 100.0
    if default_pi is None:
        default_pi = slider_configs['pi_level']['value']
    from dash import callback_context
    trigger_id = callback_context.triggered[0]['prop_id'].split('.')[0] if callback_context.triggered else None
    if pi_level is None or trigger_id == 'run-select':
        pi_level = default_pi
    pi_level = max(pi_min, min(pi_max, float(pi_level)))
    ...
    # User-selectable prediction interval mapped to z-score with post-hoc calibration
    try:
        z_base = NormalDist().inv_cdf(0.5 + (pi_level / 100.0) / 2.0)
    except Exception:
        z_base = 1.645
    z_score = z_base * float(uncert_scale)
    pi_label = f'{pi_level:.0f}% PI (±{np.round(z_score, 2)}σ)'
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate=f'{pi_level:.0f}% PI: [%{{customdata[0]:.1f}}%, %{{customdata[1]:.1f}}%]<extra></extra>',
        showlegend=False
    ))
    ...
    params_text = html.Div([
        ...
        dbc.Card([
            dbc.CardBody([
                html.Div([
                    html.Strong("Control Cu Recovery (exponential):", style={'color': 'royalblue'}),
                    html.Div([
                        "CuRec",
                        html.Sub("control"),
                        " = a",
                    ...
                    html.Strong("Catalyzed Cu Recovery (exp add-on):", style={'color': 'darkorange'}),
                    html.Div([
                        "CuRec",
                        html.Sub("catalyzed"),
                        " = CuRec",
    ...
    outputs.extend([pi_min, pi_max, pi_level, pi_level])
