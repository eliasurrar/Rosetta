...
CONFIG = {
    "seed": 2026,
    "cv_n_splits": 7,
    # 7 folds x 7 repeats = 49 ensemble members, which gives more stable tail percentiles.
    "cv_n_repeats": 7,
...
MODEL_LOGIC_VERSION = "v8_cat_target_transition_20260329"
...
class PairCurveNet(nn.Module):
    ...
    def curves_given_params(
        ...
        transition_duration_days = torch.clamp(
            ...
            min=max(self.min_transition_days * 3.0, float(self.flat_input_response_ramp_days) * 2.0),
            max=self.tmax_days,
        )
        ...
        transition_weight = self._smooth_sigmoid_transition_torch(
            t_days=t_days,
            start_days=transition_start_day,
            end_days=transition_end_day,
        )
        transition_weight = transition_weight * (t >= transition_start_day).to(p_ctrl.dtype)
