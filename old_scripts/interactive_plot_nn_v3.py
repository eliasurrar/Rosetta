...
MODEL_FOLDER = '/Users/administration/Library/CloudStorage/OneDrive-JettiResources/Rosetta/NN_PyTorch/'
MODEL_VARIANTS = {
    'leach_days': {
        'label': 'Leach Days',
        'title': 'Cu Recovery Prediction (Leach Days) - Interactive Dashboard',
        'model_path': os.path.join(MODEL_FOLDER, 'AdaptiveTwoPhaseModel_withoutReactors_leachdays.pt'),
        'slider_overrides': {
            'catalyst_start_day': {'max': 500.0, 'step': 10.0, 'value': 250.0},
            'max_time': {'min': 100.0, 'max': 2500.0, 'step': 100.0, 'value': 1000.0},
        },
    },
    'lixiviant': {
        'label': 'Lixiviant',
        'title': 'Cu Recovery Prediction (Lixiviant) - Interactive Dashboard',
        'model_path': os.path.join(MODEL_FOLDER, 'AdaptiveTwoPhaseModel_withoutReactors_lix.pt'),
        'slider_overrides': {
            'catalyst_start_day': {'max': 5.0, 'step': 0.5, 'value': 2.0},
            'max_time': {'min': 10.0, 'max': 35.0, 'step': 1.0, 'value': 25.0},
        },
    },
}
DEFAULT_MODEL_KEY = 'leach_days'
        ...
        for model in self.models:
            with torch.no_grad():
                ...
                recovery = generate_two_phase_recovery(
                    time_points, catalyst, transition_time, params, self.config
                )
...
def generate_two_phase_recovery_exp(time, catalyst, transition_time, params, config):
    """
...
def generate_two_phase_recovery(time, catalyst, transition_time, params, config):
    return generate_two_phase_recovery_exp(time, catalyst, transition_time, params, config)
...
def load_model_variant(model_key, variant_info):
    print(f"  • loading '{variant_info['label']}' from {variant_info['model_path']}")
    checkpoint = torch.load(variant_info['model_path'], map_location=device, weights_only=False)
    if 'models' not in checkpoint and isinstance(checkpoint, dict) and model_key in checkpoint:
    ...
    feature_metadata_local = {m['name']: m for m in checkpoint.get('feature_metadata', [])} if checkpoint.get('feature_metadata') else {}
    uncertainty_scale_local = float(checkpoint.get('uncertainty_scale', 1.0))
    ensemble_model.uncertainty_scale = uncertainty_scale_local
    ...
    print(f"    ✓ num_models={ensemble_model.get_ensemble_info()['num_models']} | features={len(num_cols)} | uncertainty_scale={ensemble_model.uncertainty_scale:.3f}")
    return {
        'config': config,
        'ensemble_model': ensemble_model,
        'scaler_X': scaler_X,
        'num_cols': num_cols,
        'feature_metadata': feature_metadata_local,
        'uncertainty_scale': uncertainty_scale_local,
    }


print("Loading model variants...")
MODEL_PAYLOADS = {key: load_model_variant(key, info) for key, info in MODEL_VARIANTS.items()}
print("✓ All model variants loaded.")

# Set defaults to keep global helpers aligned
ACTIVE_PAYLOAD = MODEL_PAYLOADS[DEFAULT_MODEL_KEY]
CONFIG = ACTIVE_PAYLOAD['config']
feature_metadata = ACTIVE_PAYLOAD['feature_metadata']
...
default_pi_min, default_pi_max, default_pi_pct = 50.0, 95.0, 90.0
...
BASE_SLIDER_CONFIGS = {
    ...
    'catalyst_start_day': {'label': 'Catalyst Start Day', 'min': 0, 'max': 500.0, 'step': 10.0, 'value': 250.0},
    'catalyst_dose': {'label': 'Catalyst Dose (kg/t/day)', 'min': 0.0, 'max': 0.012, 'step': 0.00005, 'value': 0.00105},
    'max_time': {'label': 'Max Time (days)', 'min': 100.0, 'max': 2500.0, 'step': 100.0, 'value': 1000.0},
    'pi_level': {'label': 'Prediction Interval (%)', 'min': default_pi_min, 'max': default_pi_max, 'step': 5.0, 'value': default_pi_pct},
}

def build_slider_configs(model_key):
    cfg = copy.deepcopy(BASE_SLIDER_CONFIGS)
    overrides = MODEL_VARIANTS.get(model_key, {}).get('slider_overrides', {})
    for slider_id, override in overrides.items():
        if slider_id in cfg:
            cfg[slider_id].update(override)
        else:
            cfg[slider_id] = override

    cyanide_bounds = (3.0, 55.0)
    cyanide_guess = 100 - cfg['acid_soluble']['value'] - cfg['residual_cpy']['value']
    cfg['cyanide_soluble'] = {'label': 'Cyanide Soluble %', 'min': 3.0, 'max': 55.0, 'step': 0.5, 'value': _clamp(cyanide_guess, cyanide_bounds)}

    sum_others = (cfg['copper_sulfides']['value'] + cfg['secondary_copper']['value'] + 
                  cfg['acid_gen_sulfides']['value'] + cfg['fe_oxides']['value'] + 
                  cfg['carbonates']['value'])
    cfg['gangue_silicates']['value'] = _clamp(100 - sum_others, (cfg['gangue_silicates']['min'], cfg['gangue_silicates']['max']))
    return cfg


SLIDER_CONFIGS_BY_MODEL = {key: build_slider_configs(key) for key in MODEL_VARIANTS.keys()}
slider_configs = copy.deepcopy(SLIDER_CONFIGS_BY_MODEL[DEFAULT_MODEL_KEY])
slider_keys = list(slider_configs.keys())
...
def build_controls_panel(active_slider_configs):
    return [
        dbc.Card([
            dbc.CardHeader(html.H5("Sample Features")),
            dbc.CardBody([
                create_slider_row('acid_soluble', active_slider_configs['acid_soluble']),
                create_slider_row('residual_cpy', active_slider_configs['residual_cpy']),
                create_slider_row('cyanide_soluble', active_slider_configs['cyanide_soluble'], disabled=True),
                create_slider_row('material_size_p80', active_slider_configs['material_size_p80']),
                create_slider_row('copper_sulfides', active_slider_configs['copper_sulfides']),
                create_slider_row('secondary_copper', active_slider_configs['secondary_copper']),
                create_slider_row('acid_gen_sulfides', active_slider_configs['acid_gen_sulfides']),
                create_slider_row('gangue_silicates', active_slider_configs['gangue_silicates'], disabled=True),
                create_slider_row('fe_oxides', active_slider_configs['fe_oxides']),
                create_slider_row('carbonates', active_slider_configs['carbonates']),
            ])
        ], className='mb-3'),
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                create_slider_row('catalyst_start_day', active_slider_configs['catalyst_start_day']),
                create_slider_row('catalyst_dose', active_slider_configs['catalyst_dose']),
                html.Div(
        ...
        dbc.Card([
            ...
            dbc.CardBody([
                create_slider_row('max_time', active_slider_configs['max_time']),
                create_slider_row('pi_level', active_slider_configs['pi_level']),
            ])
        ], className='mb-3'),
    ]
...
app.layout = dbc.Container([
    html.H1(id='page-title', className='text-center my-4'),

    dbc.Row([
        dbc.Col([
            dbc.Card([
                dbc.CardHeader(html.H5("Model Variant")),
                dbc.CardBody([
                    dcc.Dropdown(
                        id='model-selector',
                        options=[{'label': info['label'], 'value': key} for key, info in MODEL_VARIANTS.items()],
                        value=DEFAULT_MODEL_KEY,
                        clearable=False,
                    ),
                    html.Div(
                        "Leach Days covers the longer time horizon; Lixiviant is constrained to the short lix test window.",
                        style={'fontSize': '0.85em', 'color': '#555', 'marginTop': '8px'}
                    ),
                ])
            ], className='mb-3'),

            html.Div(id='controls-container', children=build_controls_panel(slider_configs)),
...
def handle_model_change(selected_model):
    """Swap the UI defaults and active CONFIG when the model variant changes."""
    global slider_configs, CONFIG, feature_metadata, ACTIVE_PAYLOAD
    slider_configs = copy.deepcopy(SLIDER_CONFIGS_BY_MODEL[selected_model])
    ACTIVE_PAYLOAD = MODEL_PAYLOADS[selected_model]
    CONFIG = ACTIVE_PAYLOAD['config']
    feature_metadata = ACTIVE_PAYLOAD['feature_metadata']
    return build_controls_panel(slider_configs), MODEL_VARIANTS[selected_model]['title']
...
for _sid in slider_configs.keys():
    ...
    def update_input_style(value, selected_model, _sid=_sid):
        cfg = SLIDER_CONFIGS_BY_MODEL[selected_model][_sid]
...
for slider_id in slider_configs.keys():
    ...
    def sync_slider_input(slider_val, input_val, selected_model, slider_id=slider_id):
        ...
        cfg = SLIDER_CONFIGS_BY_MODEL[selected_model][slider_id]

        if trigger_id == 'model-selector':
        ...
        if trigger_id == f'slider-{slider_id}':
            ...
            if input_val is not None:
                clamped_value = max(cfg['min'], min(cfg['max'], float(input_val)))
...
def update_cyanide(acid, residual, selected_model):
    cfg = SLIDER_CONFIGS_BY_MODEL[selected_model]['cyanide_soluble']
    bounds = (cfg['min'], cfg['max'])
...
def update_cyanide_input_style(acid, residual, selected_model):
    cfg = SLIDER_CONFIGS_BY_MODEL[selected_model]['cyanide_soluble']
...
def update_gangue(cu_sulf, sec_cu, acid_sulf, fe_ox, carb, selected_model):
    sum_others = cu_sulf + sec_cu + acid_sulf + fe_ox + carb
    cfg = SLIDER_CONFIGS_BY_MODEL[selected_model]['gangue_silicates']
    lo, hi = cfg['min'], cfg['max']
    raw = 100 - sum_others
...
def update_gangue_input_style(cu_sulf, sec_cu, acid_sulf, fe_ox, carb, selected_model):
    cfg = SLIDER_CONFIGS_BY_MODEL[selected_model]['gangue_silicates']
    lo, hi = cfg['min'], cfg['max']
...
@app.callback(
    ...
    [Input('model-selector', 'value')] + [Input(f'slider-{key}', 'value') for key in slider_keys]
)
def update_prediction(selected_model, acid_soluble, residual_cpy, material_size_p80, copper_sulfides,
    ...
    payload = MODEL_PAYLOADS[selected_model]
