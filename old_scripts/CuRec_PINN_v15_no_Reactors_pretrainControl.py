...
def find_best_config_for_fold(X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor,
                              ...
                              train_mask, test_sample_id, pretrain_model=None):
...
def train_pretrain_model(X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor, 
                         sample_ids, num_cols, config, device):
    """Train a single model on all data for pre-training (no LOOCV)."""
    print("Pre-training model on all data...")
    
    # Training parameters (use lower epochs for pre-training to save time)
    epochs = config.get('pytorch_epochs', 1000) // 2  # Halve epochs for pre-training
    ...
    optimizer = optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=config.get('pytorch_weight_decay', 1e-4))
    ...
    weights_train = compute_sample_weights(time_tensor, device)
    ...
    for epoch in range(epochs):
        ...
        for j in range(len(X_tensor)):
            optimizer.zero_grad()
            x_sample = X_tensor[j:j+1]
            catalyst_sample = catalyst_tensor[j].unsqueeze(0)
            time_sample = time_tensor[j].unsqueeze(0)
            tt_sample = tt_tensor[j]
            batch_sample_ids = [sample_ids[j]]
            
            params = model(x_sample, catalyst_sample, batch_sample_ids)
            predicted_recovery = generate_two_phase_recovery(
                time_sample, catalyst_sample, tt_sample, params
            )
            
            target_recovery = y_tensor[j].unsqueeze(0)
        ...
        # Simple validation: use last 20% of training data as val
        val_loss = 0.0
        val_start = int(0.8 * len(X_tensor))
        with torch.no_grad():
            for j in range(val_start, len(X_tensor)):
                # Similar to training loop, compute loss
                x_sample = X_tensor[j:j+1]
                catalyst_sample = catalyst_tensor[j].unsqueeze(0)
                time_sample = time_tensor[j].unsqueeze(0)
                tt_sample = tt_tensor[j]
                ...
                val_loss += weighted_loss_val.mean().item()
        
        avg_val_loss = val_loss / (len(X_tensor) - val_start) if (len(X_tensor) - val_start) > 0 else avg_loss
...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    fold_data, num_cols, config, device, save_dir, X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor, sample_ids, sample_col_ids, use_val_data_days, pretrain_model = fold_data_with_params
    ...
    train_indices = [j for j, mask in enumerate(train_mask) if mask]
    train_col_ids = [sample_col_ids[j] for j in train_indices]
        
    # Filter training data for control (pre-training)
    control_mask = [col == 'Control' for col in train_col_ids]
    control_indices = [train_indices[k] for k, is_control in enumerate(control_mask) if is_control]
    ...
    pretrain_model_local = None
    if len(control_indices) > 0:
        X_control = X_tensor[control_indices]
        y_control = [y_tensor[j] for j in control_indices]
        time_control = [time_tensor[j] for j in control_indices]
        catalyst_control = [catalyst_tensor[j] for j in control_indices]
        tt_control = tt_tensor[control_indices]  # Assuming tt_tensor is a tensor; adjust if list
        sample_ids_control = [sample_ids[j] for j in control_indices]
        
        pretrain_config = copy.deepcopy(config)
        pretrain_config['pytorch_epochs'] = epochs
        pretrain_config['generate_plots'] = False  # No plots during pre-training
        
        print(f"Pre-training on {len(control_indices)} control samples for fold {i+1}")
        pretrain_model_local, _ = train_pretrain_model(
            X_control, y_control, time_control, catalyst_control, tt_control,
            sample_ids_control, num_cols, pretrain_config, device
        )

    # Filter training data for catalyzed (fine-tuning)
    catalyzed_mask = [col == 'Catalyzed' for col in train_col_ids]
    catalyzed_indices = [train_indices[k] for k, is_cat in enumerate(catalyzed_mask) if is_cat]

    if len(catalyzed_indices) > 0:
        X_train = X_tensor[catalyzed_indices]
        y_train = [y_tensor[j] for j in catalyzed_indices]
        time_train = [time_tensor[j] for j in catalyzed_indices]
        catalyst_train = [catalyst_tensor[j] for j in catalyzed_indices]
        tt_train = tt_tensor[catalyzed_indices]
        train_sample_ids = [sample_ids[j] for j in catalyzed_indices]
    else:
        # Fallback if no catalyzed (unlikely)
        X_train = X_tensor[:0]
        y_train = []
        time_train = []
        catalyst_train = []
        tt_train = tt_tensor[:0]
        train_sample_ids = []
    ...
    # Load pre-trained base_network if available (from local pre-train or passed)
    pretrain_to_use = pretrain_model_local if pretrain_model_local is not None else pretrain_model
    if pretrain_to_use is not None:
        print(f"Initializing base_network with pre-trained weights for fold {i+1}")
        fold_model.base_network.load_state_dict(pretrain_to_use.base_network.state_dict())
        # Optional: Freeze base_network
        # for param in fold_model.base_network.parameters():
...
def process_fold(fold_data, X_tensor, y_tensor, time_tensor, catalyst_tensor, 
                 ...
                 use_val_data_days, pretrain_model=None):
    ...
    if not config.get('skip_hypersearch', False):
        best_config = find_best_config_for_fold(
            ...
            train_mask, test_sample_id, pretrain_model=pretrain_model
        )  # Note: To use pretrain in hypersearch, modify find_best_config_for_fold to accept/pass it.
    else:
        best_config = config
    ...
    fold_data_with_params = (
        ...
        X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor,
        sample_ids, sample_col_ids, use_val_data_days, pretrain_model  # Add pretrain_model
    )
...
def train_reactor_scaling_model_parallel(X_tensor, y_tensor, time_tensor, catalyst_tensor, 
                                        ...
                                        config, device, save_dir, use_val_data_days=0, pretrain_model=None):
...
if __name__ == "__main__":
    ...
    # Filter for Control
    control_mask = [col == 'Control' for col in sample_col_ids]
    X_control = X_tensor[control_mask]
    y_control = [y_tensor[i] for i, m in enumerate(control_mask) if m]
    time_control = [time_tensor[i] for i, m in enumerate(control_mask) if m]
    catalyst_control = [catalyst_tensor[i] for i, m in enumerate(control_mask) if m]
    tt_control = torch.tensor([tt_tensor[i] for i, m in enumerate(control_mask) if m], dtype=torch.float32).to(device)
    sample_ids_control = [sample_ids[i] for i, m in enumerate(control_mask) if m]
    sample_col_ids_control = [sample_col_ids[i] for i, m in enumerate(control_mask) if m]
    sample_ids_control = np.array(sample_ids_control)
    sample_col_ids_control = np.array(sample_col_ids_control)

    # Filter for Catalyzed
    catalyzed_mask = [col == 'Catalyzed' for col in sample_col_ids]
    X_catalyzed = X_tensor[catalyzed_mask]
    y_catalyzed = [y_tensor[i] for i, m in enumerate(catalyzed_mask) if m]
    time_catalyzed = [time_tensor[i] for i, m in enumerate(catalyzed_mask) if m]
    catalyst_catalyzed = [catalyst_tensor[i] for i, m in enumerate(catalyzed_mask) if m]
    tt_catalyzed = torch.tensor([tt_tensor[i] for i, m in enumerate(catalyzed_mask) if m], dtype=torch.float32).to(device)
    sample_ids_catalyzed = [sample_ids[i] for i, m in enumerate(catalyzed_mask) if m]
    sample_col_ids_catalyzed = [sample_col_ids[i] for i, m in enumerate(catalyzed_mask) if m]
    sample_ids_catalyzed = np.array(sample_ids_catalyzed)
    sample_col_ids_catalyzed = np.array(sample_col_ids_catalyzed)

    # Optional: Create a control-specific config (e.g., disable hypersearch or plots)
    control_config = copy.deepcopy(CONFIG)
    control_config['generate_plots'] = False  # No plots for pre-training
    control_config['skip_hypersearch'] = True  # Optional: Skip hypersearch for control

    print("\nStep 2.1: Pre-training on Control data")
    control_model, control_val_loss = train_pretrain_model(
        X_control, y_control, time_control, catalyst_control, tt_control,
        sample_ids_control, num_cols, control_config, device
    )

    print("\nStep 2.2: Fine-tuning on Catalyzed data with LOOCV")
    scaling_models, scaling_results = train_reactor_scaling_model_parallel(
        X_tensor=X_tensor,  # Full data
        y_tensor=y_tensor,
        time_tensor=time_tensor,
        catalyst_tensor=catalyst_tensor,
        tt_tensor=tt_tensor,
        ...
        pretrain_model=None  # No need for external pretrain; handled inside
    )
