...
CONFIG = {
    ...
    'catalyst_effect_power': 1.0,     # ce(t) = (cat/(cat+1))**power used in fit+train
    'catalyst_fit_weight_power': 1.5, # w = ce**power for weighted LS
    'catalyst_fit_min_weight': 0.02,  # ignore windows with very small ce
   # Global catalyst-effect calibration used ONLY in prefit (this function sets best K,p,lag)
   'ce_param_search': {
       'K_grid': [0.1, 0.3, 0.6, 1.0],   # saturation level (cat units)
       'p_grid': [0.8, 1.0, 1.2],        # exponent
       'lag_grid': [0.0, 0.2, 0.5]       # lag (same units as your time axis)
       }
}
...
#%% ---------------------------
# Step 1: Parameter Fitting Functions
# ---------------------------

def _two_exp_model(t, a1, b1, a2, b2):
    t = np.asarray(t, dtype=float)
    return a1 * (1.0 - np.exp(-b1 * t)) + a2 * (1.0 - np.exp(-b2 * t))

def fit_two_exp_params(t, y):
    """Fit a1,b1,a2,b2 to y(t) = a1(1-exp(-b1 t)) + a2(1-exp(-b2 t)). Return tuple or None."""
    t = np.asarray(t, dtype=float)
    y = np.asarray(y, dtype=float)
    if t.ndim != 1 or y.ndim != 1 or t.size < 4 or y.size < 4:
        return None
    ...
    if not np.isfinite(ymax) or ymax <= 0:
        return None
    tmax = max(float(np.nanmax(t)), 1.0)
    # guesses: one fast and one slow
    b1_0 = np.log(2) / max(0.2 * tmax, 1e-3)
    b2_0 = np.log(2) / max(0.8 * tmax, 1e-3)
    p0 = [0.6 * ymax, b1_0, 0.4 * ymax, b2_0]
    bounds = ([2.5, 1e-5, 1.5, 1e-5], [99.0, 9.0, 99.0, 9.0])
    try:
        popt, _ = curve_fit(_two_exp_model, t, y, p0=p0, bounds=bounds, maxfev=20000)
        return tuple(map(float, popt))
    except Exception:
        return None

def _two_exp_additive_with_ce(t, a3, b3, a4, b4, ce):
    """
    Additive catalyzed component with catalyst amplification:
    y_add(t) = ce(t) * [a3*(1 - exp(-b3*t)) + a4*(1 - exp(-b4*t))]
    """
    t = np.asarray(t, dtype=float)
    ce = np.asarray(ce, dtype=float)
    return ce * (a3 * (1.0 - np.exp(-b3 * t)) + a4 * (1.0 - np.exp(-b4 * t)))

def _one_exp_additive_with_ce(t, a, b, ce):
    t = np.asarray(t, dtype=float); ce = np.asarray(ce, dtype=float)
    return ce * (a * (1.0 - np.exp(-b * t)))

def fit_one_exp_params_additive_with_ce(t, y, ce, w=None):
    t = np.asarray(t, dtype=float); y = np.asarray(y, dtype=float); ce = np.asarray(ce, dtype=float)
    if t.size < 3 or np.max(ce) <= 1e-6 or np.max(y) <= 1e-6:
        return None
    tmax = max(float(np.nanmax(t)), 1.0)
    b0 = np.log(2)/max(0.5*tmax, 1e-3)
    a0 = float(np.nanmax(y) / max(np.nanmax(ce), 1e-6))
    p0 = [max(a0, 0.1), b0]
    bounds = ([2.5, 1e-5], [99.0, 9.0])
    sigma = None
    if w is not None:
        w = np.asarray(w, dtype=float); w = w/(np.max(w)+1e-8)
        sigma = 1.0/np.sqrt(np.maximum(w, 1e-6))
    try:
        func = lambda tt, a, b: _one_exp_additive_with_ce(tt, a, b, ce)
        popt, _ = curve_fit(func, t, y, p0=p0, bounds=bounds, sigma=sigma, absolute_sigma=False, maxfev=20000)
        return float(popt[0]), float(popt[1])
    except Exception:
        return None

def fit_two_exp_params_additive_with_ce(t, y, ce, w=None):
    """
    Fit a3,b3,a4,b4 to y(t) = ce(t) * [a3(1-exp(-b3 t)) + a4(1-exp(-b4 t))].
    Returns (a3,b3,a4,b4) or None.
    """
    t = np.asarray(t, dtype=float)
    y = np.asarray(y, dtype=float)
    ce = np.asarray(ce, dtype=float)
    if t.ndim != 1 or y.ndim != 1 or ce.ndim != 1:
        return None
    if t.size < 4 or y.size < 4 or ce.size < 4:
        return None
    if not (np.all(np.isfinite(t)) and np.all(np.isfinite(y)) and np.all(np.isfinite(ce))):
        return None
    if np.max(ce) <= 1e-5 or np.max(y) <= 1e-5:
        return None

    ymax = float(np.nanmax(y))
    tmax = max(float(np.nanmax(t)), 1.0)
    ce_max = max(float(np.nanmax(ce)), 1e-5)

    # Guess amplitudes in the unscaled space (divide observed max by ce_max)
    a_guess_total = ymax / ce_max
    p0_a3 = 0.6 * a_guess_total
    p0_a4 = 0.4 * a_guess_total

    # Guess two time scales
    b3_0 = np.log(2) / max(0.2 * tmax, 1e-3)
    b4_0 = np.log(2) / max(0.8 * tmax, 1e-3)

    p0 = [p0_a3, b3_0, p0_a4, b4_0]
    bounds = ([2.5, 1e-5, 1.0, 1e-5], [99.0, 9.0, 99.0, 9.0])
    ...
    if w is not None:
        ...
        w = w / (np.max(w) + 1e-8)
        sigma = 1.0 / np.sqrt(np.maximum(w, 1e-6))  # weighted LS
    try:
        func = lambda tt, a3, b3, a4, b4: _two_exp_additive_with_ce(tt, a3, b3, a4, b4, ce)
...
def compute_catalyst_effect(cat, K, p):
    """ce = (cat / (cat + K))**p, vectorized for numpy arrays."""
    cat = np.asarray(cat, dtype=float)
    K = float(max(K, 1e-12))
    p = float(p)
    return np.power(cat / (cat + K), p)

def compute_param_limits_from_actual(y_list, time_list, catalyst_list, tt_tensor, sample_ids, sample_col_ids, save_dir):
    """
    Prefit with global catalyst-effect parameters:
      0) Calibrate global (K, p, lag) by grid search across catalyzed rows
      1) Fit base (a1,b1,a2,b2) per sample from Control row (or pre-TT fallback); cap a1+a2<=90
      2) Fit catalyzed additive on post-TT with y_add = y - base, using ce=(cat/(cat+K))^p and time lag: τ = max(0, t-tt-lag)
         Enforce: a_i >= 1.0, b_i >= 1e-5, and a1+a2+a3+a4 <= 99 by scaling a3,a4 only.
    Saves constrained per-row fits, selected (K,p,lag), strict min/max per parameter, and preview plots.
    """
    eps_a, eps_b = 1.0, 1e-5

    # -------- Pass 1: fit base per sample_id from Control row if available --------
    base_by_sid = {}
    for i in range(len(y_list)):
        sid = sample_ids[i]
        colid = sample_col_ids[i]
        t = time_list[i].detach().cpu().numpy() if isinstance(time_list[i], torch.Tensor) else np.asarray(time_list[i])
        y = y_list[i].detach().cpu().numpy() if isinstance(y_list[i], torch.Tensor) else np.asarray(y_list[i])
        cat = catalyst_list[i].detach().cpu().numpy() if isinstance(catalyst_list[i], torch.Tensor) else np.asarray(catalyst_list[i])
        tt = float(tt_tensor[i].item() if isinstance(tt_tensor[i], torch.Tensor) else tt_tensor[i])

        is_control_row = (str(colid) == 'Control') or np.all(cat <= 0)
        if is_control_row:
            t_ctrl, y_ctrl = t, y
        else:
            pre_mask = (t < tt)
            t_ctrl, y_ctrl = t[pre_mask], y[pre_mask]

        if t_ctrl.size >= 4 and y_ctrl.size >= 4:
            fit = fit_two_exp_params(t_ctrl, y_ctrl)
            if fit is not None:
                a1, b1, a2, b2 = map(float, fit)
                # Positivity
                a1 = max(a1, eps_a); a2 = max(a2, eps_a)
                b1 = max(b1, eps_b); b2 = max(b2, eps_b)
                # Cap base sum at 90
                base_sum = a1 + a2
                if base_sum > 90.0 and base_sum > 0:
                    s = 90.0 / base_sum
                    a1 *= s; a2 *= s
                base_by_sid[sid] = (a1, b1, a2, b2)

    # -------- Global ce(K,p,lag) calibration by grid search across catalyzed rows --------
    grids = CONFIG.get('ce_param_search', {'K_grid': [1.0], 'p_grid': [1.0], 'lag_grid': [0.0]})
    K_grid = grids.get('K_grid', [1.0])
    p_grid = grids.get('p_grid', [1.0])
    lag_grid = grids.get('lag_grid', [0.0])

    # Prepare a list of catalyzed rows with usable base fits and post-TT
    cat_indices = []
    for i in range(len(y_list)):
        colid = sample_col_ids[i]
        t = time_list[i]; tt = tt_tensor[i]
        has_cat = torch.any(catalyst_list[i] > 0).item() if isinstance(catalyst_list[i], torch.Tensor) else np.any(catalyst_list[i] > 0)
        if has_cat and sample_ids[i] in base_by_sid:
            t_np = t.detach().cpu().numpy() if isinstance(t, torch.Tensor) else np.asarray(t)
            tt_val = float(tt.item() if isinstance(tt, torch.Tensor) else tt)
            if np.sum(t_np >= tt_val) >= 4:
                cat_indices.append(i)

    def eval_candidate(K, p, lag):
        sse_total = 0.0
        n_pts = 0
        for i in cat_indices:
            sid = sample_ids[i]
            a1, b1, a2, b2 = base_by_sid[sid]
            t = time_list[i].detach().cpu().numpy() if isinstance(time_list[i], torch.Tensor) else np.asarray(time_list[i])
            y = y_list[i].detach().cpu().numpy() if isinstance(y_list[i], torch.Tensor) else np.asarray(y_list[i])
            cat = catalyst_list[i].detach().cpu().numpy() if isinstance(catalyst_list[i], torch.Tensor) else np.asarray(catalyst_list[i])
            tt = float(tt_tensor[i].item() if isinstance(tt_tensor[i], torch.Tensor) else tt_tensor[i])
            ...
            # Base
            y_base = _two_exp_model(t, a1, b1, a2, b2)

            # Post-TT
            post_mask = (t >= tt)
            if np.sum(post_mask) < 4:
                continue
            t_shift = np.maximum(0.0, t[post_mask] - tt - lag)
            ce_post = compute_catalyst_effect(cat[post_mask], K, p)

            # y_add to fit
            y_add = np.clip(y[post_mask] - y_base[post_mask], a_min=1.0, a_max=None)

            # Weighted LS toward active catalyst windows
            w_pow = float(CONFIG.get('catalyst_fit_weight_power', 1.5))
            w = np.power(ce_post / (np.max(ce_post) + 1e-8), w_pow)
            sel = w > float(CONFIG.get('catalyst_fit_min_weight', 0.02))
            t_fit = t_shift[sel] if sel.any() else t_shift
            y_fit = y_add[sel] if sel.any() else y_add
            ce_fit = ce_post[sel] if sel.any() else ce_post
            w_fit = w[sel] if sel.any() else w

            if np.nanmax(ce_fit) <= 1e-8 or np.nanmax(y_fit) <= 1e-6 or t_fit.size < 3:
                continue

            tmp = fit_two_exp_params_additive_with_ce(t_fit, y_fit, ce_fit, w=w_fit)
            if tmp is None:
                tmp1 = fit_one_exp_params_additive_with_ce(t_fit, y_fit, ce_fit, w=w_fit)
                if tmp1 is None:
                    continue
                a3, b3 = tmp1
                a4, b4 = eps_a, 1.0
            ...
            # Enforce minima and totals
            a3 = max(float(a3), eps_a); a4 = max(float(a4), eps_a)
            b3 = max(float(b3), eps_b); b4 = max(float(b4), eps_b)
            base_sum = a1 + a2
            cat_sum = a3 + a4
            total = base_sum + cat_sum
            if total > 99.0 and cat_sum > 0:
                s = max(0.0, 99.0 - base_sum) / max(cat_sum, 1e-6)
                a3 *= s; a4 *= s

            # Predict full curve with selected ce/lag
            add_post = _two_exp_additive_with_ce(t_shift, a3, b3, a4, b4, ce_post)
            add_full = np.zeros_like(t); add_full[post_mask] = add_post
            y_pred = y_base + add_full
            ...
            if np.sum(m) > 0:
                ...
                n_pts += int(np.sum(m))
        return sse_total, n_pts

    best = (1.0, 1.0, 0.0)  # default K,p,lag
    best_sse = np.inf
    best_pts = 0
    for K in K_grid:
        for p in p_grid:
            for lag in lag_grid:
                sse, npts = eval_candidate(K, p, lag)
                if npts > 0 and sse < best_sse:
                    best_sse, best_pts, best = sse, npts, (K, p, lag)

    ce_K, ce_p, ce_lag = best
    # Persist the chosen values for later stages (optional)
    CONFIG['ce_K'] = float(ce_K)
    CONFIG['ce_p'] = float(ce_p)
    CONFIG['ce_lag'] = float(ce_lag)

    # -------- Pass 2: per-row outputs using chosen (K,p,lag) --------
    rows = []
    for i in range(len(y_list)):
        sid = sample_ids[i]
        colid = sample_col_ids[i]
        ...
        a1=b1=a2=b2=a3=b3=a4=b4=np.nan

        # Base from Control row or fallback pre-TT
        base_fit = base_by_sid.get(sid, None)
        if base_fit is None:
            pre_mask = (t < tt)
            if pre_mask.sum() >= 4:
                tmp = fit_two_exp_params(t[pre_mask], y[pre_mask])
                if tmp is not None:
                    a1, b1, a2, b2 = map(float, tmp)
                    a1 = max(a1, eps_a); a2 = max(a2, eps_a)
                    b1 = max(b1, eps_b); b2 = max(b2, eps_b)
                    base_sum = a1 + a2
                    if base_sum > 90.0 and base_sum > 0:
                        s = 90.0 / base_sum
                        a1 *= s; a2 *= s
                    base_fit = (a1, b1, a2, b2)

        if base_fit is not None:
            a1, b1, a2, b2 = base_fit

        # Catalyst additive only on Catalyzed row with valid base
        is_catalyzed_row = (str(colid) == 'Catalyzed') or np.any(cat > 0)
        if is_catalyzed_row and base_fit is not None and np.isfinite(tt):
            post_mask = (t >= tt)
            if post_mask.sum() >= 4:
                y_base_post = _two_exp_model(t[post_mask], a1, b1, a2, b2)
                y_add = np.clip(y[post_mask] - y_base_post, a_min=1.0, a_max=None)
                t_shift = np.maximum(0.0, t[post_mask] - tt - ce_lag)
                ce_post = compute_catalyst_effect(cat[post_mask], ce_K, ce_p)

                # Weight more where catalyst is active
                w_pow = float(CONFIG.get('catalyst_fit_weight_power', 1.5))
                w = np.power(ce_post / (np.max(ce_post) + 1e-8), w_pow)
                sel = w > float(CONFIG.get('catalyst_fit_min_weight', 0.02))
                if sel.sum() >= 4:
                    t_fit, y_fit, ce_fit, w_fit = t_shift[sel], y_add[sel], ce_post[sel], w[sel]
                else:
                    t_fit, y_fit, ce_fit, w_fit = t_shift, y_add, ce_post, w

                if np.nanmax(ce_fit) > 1e-8 and np.nanmax(y_fit) > 1e-6:
                    tmp = fit_two_exp_params_additive_with_ce(t_fit, y_fit, ce_fit, w=w_fit)
                    if tmp is not None:
                        a3, b3, a4, b4 = map(float, tmp)
                    else:
                        tmp1 = fit_one_exp_params_additive_with_ce(t_fit, y_fit, ce_fit, w=w_fit)
                        if tmp1 is not None:
                            a_single, b_single = tmp1
                            a3, b3 = a_single, b_single
                            a4, b4 = eps_a, 1.0

                    if np.isfinite(a3):
                        a3 = max(a3, eps_a); a4 = max(a4, eps_a)
                        b3 = max(b3, eps_b); b4 = max(b4, eps_b)
                        base_sum = a1 + a2
                        cat_sum = a3 + a4
                        total = base_sum + cat_sum
                        if total > 99.0 and cat_sum > 0:
        ...
        # Save constrained row + sums + metrics
        sum_base = (a1 + a2) if np.all(np.isfinite([a1,a2])) else np.nan
        sum_total = (a1 + a2 + (a3 if np.isfinite(a3) else 0.0) + (a4 if np.isfinite(a4) else 0.0)) \
                    if np.all(np.isfinite([a1,a2])) else np.nan

        rmse_fit = np.nan
        r2_fit = np.nan
        try:
            if np.all(np.isfinite([a1, b1, a2, b2])):
                y_base = _two_exp_model(t, a1, b1, a2, b2)
            else:
                y_base = np.full_like(t, np.nan, dtype=float)

            y_pred = y_base.copy()
            if is_catalyzed_row and np.all(np.isfinite([a3, b3, a4, b4])) and np.isfinite(tt):
                ce = compute_catalyst_effect(cat, ce_K, ce_p)
                post_mask = (t >= tt)
                if np.any(post_mask):
                    t_shift_full = np.maximum(0.0, t[post_mask] - tt - ce_lag)
                    add_post = _two_exp_additive_with_ce(t_shift_full, a3, b3, a4, b4, ce[post_mask])
                    add_full = np.zeros_like(t, dtype=float)
                    add_full[post_mask] = add_post
                    y_pred = y_base + add_full

            m = np.isfinite(y) & np.isfinite(y_pred)
            if np.sum(m) >= 1:
                err = y[m] - y_pred[m]
                rmse_fit = float(np.sqrt(np.mean(err**2)))
                if np.sum(m) >= 2 and np.var(y[m]) > 0:
                    r2_fit = float(1.0 - np.sum(err**2) / np.sum((y[m] - np.mean(y[m]))**2))
        except Exception:
            pass

        rows.append({
            'project_sample_id_reactormatch': sid,
            'project_col_id': colid,
            'a1': a1, 'b1': b1, 'a2': a2, 'b2': b2,
            'a3': a3, 'b3': b3, 'a4': a4, 'b4': b4,
            'sum_base_a1_a2': sum_base,
            'sum_total_a1_a2_a3_a4': sum_total,
            'tt': tt,
            'rmse_fit': rmse_fit,
            'r2_fit': r2_fit
        })
    ...
    plots_root = os.path.join(save_dir, 'plots')
    os.makedirs(plots_root, exist_ok=True)
    fits_dir = os.path.join(plots_root, 'param_fit_previews')
    os.makedirs(fits_dir, exist_ok=True)
    df_params.to_csv(os.path.join(plots_root, 'fitted_params_from_actual.csv'), index=False)
    # Save selected ce params
    pd.DataFrame([{'ce_K': ce_K, 'ce_p': ce_p, 'ce_lag': ce_lag, 'n_points': best_pts, 'sse': best_sse}])\
        .to_csv(os.path.join(plots_root, 'selected_catalyst_effect_params.csv'), index=False)

    # --- Build strict min/max from the constrained fits (no quantiles, no padding) ---
    limits = {}
    for k in ['a1','b1','a2','b2','a3','b3','a4','b4']:
        series = df_params[k].dropna()
        if series.empty:
            limits[k] = (1.0, 99.0) if k.startswith('a') else (1e-5, 100.0)
            continue
        lo = float(series.min()); hi = float(series.max())
        floor = 1.0 if k.startswith('a') else 1e-5
        lo = max(lo, floor)
        hi = max(hi, lo * 1.05)
        limits[k] = (lo, hi)

    pd.DataFrame([{'param': k, 'min': v[0], 'max': v[1]} for k, v in limits.items()])\
      .to_csv(os.path.join(plots_root, 'param_limits_from_actual_constrained.csv'),
              index=False, float_format='%.6g')

    # Save a simple fit plot per row (time vs actual vs fitted) using chosen ce/lag
    for row in rows:
        sid = row['project_sample_id_reactormatch']
        colid = row['project_col_id']
        i = next(idx for idx in range(len(sample_ids)) if sample_ids[idx] == sid and sample_col_ids[idx] == colid)
        ...
        a1, b1, a2, b2 = row['a1'], row['b1'], row['a2'], row['b2']
        a3, b3, a4, b4 = row['a3'], row['b3'], row['a4'], row['b4']

        # Base prediction on full t
        if np.all(np.isfinite([a1, b1, a2, b2])):
            y_base = _two_exp_model(t, a1, b1, a2, b2)
        else:
            y_base = np.full_like(t, np.nan, dtype=float)
        ...
        y_pred = y_base.copy()
        is_cat_row = (str(colid) == 'Catalyzed') or np.any(cat > 0)
        if is_cat_row and np.all(np.isfinite([a3, b3, a4, b4])) and np.isfinite(tt):
            ce = compute_catalyst_effect(cat, ce_K, ce_p)
            post_mask = (t >= tt)
            if np.any(post_mask):
                t_shift = np.maximum(0.0, t[post_mask] - tt - ce_lag)
                add_post = _two_exp_additive_with_ce(t_shift, a3, b3, a4, b4, ce[post_mask])
                add_full = np.zeros_like(t, dtype=float)
                add_full[post_mask] = add_post
                y_pred = y_base + add_full
        ...
        plt.plot(t_s, y_s, 'o', alpha=0.35, label='Actual', color='tab:blue', markeredgecolor='none')
        if np.all(np.isfinite(yb_s)):
            plt.plot(t_s, yb_s, '--', label='Base fit', color='tab:gray', linewidth=1.5)
        if np.all(np.isfinite(yp_s)):
            plt.plot(t_s, yp_s, '-', label='Full fit', color='tab:orange', linewidth=2.0)
        if is_cat_row and np.isfinite(tt):
            plt.axvline(x=tt, ymin=0, ymax=1, color='tab:orange', linestyle='--', alpha=0.7, label='Transition')

        rmse_txt = f"{row.get('rmse_fit', np.nan):.2f}" if np.isfinite(row.get('rmse_fit', np.nan)) else "NA"
        r2_txt = f"{row.get('r2_fit', np.nan):.2f}" if np.isfinite(row.get('r2_fit', np.nan)) else "NA"
        plt.title(f"{sid} | {colid}  RMSE={rmse_txt}  R²={r2_txt}", fontsize=9)
        ...
        safe_col = re.sub(r'[^A-Za-z0-9_.-]+', '_', str(colid))
        out_path = os.path.join(fits_dir, f"{safe_sid}__{safe_col}.png")
        plt.tight_layout()
...
class AdaptiveTwoPhaseRecoveryModel(nn.Module):
    ...
    """
    def __init__(self, total_features, hidden_dim=128, dropout_rate=0.33, init_mode='kaiming', feature_weight_signs=None, feature_weight_magnitudes=None):
        super().__init__()
    ...
    def forward(self, x, catalyst, sample_ids=None):
        ...
        # multiplier when leach_duration_days is not the time feature
        if 'leach_duration_days' in CONFIG.get('special_feats', {}).get('time_feat', []):
            b_multiplier = 1.0
        else:
            b_multiplier = 1000/10

        # Parameter limits (from CONFIG if available)
        lim = CONFIG.get('param_limits', {})
        a1_min, a1_max = lim.get('a1', (0.1, 50.1))
        b1_min, b1_max = lim.get('b1', (1e-4, 0.2))
        a2_min, a2_max = lim.get('a2', (0.1, 50.1))
        b2_min, b2_max = lim.get('b2', (1e-4, 0.2))

        # Catalyst parameters
        a3_min, a3_max = lim.get('a3', (0.1, 50.1))
        b3_min, b3_max = lim.get('b3', (1e-4, 0.2))
        a4_min, a4_max = lim.get('a4', (0.1, 50.1))
        b4_min, b4_max = lim.get('b4', (1e-4, 0.2))
        ...
        if sample_ids is not None:
            ...
            for unique_id in unique_sample_ids:
                ...
                if sample_indices:
                    ...
                    # Enforce base cap: a1 + a2 <= 90
                    total_asymptote = a1 + a2
                    mask_a = total_asymptote > 90.0
                    scale = torch.where(mask_a & (total_asymptote > 0),
                                        90.0 / total_asymptote.clamp(min=1.0),
        ...
        if has_catalyst.any():
            ...
            if idx.numel() > 0:
                ...
                # Enforce total cap: a1 + a2 + a3 + a4 <= 99
                total_asymptote_cat = a1_r + a2_r + a3 + a4
                mask_a = total_asymptote_cat > 99.0
                scale = torch.where(mask_a & (total_asymptote_cat > 0),
                                    99.0 / total_asymptote_cat.clamp(min=1.0),
                                    torch.tensor(1.0, device=x.device))
            ...
            # params[:, 4:] = np.nan
            params[:, 4:] = torch.full((batch_size, 4), float('nan'), device=x.device)
...
def generate_two_phase_recovery(time, catalyst, transition_time, params):
    ...
    # Use the ce parameters determined in prefit (with sensible fallbacks)
    ce_K = float(CONFIG.get('ce_K', CONFIG.get('ce_param_search', {}).get('K_grid', [1.0])[0]))
    ce_p = float(CONFIG.get('ce_p', CONFIG.get('catalyst_effect_power', 1.0)))
    ce_lag = float(CONFIG.get('ce_lag', 0.0))

    # Global catalyst effect
    catalyst_effect = (catalyst / (catalyst + ce_K)).pow(ce_p)

    # Base recovery (control)
    a1 = params[:, 0].unsqueeze(1); b1 = params[:, 1].unsqueeze(1)
    a2 = params[:, 2].unsqueeze(1); b2 = params[:, 3].unsqueeze(1)
    exp1 = torch.exp(-b1 * time).clamp(min=1e-8, max=1.0)
    exp2 = torch.exp(-b2 * time).clamp(min=1e-8, max=1.0)
    recovery_control = a1 * (1 - exp1) + a2 * (1 - exp2)
    ...
    if has_catalyst and params.shape[1] > 4 and torch.any(~torch.isnan(params[:, 4:])):
        a3 = params[:, 4].unsqueeze(1); b3 = params[:, 5].unsqueeze(1)
        a4 = params[:, 6].unsqueeze(1); b4 = params[:, 7].unsqueeze(1)

        post_mask = (catalyst > 0) & (time >= transition_i)
        if post_mask.any():
            # Apply lag consistently with prefit
            t_shift = torch.clamp(time - transition_i - ce_lag, min=0.0)
            exp3 = torch.exp(-torch.abs(b3) * t_shift).clamp(min=1e-8, max=1.0)
            exp4 = torch.exp(-torch.abs(b4) * t_shift).clamp(min=1e-8, max=1.0)
            add = (torch.abs(a3) * (1 - exp3) + torch.abs(a4) * (1 - exp4)) * catalyst_effect
            add = torch.where(post_mask, add, torch.zeros_like(add))
            recovery = recovery_control + add

    return recovery.clamp(min=0.0, max=100.0)
...
def compute_monotonic_penalty(model, lambda_penalty=0.1):
    ...
    param_scales = torch.tensor([
        ...
        1000.0,  # b1: scale = 1000.0 (tiny values need huge penalty)
        1.0,      # a2: scale = 1.0
        10000.0,  # b2: scale = 10000.0
        1.0,      # a3: scale = 1.0
        10000.0,  # b3: scale = 10000.0
        1.0,      # a4: scale = 1.0
        10000.0,  # b4: scale = 10000.0
...
def compute_sample_weights(time_steps, device):
    ...
    for time_array in time_steps:
        ...
        full_weight_curve = np.linspace(1.0, 2.0, len(time_array))
...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    for epoch in range(epochs):
        ...
        for j in range(len(X_train)):
            ...
            # NEW: catalyst-aware weighting (consistent with prefit ce)
            ce_K = float(CONFIG.get('ce_K', CONFIG.get('ce_param_search', {}).get('K_grid', [1.0])[0]))
            ce_p = float(CONFIG.get('ce_p', CONFIG.get('catalyst_effect_power', 1.0)))
            gamma = float(CONFIG.get('catalyst_fit_weight_power', 1.5))
            ce = (catalyst_sample / (catalyst_sample + ce_K)).pow(ce_p)  # [1,T]
            post_mask = (time_sample >= tt_sample) & (catalyst_sample > 0)
            ce_norm = ce / (torch.max(ce) + 1e-8)
            w_ce = torch.pow(ce_norm, gamma)
            # Only upweight post‑TT+cat points; keep others unchanged
            w_mult = torch.where(post_mask, w_ce, torch.ones_like(w_ce))
            weights_sample = weights_sample * w_mult
            ...
            # Total loss
            loss = mse_loss_value + physics_loss + monotonic_penalty
    ...
    with torch.no_grad():
        for unique_sample_id in unique_test_sample_ids:
            ...
            if catalyzed_idx is not None:
                ...
                # Enforce final cap with the actual base you keep
                total = (combined_params[0] + combined_params[2] + combined_params[4] + combined_params[6]).item()
                if total > 99.0:
                    factor = 99.0 / max(total, 1.0)
                    combined_params[4] *= factor
                    combined_params[6] *= factor
...
if __name__ == "__main__":
    ...
    param_limits = compute_param_limits_from_actual(
    y_list=y_tensor,
    time_list=time_tensor,
    catalyst_list=catalyst_tensor,
    tt_tensor=tt_tensor,
    sample_ids=sample_ids,
    sample_col_ids=sample_col_ids,
    ...
    CONFIG['param_limits'] = param_limits
    print("Derived parameter limits:")
    ...
    # Model wrapper for catalyst parameters (with catalyst)
    class CatalystModelWrapper(nn.Module):
        ...
        def forward(self, x):
            catalyst_dummy = torch.ones(x.shape[0], 1, device=x.device)  # Simulate catalyst presence
            params = self.model(x, catalyst_dummy)
            return params[:, 4:]  # Only return catalyst parameters (a3, b3, a4, b4)
    
    # Separate samples by catalyst presence
    # catalyst_tensor is a list of tensors, check if any element has catalyst > 0
    catalyst_present = np.array([torch.any(cat > 0).item() for cat in catalyst_tensor])
    non_catalyst_indices = np.where(~catalyst_present)[0]
    catalyst_indices = np.where(catalyst_present)[0]
    ...
    if len(non_catalyst_indices) > 0:
        ...
        weighted_shap_base = []
        for output_dim in range(4):  # a1, b1, a2, b2
            output_shap = sum(
    ...
    print("\n  Computing SHAP for catalyst parameters (a3, b3, a4, b4)...")
    if len(catalyst_indices) > 0:
        background_indices_cat = random.sample(
            list(catalyst_indices), 
            min(CONFIG['shap_background_size'], len(catalyst_indices))
        )
        explain_indices_cat = random.sample(
            list(catalyst_indices), 
            min(CONFIG['shap_sample_size'], len(catalyst_indices))
        )
        ...
        # Weighted average for catalyst parameters
        weighted_shap_cat = []
        for output_dim in range(4):  # a3, b3, a4, b4
            output_shap = sum(
    ...
    for output_dim, param in enumerate(param_names):
        # Use base samples for a1-b2, catalyst samples for a3-b4
        if output_dim < 4:  # Base parameters
            X_for_plot = X_explain_base.cpu().numpy()
            shap_for_plot = weighted_shap[output_dim]
        else:  # Catalyst parameters
            X_for_plot = X_explain_cat.cpu().numpy()
            shap_for_plot = weighted_shap[output_dim]
        
        # Only plot if we have valid SHAP values
    ...
    plt.title(f'SHAP % importance for Catalyzed a-params')
    plt.tight_layout()
    plt.ylabel('Feature')
    plt.xlabel('% importance')
    plt.savefig(os.path.join(plots_dir, f'percentual_feature_importance_a-catalyzed-params.png'), dpi=300)
    plt.show()
    plt.close()
    ...
    plt.title(f'SHAP % importance for Catalyzed b-params')
    plt.tight_layout()
    plt.ylabel('Feature')
    plt.xlabel('% importance')
    plt.savefig(os.path.join(plots_dir, f'percentual_feature_importance_b-catalyzed-params.png'), dpi=300)
    plt.show()
    plt.close()
