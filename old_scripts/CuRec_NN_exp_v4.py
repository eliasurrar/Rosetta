...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            if val_indices:
                for idx_pos, j in enumerate(val_indices):
                    ...
                    val_losses.append((weighted_loss_val.sum() / total_weighted_points_val) * loss_scale)
            else:
                val_losses = [torch.tensor(avg_loss, device=device)]
        ...
        avg_val_value = avg_val_loss.item() if torch.is_tensor(avg_val_loss) else float(avg_val_loss)
    ...
    if config.get('generate_plots', False):
        ...
        if np.isfinite(test_rmse_unweighted):
            subtitle_parts.append(f"Test RMSE: {test_rmse_unweighted:.4f}")
        if np.isfinite(test_mse_unweighted):
            subtitle_parts.append(f"Test MSE: {test_mse_unweighted:.4f}")
        subtitle_parts.append(f"Val Loss (train split): {best_val_loss:.4f}")
