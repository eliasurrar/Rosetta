...
CONFIG = {
    ...
    'pytorch_epochs': 10000,      # Maximum number of epochs for PyTorch training
...
project_dict = {
    ...
    '006_jetti_project_file_pvls': [],  # No matching key
...
class ReactorScalingEnsemble:
    """Ensemble model for reactor scaling with uncertainty quantification"""
    
    def __init__(self, model_states, val_losses, reactor_param_indices, total_features, config, device):
        self.device = device
        self.reactor_param_indices = reactor_param_indices
    ...
    def _create_filtered_ensemble(self, model_states, val_losses, config):
        ...
        threshold = median_loss * 2.0 # original was 1.5.- The higher the value, the more models included
        ...
        for model_state, val_loss in zip(model_states, val_losses):
            if val_loss <= threshold:
                model = ReactorScalingModel(
                    total_features=self.total_features,
                    reactor_param_indices=self.reactor_param_indices,
...
class ReactorParameterAttention(nn.Module):
    """Attention mechanism for reactor parameters"""
    def __init__(self, reactor_dim=4, hidden_dim=64):
        super().__init__()
        self.reactor_dim = reactor_dim
        self.hidden_dim = hidden_dim
        
        # Attention layers
        self.query = nn.Linear(reactor_dim, hidden_dim)
        self.key = nn.Linear(reactor_dim, hidden_dim)
        self.value = nn.Linear(reactor_dim, hidden_dim)
        self.output_proj = nn.Linear(hidden_dim, reactor_dim)
        
        # Layer normalization
        self.layer_norm = nn.LayerNorm(reactor_dim)
        
    def forward(self, reactor_params):
        # reactor_params: [batch_size, 4] (a1_true, b1_true, a2_true, b2_true)
        batch_size = reactor_params.size(0)
        
        # Self-attention on reactor parameters
        Q = self.query(reactor_params)  # [batch_size, hidden_dim]
        K = self.key(reactor_params)    # [batch_size, hidden_dim]
        V = self.value(reactor_params)  # [batch_size, hidden_dim]
        
        # Compute attention scores
        attention_scores = torch.matmul(Q.unsqueeze(1), K.unsqueeze(2)) / (self.hidden_dim ** 0.5)
        attention_weights = torch.softmax(attention_scores.squeeze(), dim=-1)
        
        # Apply attention
        attended_values = self.output_proj(V * attention_weights.unsqueeze(-1))
        
        # Residual connection and layer norm
        output = self.layer_norm(reactor_params + attended_values)
        
        return output, attention_weights

class ReactorScalingModel(nn.Module):
    """
    Model that scales reactor parameters using attention mechanism and other predictors
    """
    def __init__(self, total_features, reactor_param_indices, hidden_dim=128, dropout_rate=0.3):
        super().__init__()
        
        self.reactor_param_indices = reactor_param_indices
        self.other_features_count = total_features - len(reactor_param_indices)
        self.dropout_rate = dropout_rate
        
        # Reactor parameter attention mechanism
        self.reactor_attention = ReactorParameterAttention(reactor_dim=4, hidden_dim=64)
        
        # Network for other predictor variables (scaling factors)
        self.scaling_network = nn.Sequential(
            nn.Linear(self.other_features_count, hidden_dim),
            nn.LeakyReLU(0.1),
            ...
            nn.LeakyReLU(0.1),
            nn.Linear(32, 8)  # 4 base scaling factors + 4 catalyst scaling factors
        )
        
        self.base_processor = nn.Sequential(
            nn.Linear(4 + 4, hidden_dim // 2),  # reactor params + base scaling
            nn.LeakyReLU(0.1),
            nn.Dropout(dropout_rate),
            nn.Linear(hidden_dim // 2, 4)  # a1, b1, a2, b2
        )
        
        # Catalyst parameter processing
        self.catalyst_processor = nn.Sequential(
            nn.Linear(4 + 4, hidden_dim // 2),  # reactor params + catalyst scaling
            nn.LeakyReLU(0.1),
            nn.Dropout(dropout_rate),
            nn.Linear(hidden_dim // 2, 4)  # a3, b3, a4, b4
        )
    ...
    def _initialize_weights(self):
        ...
        for module in [self.scaling_network, self.base_processor, self.catalyst_processor]:
            for layer in module:
                if isinstance(layer, nn.Linear):
                    nn.init.kaiming_uniform_(layer.weight, nonlinearity='leaky_relu')
                    nn.init.constant_(layer.bias, 0.0)
        
        # Initialize final layer biases to reasonable values
        with torch.no_grad():
            self.base_processor[-1].bias[0] = 0.0   # a1
            self.base_processor[-1].bias[1] = -2.0  # b1
            self.base_processor[-1].bias[2] = 0.0   # a2
            self.base_processor[-1].bias[3] = -2.0  # b2
            
            self.catalyst_processor[-1].bias[0] = 0.0   # a3
            self.catalyst_processor[-1].bias[1] = -2.0  # b3
            self.catalyst_processor[-1].bias[2] = 0.0   # a4
            self.catalyst_processor[-1].bias[3] = -2.0  # b4
    
    def forward(self, x, catalyst, sample_ids=None):
        batch_size = x.size(0)
        
        # Separate reactor parameters and other features
        reactor_params = x[:, self.reactor_param_indices]  # [batch_size, 4]
        other_features = torch.cat([x[:, :self.reactor_param_indices[0]], 
                                   x[:, self.reactor_param_indices[-1]+1:]], dim=1)
        
        # Apply attention to reactor parameters
        attended_reactor_params, attention_weights = self.reactor_attention(reactor_params)
        
        # Generate scaling factors from other features
        scaling_factors = self.scaling_network(other_features)  # [batch_size, 8]
        base_scaling = scaling_factors[:, :4]      # For base parameters
        catalyst_scaling = scaling_factors[:, 4:]  # For catalyst parameters
        ...
        if sample_ids is not None:
            ...
            for unique_id in unique_sample_ids:
                ...
                if sample_indices:
                    ...
                    base_input = torch.cat([attended_reactor_params[first_idx:first_idx+1], 
                                          base_scaling[first_idx:first_idx+1]], dim=1)
                    base_raw = self.base_processor(base_input)
                    ...
                    # ensure a1 >= a2
                    mask_a = a1 < a2
                    a1_new = torch.where(mask_a, a2, a1)
                    a2_new = torch.where(mask_a, a1, a2)
                    a1, a2 = a1_new, a2_new   
            ...
            base_input = torch.cat([attended_reactor_params, base_scaling], dim=1)
            base_raw = self.base_processor(base_input)
        ...
        if has_catalyst.any():
            ...
            if idx.numel() > 0:
                ...
                catalyst_input = torch.cat([attended_reactor_params[idx], catalyst_scaling[idx]], dim=1)
                cat_raw = self.catalyst_processor(catalyst_input)
...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    fold_data, reactor_param_indices, num_cols, config, device, save_dir, X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor, sample_ids, sample_col_ids, use_val_data_days = fold_data_with_params
    ...
    fold_model = ReactorScalingModel(
        total_features=len(num_cols),
        reactor_param_indices=reactor_param_indices,
        hidden_dim=config.get('hidden_dim', 128),
        dropout_rate=config.get('dropout_rate', 0.3)
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        for j in range(len(X_train)):
            ...
            # Special attention to b2 and b4 parameters (kinetics)
            b2_penalty = torch.abs(params[0, 3] - X_train[j, reactor_param_indices[3]]) * 2.0
            if torch.any(catalyst_sample > 0):
                b4_penalty = torch.abs(params[0, 7] - X_train[j, reactor_param_indices[3]]) * 2.0
                loss += b4_penalty * 0.1
            loss += b2_penalty * 0.1
    ...
    plt.savefig(os.path.join(plots_dir, f'adaptive_loss_curve_{test_sample_id}_withReactors.png'), dpi=300)
    plt.close()
    ...
    with torch.no_grad():
        ...
        for j, unique_sample_id in enumerate(test_sample_ids):
            ...
            fold_results.append({
                ...
                'a1_true': X_test[j, reactor_param_indices[0]].item(),
                'b1_true': X_test[j, reactor_param_indices[1]].item(),
                'a2_true': X_test[j, reactor_param_indices[2]].item(),
                'b2_true': X_test[j, reactor_param_indices[3]].item(),
            })
    ...
    plots_dir = os.path.join(save_dir, 'plots')
    os.makedirs(plots_dir, exist_ok=True)
    plt.savefig(os.path.join(plots_dir, f"val_columns_{test_sample_id}_withReactors.png"), dpi=300)
    plt.close()
    ...
    if len(train_y_true) > 0 and len(all_y_true) > 0:
        ...
        if fig is not None:
            ...
            fig.savefig(os.path.join(save_dir, 'plots', f'diagnostics_{test_sample_id}_withReactors.png'), dpi=300)
            plt.close(fig)
...
def train_reactor_scaling_model_parallel(X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor, 
    ...
    # Find reactor parameter indices in num_cols
    reactor_param_names = ['a1_true', 'b1_true', 'a2_true', 'b2_true']
    reactor_param_indices = []
    for param_name in reactor_param_names:
        if param_name in num_cols:
            reactor_param_indices.append(num_cols.index(param_name))
        else:
            print(f"Warning: {param_name} not found in num_cols")
    
    if len(reactor_param_indices) != 4:
        raise ValueError(f"Expected 4 reactor parameters, found {len(reactor_param_indices)}")
    
    print(f"Reactor parameter indices: {reactor_param_indices}")
    ...
    fold_data_with_params = [
        (fold, reactor_param_indices, num_cols, config, device, save_dir, 
    ...
    summary_df.to_csv(os.path.join(save_dir, 'plots', 'column_prediction_summary_validation_withReactors.csv'), index=False)
    print(f"Saved column_prediction_summary_validation with {len(summary_df)} validation entries")
    
    # Create ensemble model
    ensemble = ReactorScalingEnsemble(
...
def create_ensemble_predictions_with_uncertainty(ensemble, X_tensor, y_tensor, time_tensor, catalyst_tensor, 
    ...
    for sample_id in unique_sample_ids:
        ...
        plt.xlim(0, max_days)
        plt.tight_layout()
        plt.savefig(os.path.join(plots_dir, f"ensemble_predictions_{sample_id}_withReactors.png"), dpi=300)
        plt.close()
    ...
    summary_df.to_csv(os.path.join(save_dir, 'plots', 'ensemble_prediction_summary_withReactors.csv'), index=False)
...
def plot_diagnostics_control_catalyzed(results_df, save_dir):
    ...
    for x in ['Control', 'Catalyzed']:
        ...
        sample_plot = results_df[results_df['project_col_id'] == x]
...
if __name__ == "__main__":
    ...
    print("\nStep 2: Training Reactor Scaling Model")
    print("=" * 60)
    ...
    torch.save({
        ...
        'num_cols': num_cols,
        'scaler_X': scaler_X
    }, os.path.join(folder_path_save, 'reactor_scaling_models.pt'))
