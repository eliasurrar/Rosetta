...
# ---------------------------
# PYMC Imports and Setup
# ---------------------------
import pymc as pm
import arviz as az
import pytensor.tensor as pt  # For custom tensor ops in model
from hyperopt import fmin, tpe, hp, STATUS_OK, Trials  # For Bayesian HPO
...
CONFIG = {
    ...
    'column_tests_feature_weighting': {
        ...
        'weights': { # title and weight
            ...
            # 'grouped_gangue_sulfides': ['Gangue Sulphides (%)', 0.0],
            'grouped_gangue_silicates': ['Gangue Silicates (%)', 0.0],
    ...
    'reactor_tests_feature_weighting': {
        ...
        'param_weights': { # Be aware that a_param now is (P - c) based on the equation c + (P - c)*(1 - np.exp(-b*t))
            ...
            'grouped_gangue_silicates': -0.2,
            'grouped_fe_oxides': -0.1,
            # 'grouped_carbonates': -0.3,
    ...
    'feature_mapping_reactor_cols' : {  # Reactor feature: Column feature (or None if not shared)
        ...
        'grouped_gangue_silicates': 'grouped_gangue_silicates',
...
def get_shared_features_from_mapping(config):
    """
    Extract shared features from feature_mapping_reactor_cols.
    Returns:
        - reactor_features: List of reactor feature names
        - column_features: List of corresponding column feature names
        - shared_features: Dict mapping reactor → column feature names
    """
    mapping = config.get('feature_mapping_reactor_cols', {})
    
    reactor_features = []
    column_features = []
    shared_features = {}
    
    for reactor_feat, column_feat in mapping.items():
        if column_feat is not None:  # Only shared features (not None)
            reactor_features.append(reactor_feat)
            column_features.append(column_feat)
            shared_features[reactor_feat] = column_feat
    
    return reactor_features, column_features, shared_features
...
def train_mf_2step_end_to_end(reactor_df, out_df_unscaled, config, save_dir):
    """
    True multi-fidelity training:
    1. Train LF model on reactor data: Ore Characteristics → Reactor Parameters
    2. Train HF model on column data: Ore Characteristics + LF_Predictions + Material Size → Column Parameters
    
    Args:
        reactor_df: DataFrame with reactor test data (for LF training)
        out_df_unscaled: DataFrame with column test data (for HF training)
        config: Configuration dictionary
        save_dir: Directory to save models
    
    Returns:
        nn_lf: Trained LF model
        nn_hf: Trained HF model
        mf_info: Dictionary with model info and scalers
    """
    
    print("=" * 60)
    print("Training True Multi-Fidelity Model")
    print("=" * 60)
    
    # ===== STEP 1: Train LF Model on Reactor Data =====
    print("\nStep 1: Training LF Model on Reactor Data")
    print("-" * 60)
    
    # Define shared ore characteristics (features that exist in both reactors and columns)
    # These are the features that the LF model uses to predict reactor behavior
    reactor_ore_features = [
        'cu_%',  # Assuming this exists in reactor_df
        'cu_seq_h2so4_norm%',  # Maps to acid_soluble_% in columns
        'cu_seq_a_r_norm%',  # Maps to residual_cpy_% in columns
        'grouped_copper_sulfides',
        'grouped_secondary_copper',
        'grouped_acid_generating_sulfides',
        'grouped_fe_oxides'
    ...
    # Filter to only use features that exist in reactor_df
    reactor_ore_features = [f for f in reactor_ore_features if f in reactor_df.columns]
    
    print(f"LF Model Input Features ({len(reactor_ore_features)}): {reactor_ore_features}")
    
    # Extract LF inputs and outputs
    lf_inputs = reactor_df[reactor_ore_features].values
    lf_outputs = reactor_df[['a1_param', 'b1_param', 'a2_param', 'b2_param']].values
    
    print(f"LF Training Data: {lf_inputs.shape[0]} reactor samples")
    
    # Normalize LF inputs
    scaler_lf = StandardScaler()
    lf_inputs_scaled = scaler_lf.fit_transform(lf_inputs)
    
    # Convert to tensors
    lf_inputs_tensor = torch.tensor(lf_inputs_scaled, dtype=torch.float32)
    lf_outputs_tensor = torch.tensor(lf_outputs, dtype=torch.float32)
    
    # Initialize LF model
    nn_lf = MFNN(
        input_dim=lf_inputs_tensor.shape[1],
        hidden_dims=[64, 64, 64],  # Deep architecture for reactor model
        output_dim=4,  # a1, b1, a2, b2
        dropout_rate=0.3
    )
    
    optimizer_lf = optim.Adam(nn_lf.parameters(), lr=0.001)
    criterion = nn.MSELoss()
    
    # Train LF model
    print("Training LF model...")
    best_lf_loss = float('inf')
    patience_lf = 50
    patience_counter_lf = 0
    
    for epoch in range(1000):
        nn_lf.train()
        optimizer_lf.zero_grad()
        pred_lf = nn_lf(lf_inputs_tensor)
        loss_lf = criterion(pred_lf, lf_outputs_tensor)
        loss_lf.backward()
        optimizer_lf.step()
        
        if (epoch + 1) % 100 == 0:
            print(f"  LF Epoch {epoch+1}/1000, Loss: {loss_lf.item():.6f}")
        
        # Early stopping
        if loss_lf.item() < best_lf_loss:
            best_lf_loss = loss_lf.item()
            patience_counter_lf = 0
            best_lf_state = nn_lf.state_dict().copy()
        else:
            patience_counter_lf += 1
            if patience_counter_lf >= patience_lf:
                print(f"  Early stopping at epoch {epoch+1}")
                break
    
    # Load best LF model
    nn_lf.load_state_dict(best_lf_state)
    print(f"LF model training complete. Best loss: {best_lf_loss:.6f}")
    
    # ===== STEP 2: Train HF Model on Column Data =====
    print("\nStep 2: Training HF Model on Column Data")
    print("-" * 60)
    
    # Define column ore features (mapped from reactor features)
    column_ore_features = [
        'acid_soluble_%',  # Maps from cu_seq_h2so4_norm%
        'residual_cpy_%',  # Maps from cu_seq_a_r_norm%
        'grouped_copper_sulfides',
        'grouped_secondary_copper',
        'grouped_acid_generating_sulfides',
        'grouped_fe_oxides'
    ...
    # Filter to only use features that exist in out_df_unscaled
    column_ore_features = [f for f in column_ore_features if f in out_df_unscaled.columns]
    
    # Column-specific features (scale-up factors)
    column_specific_features = ['material_size_p80_in']
    column_specific_features = [f for f in column_specific_features if f in out_df_unscaled.columns]
    
    print(f"HF Model Input Features:")
    print(f"  Ore characteristics ({len(column_ore_features)}): {column_ore_features}")
    print(f"  Column-specific ({len(column_specific_features)}): {column_specific_features}")
    print(f"  LF predictions (4): [a1_lf, b1_lf, a2_lf, b2_lf]")
    
    # Extract column ore characteristics
    hf_ore_inputs = out_df_unscaled[column_ore_features].values
    
    # *** KEY STEP: Get LF predictions for column samples ***
    # We need to map column ore features to reactor ore features for LF prediction
    
    # Create mapping from column features to reactor features
    feature_mapping = {
        'acid_soluble_%': 'cu_seq_h2so4_norm%',
        'residual_cpy_%': 'cu_seq_a_r_norm%',
        'grouped_copper_sulfides': 'grouped_copper_sulfides',
        'grouped_secondary_copper': 'grouped_secondary_copper',
        'grouped_acid_generating_sulfides': 'grouped_acid_generating_sulfides',
        'grouped_fe_oxides': 'grouped_fe_oxides'
    ...
    # Prepare inputs for LF model (using column data)
    lf_pred_inputs = np.zeros((len(out_df_unscaled), len(reactor_ore_features)))
    
    for i, reactor_feat in enumerate(reactor_ore_features):
        # Find corresponding column feature
        column_feat = None
        for col_f, react_f in feature_mapping.items():
            if react_f == reactor_feat and col_f in out_df_unscaled.columns:
                column_feat = col_f
                break
        
        if column_feat:
            lf_pred_inputs[:, i] = out_df_unscaled[column_feat].values
        elif reactor_feat in out_df_unscaled.columns:
            # Direct match (e.g., grouped features)
            lf_pred_inputs[:, i] = out_df_unscaled[reactor_feat].values
        else:
            print(f"  Warning: Could not map reactor feature '{reactor_feat}' to column data")
    
    # Scale and get LF predictions for column samples
    lf_pred_inputs_scaled = scaler_lf.transform(lf_pred_inputs)
    
    with torch.no_grad():
        nn_lf.eval()
        lf_predictions = nn_lf(torch.tensor(lf_pred_inputs_scaled, dtype=torch.float32)).numpy()
    
    print(f"Generated LF predictions for {len(lf_predictions)} column samples")
    print(f"  LF predictions shape: {lf_predictions.shape}")
    print(f"  LF predictions mean: a1={lf_predictions[:, 0].mean():.2f}, b1={lf_predictions[:, 1].mean():.4f}, "
          f"a2={lf_predictions[:, 2].mean():.2f}, b2={lf_predictions[:, 3].mean():.4f}")
    
    # Extract column-specific features
    if column_specific_features:
        hf_column_specific = out_df_unscaled[column_specific_features].values
    else:
        hf_column_specific = np.array([]).reshape(len(out_df_unscaled), 0)
    
    # *** Fuse HF inputs: [ore characteristics, LF predictions, column-specific] ***
    if hf_column_specific.shape[1] > 0:
        hf_aug_inputs = np.hstack((hf_ore_inputs, lf_predictions, hf_column_specific))
    else:
        hf_aug_inputs = np.hstack((hf_ore_inputs, lf_predictions))
    
    print(f"HF augmented input shape: {hf_aug_inputs.shape}")
    
    # Prepare target data: recovery curves, time, catalyst
    recovery_curves = []
    time_arrays = []
    catalyst_arrays = []
    transition_times = []
    ...
    for idx, row in out_df_unscaled.iterrows():
        # Parse arrays
        recovery = parse_array_str(row['cu_recovery_%']) if isinstance(row['cu_recovery_%'], str) else row['cu_recovery_%']
        time = parse_array_str(row['leach_duration_days']) if isinstance(row['leach_duration_days'], str) else row['leach_duration_days']
        catalyst = parse_array_str(row['cumulative_catalyst_addition_kg_t']) if isinstance(row['cumulative_catalyst_addition_kg_t'], str) else row['cumulative_catalyst_addition_kg_t']
        
        # Store as tensors
        recovery_curves.append(torch.tensor(recovery, dtype=torch.float32))
        time_arrays.append(torch.tensor(time, dtype=torch.float32))
        catalyst_arrays.append(torch.tensor(catalyst, dtype=torch.float32))
        transition_times.append(torch.tensor(row['transition_time'], dtype=torch.float32))
        
        # Check if catalyzed
        is_catalyzed = (row['catalyst_status'] != 'no_catalyst') if 'catalyst_status' in row.index else (np.any(np.array(catalyst) > 0))
        is_catalyzed_flags.append(is_catalyzed)
        
        # Store identifiers
        sample_ids.append(row['project_sample_id_reactormatch'])
        col_ids.append(row['project_col_id'])
    
    print(f"\nHF Training Data:")
    print(f"  Total samples: {len(recovery_curves)}")
    print(f"  Catalyzed: {sum(is_catalyzed_flags)}")
    print(f"  Control: {len(is_catalyzed_flags) - sum(is_catalyzed_flags)}")
    
    # Normalize HF inputs
    scaler_hf = StandardScaler()
    hf_aug_inputs_scaled = scaler_hf.fit_transform(hf_aug_inputs)
    hf_aug_inputs_tensor = torch.tensor(hf_aug_inputs_scaled, dtype=torch.float32)
    
    # Initialize HF model
    nn_hf = MFNN(
        input_dim=hf_aug_inputs_tensor.shape[1],
        hidden_dims=[128, 64],  # Match working model architecture
        output_dim=8,  # Always predict 8 parameters (a1-a4, b1-b4)
        dropout_rate=0.3
    )
    
    optimizer_hf = optim.Adam(nn_hf.parameters(), lr=0.001)
    
    # Train HF model end-to-end
    print("\nTraining HF model end-to-end...")
    best_hf_loss = float('inf')
    patience_hf = 100
    patience_counter_hf = 0
    ...
    for epoch in range(3000):
        ...
        # Process each sample
        for i in range(len(hf_aug_inputs_tensor)):
            # Predict parameters
            pred_params_raw = nn_hf(hf_aug_inputs_tensor[i:i+1])
            
            # *** For control samples, set catalyst parameters to NaN ***
            if not is_catalyzed_flags[i]:
                pred_params = pred_params_raw.clone()
                pred_params[:, 4:] = float('nan')
            else:
                pred_params = pred_params_raw
            
            # Generate recovery curve using predicted parameters
            pred_recovery = generate_two_phase_recovery(
                time=time_arrays[i].unsqueeze(0),
                catalyst=catalyst_arrays[i].unsqueeze(0),
                transition_time=transition_times[i].unsqueeze(0),
                params=pred_params
            )
            
            # Compute loss against actual recovery
            target_recovery = recovery_curves[i].unsqueeze(0)
            loss_sample = nn.MSELoss()(pred_recovery, target_recovery)
            total_loss += loss_sample
        
        # Average loss
        loss = total_loss / len(hf_aug_inputs_tensor)
        
        # Backward pass
        loss.backward()
        torch.nn.utils.clip_grad_norm_(nn_hf.parameters(), max_norm=1.0)
        optimizer_hf.step()
        
        if (epoch + 1) % 100 == 0:
            print(f"  HF Epoch {epoch+1}/3000, Loss: {loss.item():.6f}")
        
        # Early stopping
        if loss.item() < best_hf_loss:
            best_hf_loss = loss.item()
            patience_counter_hf = 0
            best_hf_state = nn_hf.state_dict().copy()
        else:
            patience_counter_hf += 1
            if patience_counter_hf >= patience_hf:
                print(f"  Early stopping at epoch {epoch+1}")
                break
    
    # Load best HF model
    nn_hf.load_state_dict(best_hf_state)
    print(f"HF model training complete. Best loss: {best_hf_loss:.6f}")
    
    # ===== Save Models =====
    torch.save({
        'nn_lf': nn_lf.state_dict(),
        'nn_hf': nn_hf.state_dict(),
        'scaler_lf': scaler_lf,
        'scaler_hf': scaler_hf,
        'reactor_ore_features': reactor_ore_features,
        'column_ore_features': column_ore_features,
        'column_specific_features': column_specific_features,
        'feature_mapping': feature_mapping,
        'best_lf_loss': best_lf_loss,
        'best_hf_loss': best_hf_loss
    }, os.path.join(save_dir, 'mf_2step_models.pt'))
    
    print(f"\nModels saved to: {os.path.join(save_dir, 'mf_2step_models.pt')}")
    
    # Return models and info
    return nn_lf, nn_hf, {
        'scaler_lf': scaler_lf,
        'scaler_hf': scaler_hf,
        'reactor_ore_features': reactor_ore_features,
...
class MFNN(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim=1, dropout_rate=0.3):
        ...
        for h in hidden_dims:
            ...
            layers.append(nn.LeakyReLU(0.1))  # Match working model
...
if __name__ == "__main__":
    ...
    # ===== Step 2: Train Multi-Fidelity Model =====
    print("\nStep 2: Training Multi-Fidelity Model")
    print("=" * 60)
    
    nn_lf, nn_hf, mf_info = train_mf_2step_end_to_end(
        reactor_df=reactor_filtered,  # For LF training
        out_df_unscaled=out_df_unscaled,  # For HF training
        config=CONFIG,
        save_dir=folder_path_save
    )
    
    print(f"\nTraining Complete!")
    print(f"LF Best Loss: {mf_info['best_lf_loss']:.4f}")
    print(f"HF Best Loss: {mf_info['best_hf_loss']:.4f}")

    # ===== Step 3: Make Predictions Using Multi-Fidelity Model =====
    print("\nStep 3: Making predictions with multi-fidelity model...")
    results = []
    predictions_list = []
    ...
    for idx, row in out_df_unscaled.iterrows():
        ...
        print(f"Processing {sample_id}_{col_id}...")
        
        try:
            # Get observed data
            recoveries_obs = parse_array_str(row['cu_recovery_%']) if isinstance(row['cu_recovery_%'], str) else row['cu_recovery_%']
            times_obs = parse_array_str(row['leach_duration_days']) if isinstance(row['leach_duration_days'], str) else row['leach_duration_days']
            catalyst_obs = parse_array_str(row['cumulative_catalyst_addition_kg_t']) if isinstance(row['cumulative_catalyst_addition_kg_t'], str) else row['cumulative_catalyst_addition_kg_t']
            transition_time = row['transition_time']
            
            # Check if catalyzed
            is_catalyzed = (row['catalyst_status'] != 'no_catalyst') if 'catalyst_status' in row.index else (np.any(catalyst_obs > 0))
            
            # ===== Step 3a: Get LF predictions for this sample =====
            # Extract ore characteristics
            lf_input_sample = np.zeros((1, len(mf_info['reactor_ore_features'])))
            
            for i, reactor_feat in enumerate(mf_info['reactor_ore_features']):
                # Find corresponding column feature
                column_feat = None
                for col_f, react_f in mf_info['feature_mapping'].items():
                    if react_f == reactor_feat and col_f in row.index:
                        column_feat = col_f
                        break
                
                if column_feat:
                    lf_input_sample[0, i] = row[column_feat]
                elif reactor_feat in row.index:
                    lf_input_sample[0, i] = row[reactor_feat]
            
            # *** Check for NaN in LF input ***
            if np.any(np.isnan(lf_input_sample)):
                print(f"  WARNING: NaN in LF input for {sample_id}_{col_id}")
                print(f"  LF input: {lf_input_sample}")
                continue
            
            # Scale and get LF prediction
            lf_input_scaled = mf_info['scaler_lf'].transform(lf_input_sample)
            with torch.no_grad():
                nn_lf.eval()
                lf_pred = nn_lf(torch.tensor(lf_input_scaled, dtype=torch.float32)).numpy()[0]
            
            # *** Check for NaN in LF prediction ***
            if np.any(np.isnan(lf_pred)):
                print(f"  WARNING: LF model predicted NaN for {sample_id}_{col_id}")
                print(f"  LF predictions: {lf_pred}")
                continue
            
            # ===== Step 3b: Prepare HF input =====
            # Extract ore characteristics
            hf_ore_input = row[mf_info['column_ore_features']].values.reshape(1, -1)
            
            # *** Check for NaN in ore input ***
            if np.any(np.isnan(hf_ore_input)):
                print(f"  WARNING: NaN in ore characteristics for {sample_id}_{col_id}")
                print(f"  Ore input: {hf_ore_input}")
                continue
            
            # Extract column-specific features
            if mf_info['column_specific_features']:
                hf_column_specific = row[mf_info['column_specific_features']].values.reshape(1, -1)
            else:
                hf_column_specific = np.array([]).reshape(1, 0)
            
            # *** Check for NaN in column-specific features ***
            if hf_column_specific.shape[1] > 0 and np.any(np.isnan(hf_column_specific)):
                print(f"  WARNING: NaN in column-specific features for {sample_id}_{col_id}")
                print(f"  Column-specific: {hf_column_specific}")
                continue
            
            # Fuse: [ore characteristics, LF predictions, column-specific]
            if hf_column_specific.shape[1] > 0:
                hf_aug_input = np.hstack((hf_ore_input, lf_pred.reshape(1, -1), hf_column_specific))
            else:
                hf_aug_input = np.hstack((hf_ore_input, lf_pred.reshape(1, -1)))
            
            # Scale and predict column parameters
            hf_aug_scaled = mf_info['scaler_hf'].transform(hf_aug_input)
            with torch.no_grad():
                nn_hf.eval()
                pred_params_all = nn_hf(torch.tensor(hf_aug_scaled, dtype=torch.float32)).numpy()[0]
            
            # *** Check for NaN in HF prediction ***
            if np.any(np.isnan(pred_params_all[:4])):  # Check base parameters
                print(f"  WARNING: HF model predicted NaN for {sample_id}_{col_id}")
                print(f"  HF predictions: {pred_params_all}")
                continue
            
            # Extract parameters
            a1_pred, b1_pred, a2_pred, b2_pred = pred_params_all[:4]
            
            # *** Validate base parameters ***
            if a1_pred <= 0 or b1_pred <= 0 or a2_pred <= 0 or b2_pred <= 0:
                print(f"  WARNING: Invalid base parameters for {sample_id}_{col_id}")
                print(f"  a1={a1_pred:.4f}, b1={b1_pred:.6f}, a2={a2_pred:.4f}, b2={b2_pred:.6f}")
                # Clip to valid range
                a1_pred = max(a1_pred, 0.1)
                b1_pred = max(b1_pred, 0.0001)
                a2_pred = max(a2_pred, 0.1)
                b2_pred = max(b2_pred, 0.0001)
            
            if is_catalyzed:
                # Use all 8 parameters for catalyzed samples
                a3_pred, b3_pred, a4_pred, b4_pred = pred_params_all[4:8]
                
                # *** Validate catalyst parameters ***
                if np.any(np.isnan([a3_pred, b3_pred, a4_pred, b4_pred])):
                    print(f"  WARNING: NaN in catalyst parameters for {sample_id}_{col_id}")
                    print(f"  Setting catalyst parameters to 0")
                    a3_pred, b3_pred, a4_pred, b4_pred = 0.0, 0.001, 0.0, 0.001
                
                params_tensor = torch.tensor([[a1_pred, b1_pred, a2_pred, b2_pred, 
                                            a3_pred, b3_pred, a4_pred, b4_pred]], dtype=torch.float32)
            else:
                # Use NaN for catalyst parameters in control samples
                a3_pred, b3_pred, a4_pred, b4_pred = float('nan'), float('nan'), float('nan'), float('nan')
                params_tensor = torch.tensor([[a1_pred, b1_pred, a2_pred, b2_pred, 
                                            float('nan'), float('nan'), float('nan'), float('nan')]], dtype=torch.float32)
            
            # Generate predicted recovery curve
            times_tensor = torch.tensor(times_obs, dtype=torch.float32)
            catalyst_tensor_sample = torch.tensor(catalyst_obs, dtype=torch.float32)
            transition_tensor = torch.tensor(transition_time, dtype=torch.float32)
            
            recoveries_pred_tensor = generate_two_phase_recovery(
                time=times_tensor.unsqueeze(0),
                catalyst=catalyst_tensor_sample.unsqueeze(0),
                transition_time=transition_tensor.unsqueeze(0),
                params=params_tensor
            )
            
            recoveries_pred = recoveries_pred_tensor.squeeze().numpy()
            
            # *** Check for NaN in predicted recovery ***
            if np.any(np.isnan(recoveries_pred)):
                print(f"  ERROR: NaN in predicted recovery for {sample_id}_{col_id}")
                print(f"  Parameters: a1={a1_pred:.4f}, b1={b1_pred:.6f}, a2={a2_pred:.4f}, b2={b2_pred:.6f}")
                if is_catalyzed:
                    print(f"  Catalyst params: a3={a3_pred:.4f}, b3={b3_pred:.6f}, a4={a4_pred:.4f}, b4={b4_pred:.6f}")
                print(f"  Predicted recovery (first 5): {recoveries_pred[:5]}")
                continue
            
            # *** Check for Inf in predicted recovery ***
            if np.any(np.isinf(recoveries_pred)):
                print(f"  ERROR: Inf in predicted recovery for {sample_id}_{col_id}")
                continue
            
            # Compute metrics
            rmse = root_mean_squared_error(recoveries_obs, recoveries_pred)
            bias = np.mean(recoveries_pred - recoveries_obs)
            r2 = r2_score(recoveries_obs, recoveries_pred)
            ...
            results.append({
                ...
                'bias': bias,
                'r2': r2,
                'catalyzed': is_catalyzed,
                'a1_pred': a1_pred,
                'b1_pred': b1_pred,
                'a2_pred': a2_pred,
                'b2_pred': b2_pred,
                'a3_pred': a3_pred if is_catalyzed else None,
                'b3_pred': b3_pred if is_catalyzed else None,
                'a4_pred': a4_pred if is_catalyzed else None,
                'b4_pred': b4_pred if is_catalyzed else None,
                'lf_a1': lf_pred[0],  # Store LF predictions for analysis
                'lf_b1': lf_pred[1],
                'lf_a2': lf_pred[2],
                'lf_b2': lf_pred[3],
                'confidence_level': 100 * (1 - rmse / 100),
                'uncertainty': rmse / 2
            })
            ...
            predictions_list.append({
                ...
                'col_id': col_id,
                'times': times_obs,
                'recoveries_obs': recoveries_obs,
                'recoveries_pred': recoveries_pred
            })
            
            print(f"  {sample_id}_{col_id}: RMSE={rmse:.2f}, R²={r2:.3f}, Bias={bias:.2f}")
        
        except Exception as e:
            print(f"  ERROR processing {sample_id}_{col_id}: {str(e)}")
            import traceback
            traceback.print_exc()
            continue
    ...
    if len(results) == 0:
        ...
        print("Please check the warnings above to diagnose the issue.")
    else:
        # Save results
        results_df = pd.DataFrame(results)
        results_df.to_csv(os.path.join(folder_path_save, 'mf_prediction_results.csv'), index=False)

        print(f"\nPrediction Results Summary:")
        print(f"  Successful predictions: {len(results)}/{len(out_df_unscaled)}")
        print(f"  Average RMSE: {results_df['rmse'].mean():.4f}")
        print(f"  Average R²: {results_df['r2'].mean():.4f}")
        print(f"  Average Bias: {results_df['bias'].mean():.4f}")

        # ===== Step 4: Generate Plots =====
        print("\nStep 4: Generating plots...")
        os.makedirs(os.path.join(folder_path_save, 'plots'), exist_ok=True)
        
        plot_reactor_scaling_diagnostics(results_df, folder_path_save)
        plot_reactor_scaling_diagnostics_uncertainty(results_df, folder_path_save)
        plot_diagnostics_control_catalyzed(results_df, folder_path_save)
        
        print("\nMulti-Fidelity Model Training and Prediction Complete!")
        print(f"Results saved to: {folder_path_save}")

    # ===== Step 5: Create Extended Predictions (0 to 2500 days) =====
    print("\nStep 5: Creating extended predictions (0-2500 days)...")
    ...
    for idx, row in out_df_unscaled.iterrows():
        ...
        try:
            ...
            # Create extended time array (0 to 2500 days)
            new_times = np.linspace(0, 2500, 100)
            
            # Interpolate catalyst values for extended times
            catalyst_extended = np.interp(new_times, times_obs, catalyst_obs)
            
            # ===== Get LF predictions for this sample =====
            # *** CORRECTED: Use correct key names ***
            reactor_features = mf_info['reactor_ore_features']  # CORRECTED
            column_features = mf_info['column_ore_features']    # CORRECTED
            
            lf_input_sample = np.zeros((1, len(reactor_features)))
            
            for i, reactor_feat in enumerate(reactor_features):
                # Find corresponding column feature
                column_feat = None
                for col_f, react_f in mf_info['feature_mapping'].items():
                    if react_f == reactor_feat and col_f in row.index:
            ...
            # Check for NaN
            if np.any(np.isnan(lf_input_sample)):
                print(f"  WARNING: NaN in LF input for extended prediction {sample_id}_{col_id}")
                continue
            ...
            if np.any(np.isnan(lf_pred)):
                print(f"  WARNING: NaN in LF prediction for extended prediction {sample_id}_{col_id}")
                continue
            
            # ===== Prepare HF input =====
            hf_ore_input = row[column_features].values.reshape(1, -1)
            
            if np.any(np.isnan(hf_ore_input)):
                print(f"  WARNING: NaN in ore input for extended prediction {sample_id}_{col_id}")
                continue
            ...
            if np.any(np.isnan(pred_params_all[:4])):
                print(f"  WARNING: NaN in HF prediction for extended prediction {sample_id}_{col_id}")
                continue
            ...
            # Generate extended recovery curve
            times_tensor = torch.tensor(new_times, dtype=torch.float32)
            catalyst_tensor_extended = torch.tensor(catalyst_extended, dtype=torch.float32)
            transition_tensor = torch.tensor(transition_time, dtype=torch.float32)
            
            recoveries_extended_tensor = generate_two_phase_recovery(
                time=times_tensor.unsqueeze(0),
                catalyst=catalyst_tensor_extended.unsqueeze(0),
                transition_time=transition_tensor.unsqueeze(0),
                params=params_tensor
            )
            
            recoveries_extended = recoveries_extended_tensor.squeeze().numpy()
            
            # Check for NaN
            if np.any(np.isnan(recoveries_extended)):
                print(f"  WARNING: NaN in extended recovery for {sample_id}_{col_id}")
                continue
            
            # Store extended prediction
            extended_predictions.append({
                'sample_id': sample_id,
                'sample_col_id': col_id,
                'times': new_times,
                'predicted': recoveries_extended,
                'uncertainty': np.ones_like(recoveries_extended) * 2.0,  # Approximate uncertainty
                'catalyzed': is_catalyzed
            })
            
        except Exception as e:
            print(f"  ERROR in extended prediction for {sample_id}_{col_id}: {str(e)}")
            continue

    print(f"Generated {len(extended_predictions)} extended predictions")

    # Save extended predictions
    if len(extended_predictions) > 0:
        # Create a DataFrame for extended predictions
        extended_df_data = []
        for pred in extended_predictions:
            for i, (t, rec, unc) in enumerate(zip(pred['times'], pred['predicted'], pred['uncertainty'])):
                extended_df_data.append({
                    'sample_id': pred['sample_id'],
                    'sample_col_id': pred['sample_col_id'],
                    'time': t,
                    'predicted_recovery': rec,
                    'uncertainty': unc,
                    'catalyzed': pred['catalyzed']
                })
        
        extended_df = pd.DataFrame(extended_df_data)
        extended_df.to_csv(os.path.join(folder_path_save, 'extended_predictions.csv'), index=False)
        print(f"Extended predictions saved to: {os.path.join(folder_path_save, 'extended_predictions.csv')}")

    # Plot extended predictions (similar to ensemble plots)
    for pred in extended_predictions[:9]:  # Plot first 9
        plt.figure(figsize=(10, 6))
        plt.plot(pred['times'], pred['predicted'], 'b-', label='Predicted', linewidth=2)
        plt.fill_between(pred['times'],
                        pred['predicted'] - pred['uncertainty'],
                        pred['predicted'] + pred['uncertainty'],
                        alpha=0.3, label='Uncertainty')
        plt.xlabel('Days', fontsize=12)
        plt.ylabel('Cu Recovery %', fontsize=12)
        plt.title(f"Extended Prediction: {pred['sample_id']}\n{pred['sample_col_id']}", fontsize=14)
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.savefig(os.path.join(folder_path_save, 'plots', f'extended_pred_{pred["sample_id"]}_{pred["sample_col_id"]}.png'), dpi=300, bbox_inches='tight')
        plt.close()

    # ===== Final Summary =====
    print("\n" + "=" * 60)
    print("Multi-Fidelity Model Training and Prediction Complete!")
    print("=" * 60)
    print(f"Results saved to: {folder_path_save}")
    ...
    torch.save({
        ...
        'results': results_df,
        'config': CONFIG
    }, os.path.join(folder_path_save, 'MultiFidelityModel.pt'))

    print("\nAll results and plots have been saved.")
    print(f"Model saved as: MultiFidelityModel.pt")
