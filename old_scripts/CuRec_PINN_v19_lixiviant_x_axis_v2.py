...
def fit_two_exp_params(t, y):
    ...
    try:
        popt, _ = curve_fit(_two_exp_model, t, y, p0=p0, bounds=bounds, maxfev=50000)
...
def fit_one_exp_params_additive_with_ce(t, y, ce, w=None):
    ...
    try:
        ...
        popt, _ = curve_fit(func, t, y, p0=p0, bounds=bounds, sigma=sigma, absolute_sigma=False, maxfev=50000)
...
def fit_two_exp_params_additive_with_ce(t, y, ce, w=None):
    ...
    try:
        ...
        popt, _ = curve_fit(func, t, y, p0=p0, bounds=bounds,
                            sigma=sigma, absolute_sigma=False, maxfev=50000)
        return tuple(map(float, popt))
...
def compute_param_limits_from_actual(y_list, time_list, catalyst_list, tt_tensor, sample_ids, sample_col_ids, save_dir):
    """
    Two-pass approach per sample_id:
      1) Fit base (a1,b1,a2,b2) on the Control row (or pre-TT if no Control row).
      2) On the Catalyzed row, fit additive y_add = y - base(t) on t>=tt to get (a3,b3,a4,b4).
    Then enforce:
      - all params > 0
      - a1 + a2 <= 75
      - a1 + a2 + a3 + a4 <= 90
    Saves constrained per-row fits and returns strict min/max per parameter.
    """
    ...
    # -------- Pass 2: per-row outputs, fit catalyst only on Catalyzed row --------
    rows = []
    for i in range(len(y_list)):
        sid = sample_ids[i]
        colid = sample_col_ids[i]
        ...
        if is_catalyzed_row and base_fit is not None and np.isfinite(tt):
            ...
            if post_mask.sum() >= 4:
                ...
                t_shift = t[post_mask] - tt
                ce_post = np.asarray(cat[post_mask], dtype=float)
                ce_power = float(CONFIG.get('catalyst_effect_power', 1.0))
                ce_post = (ce_post / (ce_post + 1.0)) ** ce_power
                # Weight more where catalyst is active
                w_pow = float(CONFIG.get('catalyst_fit_weight_power', 2.0))
                w = np.power(ce_post / (np.max(ce_post) + 1e-8), w_pow)
                sel = w > float(CONFIG.get('catalyst_fit_min_weight', 0.05))
                if sel.sum() >= 4:
                ...
                if np.nanmax(ce_fit) > 1e-8 and np.nanmax(y_fit) > 1e-6:
                    ...
                    if tmp is not None:
                        ...
                        # Positivity and cap total at 99 by scaling a3,a4 only
                        ...
                        if total > 99.0 and cat_sum > 0:
                            headroom = max(0.0, 99.0 - base_sum)
                            s = headroom / max(cat_sum, 1e-6)
                            a3 *= s; a4 *= s
                    # Fallback: single exponential if two-exp is poor
                    else:
                        tmp1 = fit_one_exp_params_additive_with_ce(t_fit, y_fit, ce_fit, w=w_fit)
                        if tmp1 is not None:
                            a_single, b_single = tmp1
                            a3, b3 = max(a_single, eps_a), max(b_single, eps_b)
                            a4, b4 = eps_a, eps_b
                            base_sum = a1 + a2
                            cat_sum = a3 + a4
                            total = base_sum + cat_sum
        ...
        # Save constrained row + sums for debugging
        ...
        # Compute RMSE and R² for the constrained fit on this row
        rmse_fit = np.nan
        r2_fit = np.nan
        try:
            # Base prediction (control curve)
            ...
            # Add catalyzed component if present
            is_catalyzed_row = (str(colid) == 'Catalyzed') or np.any(cat > 0)
    ...
    # Save a simple fit plot per row (time vs actual vs fitted)
    for row in rows:
        sid = row['project_sample_id_reactormatch']
        colid = row['project_col_id']
        # recover original arrays for this row
        ...
        # Plot (sorted by time for clean lines)
        order = np.argsort(t)
    ...
    def forward(self, x, catalyst, sample_ids=None):
        ...
        if sample_ids is not None:
            ...
            for unique_id in unique_sample_ids:
                ...
                if sample_indices:
                    ...
                    # Ensure total asymptote doesn't exceed 75
        ...
        if has_catalyst.any():
            ...
            if idx.numel() > 0:
                ...
                # Ensure total asymptote doesn't exceed 90
...
def generate_two_phase_recovery(time, catalyst, transition_time, params):
    ...
    ce_power = float(CONFIG.get('catalyst_effect_power', 1.0))
    catalyst_effect = (catalyst / (catalyst + 1)).pow(ce_power)
