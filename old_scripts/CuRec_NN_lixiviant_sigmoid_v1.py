...
CONFIG = {
    ...
    'cat_effect_power': 1.0,    # ce^power, ce = cat/(cat+1)
    'cat_rate_gain_b3': 1.0,    # rate multiplier = 1 + gain * ce^power
    'cat_rate_gain_b4': 0.5,    # tune per dataset; set 0.0 to disable
}
...
def project_params_to_caps(params: torch.Tensor,
    ...
    Input / Output:
      params: [B, 8] tensor -> (a1,b1,a2,b2,a3,b3,a4,b4)
      returns projected tensor with same shape.
    ...
    if rcfg.get('enable', True):
        ...
        # b3/b1 <= max_b3_over_b1 (scale b3 down only)
        if rcfg.get('hard_avoid_b3_over_b1', True):
            safe_b1 = b1.clamp(min=1e-8)
            ratio_b3 = b3 / safe_b1
            over = ratio_b3 > max_b3_over_b1
            b3 = torch.where(over, max_b3_over_b1 * safe_b1, b3)

        # a1/b1 >= min_a1_over_b1 (increase a1 if possible; else reduce b1)
        safe_b1 = b1.clamp(min=1e-8)
        ratio_a1 = (a1 / safe_b1)
        under = ratio_a1 < min_a1_over_b1
        if under.any():
            # Try increasing a1 within remaining amplitude room
            desired_a1 = min_a1_over_b1 * safe_b1
            # Do not violate amplitude base cap: a1 + a2 <= base_cap already enforced earlier
            room = (base_cap - a2).clamp(min=0.0)
            a1_new = torch.minimum(desired_a1, room)
            a1 = torch.where(under, a1_new, a1)
            # If still under (cannot raise a1), lower b1 to raise ratio
            ratio_after = a1 / b1.clamp(min=1e-8)
            still_under = ratio_after < min_a1_over_b1
            if still_under.any():
                b1 = torch.where(still_under, (a1 / min_a1_over_b1).clamp(min=1e-8), b1)
...
def compute_ratio_penalty(params: torch.Tensor) -> torch.Tensor:
    ...
    pw = rcfg.get('potentiate_weight', 0.1)
    aw = rcfg.get('avoid_weight', 0.1)

    a1, b1, a2, b2, a3, b3, a4, b4 = [params[:, i] for i in range(8)]
    safe_b1 = b1.clamp(min=1e-8)
    safe_b3 = torch.nan_to_num(b3, nan=1e-8).clamp(min=1e-8)

    # Potentiate terms (ReLU(target - ratio))
    pot_a1_b1 = torch.relu(rcfg.get('target_a1_over_b1', 15.0) - a1 / safe_b1)
    pot_a1_b3 = torch.relu(rcfg.get('target_a1_over_b3', 50.0) - a1 / safe_b3)
    pot_a2_b1 = torch.relu(rcfg.get('target_a2_over_b1', 20.0) - a2 / safe_b1)

    # Avoid terms
    avoid_b3_b1 = torch.relu((b3 / safe_b1) - rcfg.get('max_b3_over_b1', 0.1))
    avoid_low_a1_b1 = torch.relu(rcfg.get('min_a1_over_b1', 10.0) - a1 / safe_b1)

    penalty = pw * (pot_a1_b1 + pot_a1_b3 + pot_a2_b1) + aw * (avoid_b3_b1 + avoid_low_a1_b1)
    return penalty.mean()
...
def generate_two_phase_recovery_exp(time, catalyst, transition_time, params):
    ...
    if has_catalyst and params.shape[1] > 4 and torch.any(~torch.isnan(params[:, 4:])):
        ...
        if has_catalyst_points.any():
            ...
            # NEW: dose-dependent rate multipliers for b3 and b4
            gain_b3 = float(CONFIG.get('cat_rate_gain_b3', 0.0))
            gain_b4 = float(CONFIG.get('cat_rate_gain_b4', 0.0))
            # shape broadcast: [B,1] * [1,T] * [1,T] -> [B,T]
...
def generate_two_phase_recovery_sigmoid(time, catalyst, transition_time, params):
    """
    Sigmoid-based version with the same interface:
    - Control: logistic terms anchored at 0, capped by CONFIG.
    - Catalyzed: smooth additive logistic terms that start at 0 at transition time,
      scaled by catalyst_effect, so catalyzed >= control and curves touch at transition.
    """
    time = time.to(params.device)
    catalyst = catalyst.to(params.device)
    ...
    if has_catalyst and params.shape[1] > 4 and torch.any(~torch.isnan(params[:, 4:])):
        ...
        recovery = recovery_control + additional  # smooth, non-negative add-on
...
# Default to the sigmoid version for modelling; the exponential version remains available for comparison.
def generate_two_phase_recovery(time, catalyst, transition_time, params):
    return generate_two_phase_recovery_sigmoid(time, catalyst, transition_time, params)
...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    for epoch in range(epochs):
        ...
        for j in range(len(X_train)):
            ...
            ratio_penalty = compute_ratio_penalty(params)
            loss = mse_loss_value + physics_loss + monotonic_penalty + ratio_penalty
        ...
        with torch.no_grad():
            for j in range(val_samples):
                ...
                ratio_penalty_val = compute_ratio_penalty(params)
                weighted_loss_val = weighted_loss_val + ratio_penalty_val
