...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_free_model"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_free_model")
...
class PairCurveNet(nn.Module):
    def __init__(
        ...
        # Free monotonic curve cores. These no longer parameterize a biexponential shape.
        self.ctrl_core_head = nn.Linear(hidden_dim, 10)
        self.cat_core_head = nn.Linear(hidden_dim, 8)
        ...
        self.primary_focus_head = nn.Linear(hidden_dim, 1)
        self.chem_mix_head = nn.Linear(hidden_dim, 1)
        self.fast_inventory_head = nn.Linear(hidden_dim, 1)
        self.secondary_inventory_head = nn.Linear(hidden_dim, 1)
        self.oxide_inventory_head = nn.Linear(hidden_dim, 1)
        self.carb_buffer_head = nn.Linear(hidden_dim, 1)
        self.buffer_decay_head = nn.Linear(hidden_dim, 1)
        self.gangue_drag_head = nn.Linear(hidden_dim, 1)
        ...
        self.catalyst_reactivity_head = nn.Linear(hidden_dim, 1)
        self.sep_abs_head = nn.Linear(hidden_dim, 4)
        self.sep_norm_head = nn.Linear(hidden_dim, 4)
        self.ctrl_uplift_head = nn.Linear(hidden_dim, 3)
        ...
        self.residual_cpy_idx = (
            STATIC_PREDICTOR_COLUMNS.index("residual_cpy_%")
            if "residual_cpy_%" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.total_cu_idx = (
            STATIC_PREDICTOR_COLUMNS.index("cu_%")
            if "cu_%" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.acid_soluble_idx = (
            STATIC_PREDICTOR_COLUMNS.index("acid_soluble_%")
            if "acid_soluble_%" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.cyanide_soluble_idx = (
            STATIC_PREDICTOR_COLUMNS.index("cyanide_soluble_%")
            if "cyanide_soluble_%" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.primary_sulfide_idx = (
            STATIC_PREDICTOR_COLUMNS.index("copper_primary_sulfides_equivalent")
            if "copper_primary_sulfides_equivalent" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.secondary_sulfide_idx = (
            STATIC_PREDICTOR_COLUMNS.index("copper_secondary_sulfides_equivalent")
            if "copper_secondary_sulfides_equivalent" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.oxides_idx = (
            STATIC_PREDICTOR_COLUMNS.index("copper_oxides_equivalent")
            if "copper_oxides_equivalent" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.cu_fe_ratio_idx = (
            STATIC_PREDICTOR_COLUMNS.index("cu:fe")
            if "cu:fe" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.fe_cu_ratio_idx = (
            STATIC_PREDICTOR_COLUMNS.index("fe:cu")
            if "fe:cu" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.carb_idx = (
            STATIC_PREDICTOR_COLUMNS.index("grouped_carbonates")
            if "grouped_carbonates" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.gangue_silicates_idx = (
            STATIC_PREDICTOR_COLUMNS.index("grouped_gangue_silicates")
            if "grouped_gangue_silicates" in STATIC_PREDICTOR_COLUMNS
            else None
        )
        self.material_size_idx = (
            STATIC_PREDICTOR_COLUMNS.index("material_size_p80_in")
            if "material_size_p80_in" in STATIC_PREDICTOR_COLUMNS
            else None
        )
    ...
    def predict_params(
        ...
        total_cu_feat = self._feature_column(x_static, self.total_cu_idx)
        residual_feat = self._feature_column(x_static, self.residual_cpy_idx)
        acid_feat = self._feature_column(x_static, self.acid_soluble_idx)
        cyanide_feat = self._feature_column(x_static, self.cyanide_soluble_idx)
        primary_feat = self._feature_column(x_static, self.primary_sulfide_idx)
        secondary_feat = self._feature_column(x_static, self.secondary_sulfide_idx)
        oxides_feat = self._feature_column(x_static, self.oxides_idx)
        cu_fe_ratio_feat = self._feature_column(x_static, self.cu_fe_ratio_idx)
        fe_cu_ratio_feat = self._feature_column(x_static, self.fe_cu_ratio_idx)
        carbonate_feat = self._feature_column(x_static, self.carb_idx)
        gangue_silicates_feat = self._feature_column(x_static, self.gangue_silicates_idx)
        material_size_feat = self._feature_column(x_static, self.material_size_idx)

        chem_raw = self.chem_mix_head(h)
        if self.chem_direct_head is not None:
            chem_raw = chem_raw + 0.20 * self.chem_direct_head(x_static[:, self.chem_idx])

        primary_focus = torch.sigmoid(
            self.primary_focus_head(h)
            + 0.65 * residual_feat
            + 0.95 * primary_feat
            - 0.25 * secondary_feat
            + 0.15 * fe_cu_ratio_feat
            - 0.05 * cu_fe_ratio_feat
            + 0.10 * material_size_feat
        )
        ...
        chem_interaction = 0.50 + torch.sigmoid(
            ...
            + 0.10 * oxides_feat
            + 0.08 * secondary_feat
            + 0.05 * fe_cu_ratio_feat
            + 0.05 * gangue_silicates_feat
            - 0.10 * carbonate_feat
            - 0.05 * cyanide_feat
        )
        ...
        secondary_inventory = torch.sigmoid(
            self.secondary_inventory_head(h)
            + 0.55 * secondary_feat
            + 0.15 * cyanide_feat
            + 0.10 * oxides_feat
            - 0.10 * primary_feat
        ...
        acid_buffer_strength = 0.85 * torch.sigmoid(
            self.carb_buffer_head(h)
            + 0.75 * carbonate_feat
            + 0.20 * gangue_silicates_feat
            - 0.15 * acid_feat
        )
        acid_buffer_decay = 0.20 + 1.40 * torch.sigmoid(
            self.buffer_decay_head(h)
            + 0.25 * acid_feat
            + 0.15 * oxides_feat
            + 0.10 * total_cu_feat
        )
        diffusion_drag_strength = 0.75 * torch.sigmoid(
            self.gangue_drag_head(h)
            + 0.45 * gangue_silicates_feat
            + 0.30 * material_size_feat
            + 0.15 * primary_feat
        )
        ferric_synergy = 0.50 + torch.sigmoid(
            self.ferric_synergy_head(h)
            + 0.35 * fe_cu_ratio_feat
            - 0.10 * cu_fe_ratio_feat
            + 0.10 * primary_feat
        )
        surface_refresh = 0.50 + torch.sigmoid(
            self.surface_refresh_head(h)
            - 0.35 * material_size_feat
            + 0.20 * oxides_feat
            + 0.15 * secondary_feat
            - 0.05 * gangue_silicates_feat
        )
        ...
        depassivation_strength = 1.35 * torch.sigmoid(
            ...
            + 0.20 * acid_feat
            + 0.20 * ferric_synergy
            + 0.15 * surface_refresh
        )
        catalyst_reactivity = 0.50 + torch.sigmoid(
            self.catalyst_reactivity_head(h)
            + 0.20 * chem_raw
            + 0.15 * oxides_feat
            + 0.10 * secondary_feat
        )
        sep_abs_capacity = 2.0 + 35.0 * torch.sigmoid(
            self.sep_abs_head(h)[:, 0:1]
            + 0.95 * primary_focus
            + 0.25 * chem_interaction
            + 0.15 * ferric_synergy
        )
        sep_abs_scale = 0.10 + 3.00 * torch.sigmoid(
            self.sep_abs_head(h)[:, 1:2]
            + 0.80 * primary_focus
            + 0.20 * catalyst_reactivity
            + 0.10 * surface_refresh
        )
        sep_abs_mid = 0.05 + 0.85 * torch.sigmoid(
            self.sep_abs_head(h)[:, 2:3]
            + 0.55 * primary_focus
            - 0.20 * fast_leach_inventory
            + 0.10 * diffusion_drag_strength
        )
        sep_abs_temp = 0.05 + 0.45 * torch.sigmoid(
            self.sep_abs_head(h)[:, 3:4]
            + 0.15 * chem_interaction
            + 0.10 * diffusion_drag_strength
        )
        sep_norm_scale = 0.05 + 1.20 * torch.sigmoid(
            self.sep_norm_head(h)[:, 0:1]
            + 0.75 * primary_focus
            + 0.20 * chem_interaction
        )
        sep_norm_progress_gain = 0.10 + 0.90 * torch.sigmoid(
            self.sep_norm_head(h)[:, 1:2]
            + 0.35 * ferric_synergy
            - 0.15 * carbonate_feat
        )
        sep_norm_depassivation_gain = 0.10 + 0.90 * torch.sigmoid(
            self.sep_norm_head(h)[:, 2:3]
            + 0.25 * surface_refresh
            + 0.20 * catalyst_reactivity
        )
        sep_norm_cap = 0.20 + 3.50 * torch.sigmoid(
            self.sep_norm_head(h)[:, 3:4]
            + 0.70 * primary_focus
            + 0.15 * chem_interaction
        )
        ctrl_uplift_progress_mix = 0.05 + 0.90 * torch.sigmoid(
            self.ctrl_uplift_head(h)[:, 0:1]
            + 0.50 * primary_focus
            + 0.10 * chem_interaction
        )
        ctrl_uplift_increment_mix = 0.05 + 0.90 * torch.sigmoid(
            self.ctrl_uplift_head(h)[:, 1:2]
            + 0.20 * catalyst_reactivity
            + 0.15 * surface_refresh
        )
        ctrl_uplift_sharpness = 0.50 + 3.00 * torch.sigmoid(
            self.ctrl_uplift_head(h)[:, 2:3]
            + 0.40 * primary_focus
            + 0.15 * ferric_synergy
        )

        ctrl_raw = self.ctrl_core_head(h)
        ctrl_core = torch.cat(
            [
                0.05 + F.softplus(ctrl_raw[:, 0:1] + 0.60 * fast_leach_inventory + 0.10 * oxide_inventory),
                0.05 + F.softplus(ctrl_raw[:, 1:2] + 0.45 * secondary_inventory + 0.15 * chem_interaction),
                0.05 + F.softplus(ctrl_raw[:, 2:3] + 0.95 * primary_focus + 0.15 * chem_interaction),
                0.02 + F.softplus(ctrl_raw[:, 3:4] + 0.25 * primary_focus + 0.15 * diffusion_drag_strength),
                0.20 + F.softplus(ctrl_raw[:, 4:5] + 0.25 * oxide_inventory + 0.15 * fast_leach_inventory),
                0.06 + F.softplus(ctrl_raw[:, 5:6] + 0.15 * secondary_inventory),
                0.02 + F.softplus(ctrl_raw[:, 6:7] - 0.10 * primary_focus + 0.15 * chem_interaction),
                0.004 + F.softplus(ctrl_raw[:, 7:8] + 0.10 * diffusion_drag_strength),
                0.01 + F.softplus(ctrl_raw[:, 8:9] + 0.10 * total_cu_feat),
                25.0 + 70.0 * torch.sigmoid(ctrl_raw[:, 9:10] + 0.15 * total_cu_feat + 0.10 * chem_interaction),
            ],
            dim=1,
        )

        cat_raw = self.cat_core_head(h)
        cat_core = torch.cat(
            [
                0.05 + F.softplus(cat_raw[:, 0:1] + 1.05 * primary_focus + 0.25 * chem_interaction),
                0.05 + F.softplus(cat_raw[:, 1:2] + 0.30 * secondary_inventory + 0.20 * oxide_inventory),
                0.02 + F.softplus(cat_raw[:, 2:3] + 0.65 * primary_focus + 0.20 * diffusion_drag_strength),
                0.04 + F.softplus(cat_raw[:, 3:4] + 0.20 * catalyst_reactivity + 0.10 * surface_refresh),
                0.006 + F.softplus(cat_raw[:, 4:5] + 0.15 * surface_refresh),
                0.10 + 2.50 * torch.sigmoid(cat_raw[:, 5:6] + 0.60 * primary_focus + 0.35 * ferric_synergy),
                torch.sigmoid(cat_raw[:, 6:7] + 0.25 * secondary_inventory - 0.10 * diffusion_drag_strength),
                3.0 + 55.0 * torch.sigmoid(cat_raw[:, 7:8] + 0.75 * primary_focus + 0.25 * chem_interaction),
            ],
            dim=1,
        )
    ...
    def curves_given_params(
        ...
        ctrl_capacity = torch.clamp(p_ctrl[:, 9:10], min=5.0, max=100.0)
        ...
        for i in range(t.shape[1]):
            ...
            remaining_ctrl = torch.clamp(
                (ctrl_capacity - prev_ctrl) / torch.clamp(ctrl_capacity, min=1.0),
                min=self.min_remaining_ore_factor,
                max=1.0,
            )
            # Free model under accumulated-sum regime:
            # each incremental recovery term is forced nonnegative before accumulation.
            ctrl_inc = torch.clamp(dt * F.softplus(ctrl_drive) * remaining_ctrl, min=0.0)
            ...
            remaining_uplift = torch.clamp(
                (uplift_capacity - existing_uplift) / torch.clamp(uplift_capacity, min=1.0),
                min=0.0,
                max=1.0,
            )
            extra_inc = torch.clamp(dt * F.softplus(uplift_drive) * remaining_uplift, min=0.0)

            next_cat = torch.clamp(prev_cat + ctrl_inc + extra_inc, min=0.0, max=100.0)
