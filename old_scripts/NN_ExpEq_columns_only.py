...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_ExpEq_columns_only"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only")
...
class PairCurveNet(nn.Module):
    ...
    def predict_params(
        ...
        return p_ctrl, p_cat, tau_days, temp_days, kappa, aging_strength
    ...
    def curves_given_params(
        ...
        t = t_days.view(1, -1)
        y_ctrl = self._double_exp_curve_torch(p_ctrl, t_days)
        y_cat_cap = self._double_exp_curve_torch(p_cat, t_days)
        ...
        separation_factor = torch.clamp(
            delay_factor * catalyst_factor * remaining_ore_factor * late_tau_factor,
            min=0.0,
            max=1.0,
        )
        ...
        y_ctrl = y_ctrl.squeeze(0)
        y_cat = y_cat.squeeze(0)
        return y_ctrl, y_cat
...
def predict_new_sample_member(
    ...
    with torch.no_grad():
        ...
        p_ctrl, p_cat, tau, temp, kappa, aging_strength = model.predict_params(x)
        pred_ctrl, pred_cat = model.curves_given_params(p_ctrl, p_cat, t, c, tau, temp, kappa, aging_strength)
...
def pair_training_loss(
    ...
    pred_ctrl, _ = model.curves_given_params(p_ctrl, p_cat, ctrl_t, ctrl_c, tau, temp, kappa, aging_strength)
    pred_ctrl_cat_time, pred_cat = model.curves_given_params(
        p_ctrl,
        p_cat,
        cat_t,
        cat_c,
