...
def compute_sample_weights_per_time(time_steps, device):
    ...
    full_weight_curve = torch.linspace(3.0, 1.0, num_max_days_standard, device=device) # try to even out the influence of early time points
...
def compute_sample_weights(time_steps, device):
    ...
    for time_array in time_steps:
        if not isinstance(time_array, torch.Tensor):
            time_array = torch.tensor(time_array, dtype=torch.float32, device=device)
        full_weight_curve = torch.linspace(3.0, 1.0, steps=time_array.shape[-1], device=device)
...
def train_single_fold_reactor_scaling(fold_data_with_params):
    ...
    # Diagnostic plots
    if do_plots and train_y_true is not None and all_y_true is not None and len(train_y_true) > 0 and len(all_y_true) > 0:
        # Skip stacking across varying lengths; handle per-sample if needed in future
        pass
