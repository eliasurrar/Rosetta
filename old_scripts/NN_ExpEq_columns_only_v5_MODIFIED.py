...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_ExpEq_columns_only_v5"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v5")
...
CONFIG = {
    ...
    "fit_plateau_target_day": 2500.0 + 1000.0,
...
def compute_remaining_ore_factor_chemistry_based(
    ...
    """
    Compute remaining ore factor based on ore chemistry.
    ...
    Returns:
        remaining_ore_factor: Fraction of primary sulfides still available (0-1)
    """
    # Avoid division by zero
    cu_safe = torch.clamp(cu_percent, min=1e-6)
    
    # Maximum leachable fraction (oxides + primary sulfides)
    max_leachable = (copper_oxides_equiv + copper_primary_sulfides_equiv) / cu_safe
    max_leachable = torch.clamp(max_leachable, min=0.0, max=1.0)
    
    # How much copper has been extracted in the control curve?
    # (as a fraction of the maximum leachable)
    extracted_fraction = torch.clamp(y_ctrl / 100.0, min=0.0, max=1.0)
    
    # How much of the primary sulfides has been extracted?
    # (assuming oxides are extracted first, then primary sulfides)
    oxide_fraction = torch.clamp(copper_oxides_equiv / cu_safe, min=0.0, max=1.0)
    
    # If we haven't extracted all oxides yet, no primary sulfides have been touched
    primary_extraction_fraction = torch.clamp(
        (extracted_fraction - oxide_fraction) / torch.clamp(
            copper_primary_sulfides_equiv / cu_safe, min=1e-6
        ),
        min=0.0,
        max=1.0,
    )
    
    # Remaining primary sulfides as a fraction of total primary sulfides
    remaining_fraction = 1.0 - primary_extraction_fraction
    
    # Apply minimum floor to allow some residual uplift even at high recovery
    ...
    def curves_given_params(
        ...
        if cu_percent is not None and cu_primary is not None and cu_oxides is not None:
            remaining_ore_factor = self._compute_remaining_ore_factor_chemistry_based(
                y_ctrl, cu_percent, cu_primary, cu_oxides, min_floor=self.min_remaining_ore_factor
            )
        else:
