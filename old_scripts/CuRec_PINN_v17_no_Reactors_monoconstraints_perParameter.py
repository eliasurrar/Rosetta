...
CONFIG = {
    ...
    'column_tests_feature_weighting': {
        ...
        'weights': { # title and effect per parameter predicted in this order: a1, b1, a2, b2, a3, b3, a4, b4
            ...
            'acid_soluble_%': ['Acid Soluble Cu (%norm)',  1, 1, 1, 1, 0, 0, 0, 0], 
            'residual_cpy_%': ['Residual Chalcopyrite (%norm)', -1, -1, -1, -1, 1, 1, 1, 1],
            ...
            'material_size_p80_in': ['Material Size P80 (in)', -1, -1, -1, -1, -1, -1, -1, -1],
            # 'reactors_PCA1': ['Reactors PCA1', 0.0],
            # 'reactors_PCA2': ['Reactors PCA2', 0.0],
            'grouped_copper_sulfides': ['Copper Sulphides (%)', 1, 1, 1, 1, 1, 1, 1, 1],
            'grouped_secondary_copper': ['Secondary Copper (%)', 1, 1, 1, 1, 1, 1, 1, 1],
            'grouped_acid_generating_sulfides': ['Acid Generating Sulphides (%)', 0, 1, 0, 1, 0, 1, 0, 1],
            ...
            'grouped_fe_oxides': ['Fe Oxides (%)', -1, -1, -1, -1, -1, -1, -1, -1],
            # 'grouped_accessory_misc': ['Accessory Misc (%)', 0.0],
            'grouped_carbonates': ['Carbonates (%)', -1, 1, -1, 1, -1, 1, -1, 1],
    ...
    'adaptive_penalty': {
        ...
        'max_penalty_weight': 10.0,        # Maximum penalty weight
    },
    ...
    # 'skip_hypersearch': True, # whether to skip hyperparameter search (set to True during hypersearch)
    'skip_hypersearch_if_single': True,  # skip CV/hypersearch if only one combo
}

HYPERPARAMETER_SPACE = {
    'pytorch_hidden_dim': [128],
    'pytorch_dropout_rate': [0.33],
...
def find_best_config_for_fold(X_tensor, y_tensor, time_tensor, catalyst_tensor, tt_tensor,
    ...
    # Build combinations from the global hyperparameter space
    ...
    # NEW: skip CV/hypersearch if only one combination
    if len(combinations) == 1 and CONFIG.get('skip_hypersearch_if_single', True):
        hd, dr, init_mode, sched = combinations[0]
        print(f"Skipping hyperparameter search for {test_sample_id}: single combination detected.")
        return {
            'test_sample_id': test_sample_id,
            'pytorch_hidden_dim': hd,
            'pytorch_dropout_rate': dr,
            'init_mode': init_mode,
            'scheduler_type': sched,
            'val_loss': np.nan  # no CV was run
        }
...
def train_reactor_scaling_model_parallel(X_tensor, y_tensor, time_tensor, catalyst_tensor, 
    ...
    # Early exit: skip CV/hypersearch and the first training pass if only one combo
    combos_len = (
        len(HYPERPARAMETER_SPACE['pytorch_hidden_dim']) *
        len(HYPERPARAMETER_SPACE['pytorch_dropout_rate']) *
        len(HYPERPARAMETER_SPACE['init_mode']) *
        len(HYPERPARAMETER_SPACE['scheduler_type'])
    )
    print(f"Hypersearch combinations: {combos_len} (skip_if_single={config.get('skip_hypersearch_if_single', True)})")

    if combos_len == 1 and config.get('skip_hypersearch_if_single', True):
        hd = HYPERPARAMETER_SPACE['pytorch_hidden_dim'][0]
        dr = HYPERPARAMETER_SPACE['pytorch_dropout_rate'][0]
        init_mode = HYPERPARAMETER_SPACE['init_mode'][0]
        sched = HYPERPARAMETER_SPACE['scheduler_type'][0]

        print("Single hyperparameter combination detected. Skipping hypersearch and first training pass.")
        final_config = copy.deepcopy(config)
        final_config['pytorch_hidden_dim'] = hd
        final_config['pytorch_dropout_rate'] = dr
        final_config['init_mode'] = init_mode
        final_config['adaptive_lr']['scheduler_type'] = sched
        final_config['generate_plots'] = True
        final_config['skip_hypersearch'] = True

        # Run only the final training pass per fold
        final_results = []
        final_models = []
        final_val_losses = []
        ...
        # Create ensemble (best_configs not used downstream; pass the single config)
        all_best_configs = [{
            'pytorch_hidden_dim': hd,
            'pytorch_dropout_rate': dr,
            'init_mode': init_mode,
    ...
    # ELSE: multi-combo path (keep existing hypersearch + final training)
    all_models = []
    all_val_losses = []
    all_results = []
    all_best_configs = []

    # Build fold_results via the first pass (hypersearch per fold)
    with Pool(processes=mp.cpu_count()) as pool:
        fold_results = list(tqdm(pool.imap(
            partial(process_fold,
                    ...
                    sample_col_ids=sample_col_ids,
                    num_cols=num_cols,
                    # Important: allow hypersearch in this first pass
                    config={**config, 'skip_hypersearch': False, 'generate_plots': False},
                    device=device,
                    save_dir=save_dir,
                    use_val_data_days=use_val_data_days),
            enumerate(unique_sample_ids)
    ...
    # Decide overall best config
    single_combo_only = all_configs_df[['pytorch_hidden_dim','pytorch_dropout_rate','init_mode','scheduler_type']].drop_duplicates().shape[0] == 1

    if config.get('skip_hypersearch', False) or single_combo_only:
        # Use the single (or fixed) config directly; avoid extra selection/training
        best_overall = all_configs_df.iloc[0][['pytorch_hidden_dim','pytorch_dropout_rate','init_mode','scheduler_type']]
        # keep interface consistent
        if 'val_loss' in all_configs_df.columns:
            best_overall['val_loss'] = all_configs_df['val_loss'].mean()
        print("Using provided/single configuration; skipping overall best selection.")
    else:
    ...
    # Save best_overall
    pd.DataFrame([best_overall]).to_csv(os.path.join(save_dir, 'plots', 'best_overall_config.csv'), index=False)
...
def create_ensemble_predictions_with_uncertainty(ensemble, X_tensor, y_tensor, time_tensor, catalyst_tensor, 
    ...
    summary = []
    unique_sample_ids = list(set(sample_ids))
    alpha = 0.10 # 90% predictive interval (empirical 5th–95th percentiles)
    ...
    for sample_id in unique_sample_ids:
        ...
        for idx in sample_indices:
            ...
            # Empirical predictive interval (used for both control and catalyzed)
            lower_pi_2d, upper_pi_2d = ensemble.predictive_interval(
                x_sample, extended_catalyst, tt_sample, extended_time_tensor, batch_sample_ids, alpha=alpha
            )
            lower_pi = lower_pi_2d.squeeze()
            upper_pi = upper_pi_2d.squeeze()
            ...
            if is_catalyzed:
                ...
                lower_pi_filtered = lower_pi[post_transition_mask]
                upper_pi_filtered = upper_pi[post_transition_mask]

                if len(full_time_filtered) > 0:
                    plt.plot(full_time_filtered, mean_pred_filtered, '-',
                             label=f'Predicted ({col_id})\nRMSE: {rmse:.2f}, Bias: {bias:.2f}\nR²: {r2:.2f}, CL: {cl:.0f}%',
                             color=colors[is_catalyzed])
                    # Use empirical 90% PI (ONLY)
