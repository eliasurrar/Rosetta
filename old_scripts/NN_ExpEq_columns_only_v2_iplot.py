...
BASE_SCRIPT_PATH = os.path.join(THIS_DIR, "NN_ExpEq_columns_only_v2.py")
MINERALOGY_CLUSTERING_PATH = os.path.join(THIS_DIR, "rosetta_mineralogy_clustering.py")


def load_base_module() -> Any:
    spec = importlib.util.spec_from_file_location("nn_expeq_columns_only_v2_base", BASE_SCRIPT_PATH)
...
def candidate_project_roots() -> List[str]:
    ...
    for attr in ["PROJECT_ROOT", "DEFAULT_PROJECT_ROOT", "LOCAL_PROJECT_ROOT"]:
        ...
        if isinstance(value, str) and value not in roots:
            roots.append(value)
    sibling_root = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v2")
...
def resolve_project_root(requested_root: str | None = None) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v2.py first so it writes the checkpoint paths listed in outputs/run_manifest.json.\n"
        f"Searched:\n{searched}"
    )
...
def resolve_member_model_dir(project_root: str) -> str:
    ...
    raise FileNotFoundError(
        ...
        "Run NN_ExpEq_columns_only_v2.py first and then use the checkpoint directory recorded in outputs/run_manifest.json.\n"
        f"Searched:\n{searched_text}"
    )
...
def graph_export_config() -> Dict[str, Any]:
    return {
        ...
        "toImageButtonOptions": {
            "format": "png",
            "filename": "interactive_ensemble_prediction_v2_300dpi",
...
def load_saved_member_models(project_root: str) -> Tuple[List[Dict[str, Any]], str, List[str]]:
    ...
    if len(model_paths) == 0:
        raise FileNotFoundError(
            ...
            "Run NN_ExpEq_columns_only_v2.py first to generate them."
        )
    ...
    for path in model_paths:
        ...
        try:
            ...
            raise RuntimeError(
                ...
                "Re-run NN_ExpEq_columns_only_v2.py to regenerate the saved member models with the current model structure."
            ) from exc
        model.eval()
        members.append(
            {
    ...
    if len(members) == 0:
        ...
        raise RuntimeError(
            ...
            "Re-run NN_ExpEq_columns_only_v2.py to regenerate them.\n"
            f"Skipped:\n{skipped_text}"
        )
    ...
    if np.isfinite(volume_m3) and volume_m3 > 0.0:
        ...
        if loading_estimate.get("used_fallback", False):
            note = (
                ...
                f"The current hidden model input is feed_mass_kg={estimated_mass:.2f} kg."
            )
        else:
            note = (
                "Feed mass is hidden from the UI and auto-estimated from column volume plus a calibrated "
                f"Rosin-Rammler sphere-loading approximation using P80={p80_in:.3f} in and shape n={loading_estimate['shape_n']:.1f}. "
                f"The resulting hidden model input is feed_mass_kg={estimated_mass:.2f} kg."
            )
    else:
        note = (
            "Feed mass is hidden from the UI and falls back to the internal default estimate until "
            f"a valid column height and diameter define the column volume. The current hidden model input is feed_mass_kg={estimated_mass:.2f} kg."
        )
    return out, {
        "resolved": True,
...
def make_prediction_figure(pred: Dict[str, Any], max_day: float) -> go.Figure:
    ...
    fig.update_layout(
        ...
        title="Interactive Ensemble Prediction (v2)",
        height=760,
...
def build_prediction_summary(
    ...
    apparent_density_text = f"{apparent_density:.4f} t/m3" if np.isfinite(apparent_density) else "n/a"
    ...
    summary_lines: List[Any] = [
        ...
        html.Div(
            f"Internal geometry values: Column volume {geometry_values.get('column_volume_m3', np.nan):.4f} m3 | "
...
def create_app(project_root: str) -> Dash:
    ...
    def download_predictions_excel(
        ...
        *predictor_values: float,
        ...
        return dcc.send_bytes(excel_bytes, "interactive_ensemble_prediction_v2_7day.xlsx")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Interactive Plotly/Dash app for NN_ExpEq_columns_only_v2 using the saved "
            "member checkpoints created after running NN_ExpEq_columns_only_v2.py."
        )
    )
    ...
    parser.add_argument(
        "--project-root",
        default=None,
        help="Optional override for the NN_Pytorch_ExpEq_columns_only_v2 root.",
    )
