...
DEFAULT_PROJECT_ROOT = (
    "/Users/administration/Library/CloudStorage/OneDrive-JettiResources/"
    "Rosetta/NN_Pytorch_ExpEq_columns_only_v10"
)
LOCAL_PROJECT_ROOT = os.path.join(THIS_DIR, "NN_Pytorch_ExpEq_columns_only_v10")
...
CONFIG = {
    ...
    "loss_weights": {
        ...
        "control_early_fit": 1.15,
        "control_tail_fit": 0.55,
    ...
    for sample_id, group in grouped:
        ...
        for idx, row in group.iterrows():
            ...
            static_vec = validate_required_feature_vector(
                static_vec,
                STATIC_PREDICTOR_COLUMNS,
                f"sample_id={sample_id} row_index={idx} static predictors",
            )
        ...
        merged_static = combine_static_vectors(
            ...
            context=f"sample_id={sample_id} static predictors",
        )
        merged_input_only = combine_static_vectors(
            ctrl_input_only,
            cat_input_only,
            columns=INPUT_ONLY_COLUMNS,
            context=f"sample_id={sample_id} input-only predictors",
        )
...
def apply_static_transformers(
    ...
    for p, xi, xs in zip(pairs, X_imp, X_scaled):
        validate_required_feature_vector(
            xi,
            STATIC_PREDICTOR_COLUMNS,
            f"sample_id={p.sample_id} static_imputed",
        )
        validate_required_feature_vector(
            xs,
            STATIC_PREDICTOR_COLUMNS,
            f"sample_id={p.sample_id} static_scaled",
        )
...
class PairCurveNet(nn.Module):
    ...
    def _compute_sample_leach_caps_torch(
        ...
        ctrl_cap = 100.0 * (pct_ox * ox + pct_sec * sec + pct_pri_ctrl * pri) / cu_safe
        cat_cap = 100.0 * (pct_ox * ox + pct_sec * sec + pct_pri_cat * pri) / cu_safe
...
def aggregate_ensemble_predictions(
    ...
    for pair in pairs:
        ...
        rec = {
            ...
            "mid_life_days": float(np.mean([float(r.get("mid_life_days", np.nan)) for r in member_recs])),
            "catalyst_use_frac": float(np.mean([float(r.get("catalyst_use_frac", np.nan)) for r in member_recs])),
            "active_catalyst_mean": float(np.mean([float(r.get("active_catalyst_mean", np.nan)) for r in member_recs])),
            "active_catalyst_peak": float(np.mean([float(r.get("active_catalyst_peak", np.nan)) for r in member_recs])),
            "active_catalyst_recent": float(np.mean([float(r.get("active_catalyst_recent", np.nan)) for r in member_recs])),
...
def main() -> None:
    ...
    for split in cv_splits:
        ...
        val_pairs_metrics_seed = [pairs_observed[i] for i in val_indices]  # Observed-only validation metrics / OOF reporting
        # val_pairs_seed = [pairs_observed[i] for i in val_indices]
